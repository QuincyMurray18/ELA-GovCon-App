...
def save_opportunities(df):
    ...
    for _, r in df.iterrows():
        cur.execute("""insert into opportunities
            ...
            values(?,?,?,?,?,?,?,?,?,?,?,?)""",
            (r.sam_notice_id, r.title, r.agency, r.naics, r.psc, r.place_of_performance,
             r.response_due, r.posted, r.type, r.url, r.attachments_json, "New"))
    conn.commit()
...
# ===== end app.py =====
# === ELA Enhancements Pack (SOW extraction, Pricing & Schedule, Past Performance Helper, CO-style RFP Analyzer, Proposal Checklist) ===
import math
...
def _ela_extract_key_dates(text: str):
    result = {}
    m = re.search(r"(no later than|due|offers shall be submitted).*?([A-Z][a-z]+ \d{1,2}, \d{4}).{0,40}?(\d{1,2}:\d{2}\s*[AP]M)", text, flags=re.I | re.S)
    if m:
        result["offer_due"] = f"{m.group(2)} {m.group(3)}"
    q = re.search(r"questions.*?no later than.*?([A-Z][a-z]+ \d{1,2}, \d{4}).{0,40}?(\d{1,2}:\d{2}\s*[AP]M)", text, flags=re.I | re.S)
    if q:
        result["questions_due"] = f"{q.group(1)} {q.group(2)}"
    emails = re.findall(r"[A-Za-z0-9\.\-_]+@[A-Za-z0-9\.\-_]+\.[A-Za-z]{2,}", text)
    if emails:
        result["emails"] = list(dict.fromkeys(emails))
    if re.search(r"site visit", text, flags=re.I):
        result["site_visit"] = "Yes"
    n = re.search(r"NAICS.*?(\d{6})", text, flags=re.I)
    if n:
        result["naics"] = n.group(1)
    return result

def _ela_extract_eval_criteria(text: str):
    out = {"factors": [], "notes": []}
    factors = re.findall(r"FACTOR\s+([IVX]+)\s*[\u2013\-]?\s*([^\n]+)", text, flags=re.I)
    for roman, title in factors:
        out["factors"].append(f"Factor {roman} {title.strip()}")
    subs = re.findall(r"Sub[\-\s]?Factor\s+\d+\s*[\u2013\-]?\s*([^\n]+)", text, flags=re.I)
    for s in subs:
        out["factors"].append(f"Sub Factor {s.strip()}")
    if re.search(r"price.*primary evaluation factor", text, flags=re.I):
        out["notes"].append("Price is the primary factor for award")
    if re.search(r"unknown past performance.*considered acceptable", text, flags=re.I):
        out["notes"].append("Unknown past performance considered acceptable")
    if re.search(r"options.*evaluat", text, flags=re.I):
        out["notes"].append("Options prices will be evaluated")
    return out

def _ela_extract_sow_sections(text: str):
    sections = {
        "scope_of_work": "",
    ...
    m = re.search(r"(1\.0\s*Background and Purpose[\s\S]*?)(?:\n\d+\.\d|\Z)", text, flags=re.I)
    if m:
        sections["scope_of_work"] = m.group(1).strip()
    t = re.search(r"(3\.0\s*Technical.*?Tasks[\s\S]*?)(?:\n\d+\.\d|\Z)", text, flags=re.I)
    if t:
        sections["technical_specifications"] = t.group(1).strip()
    d = re.search(r"(4\.0\s*Deliverable.*?Schedule[\s\S]*?)(?:\n\d+\.\d|\Z)", text, flags=re.I)
    if d:
        sections["deliverables"] = d.group(1).strip()
        if re.findall(r"7\s*days", d.group(1), flags=re.I):
            sections["timeline_milestones"] = "Advance notice minimum 7 days for each service. Monthly report due by the fifth of each month."
        if re.search(r"fifth.*month", d.group(1), flags=re.I):
            sections["reporting_requirements"] = "Monthly cumulative report due by the fifth day of each month to the COR."
    p = re.search(r"(11\.0\s*Performance Requirement Measures[\s\S]*?)\Z", text, flags=re.I)
    if p:
        sections["performance_metrics"] = p.group(1).strip()
    return sections

def _ela_rfp_score(tech_accept: bool, service_plan_quality: int, past_perf_level: str, price_rank: int, total_bidders: int):
    w_price, w_tech, w_past = 0.6, 0.25, 0.15
    tech_score = (1.0 if tech_accept else 0.0) * (service_plan_quality / 5.0)
    past_map = {"acceptable": 1.0, "unknown": 0.8, "marginal": 0.5, "unacceptable": 0.0}
    pp = past_map.get(past_perf_level.lower(), 0.8)
    price_score = 1.0 - ((price_rank - 1) / max(1, total_bidders - 1)) if total_bidders > 1 else 1.0
    comp = w_price * price_score + w_tech * tech_score + w_past * pp
    return {
        "price_component": round(w_price * price_score, 4),
        "tech_component": round(w_tech * tech_score, 4),
        "past_component": round(w_past * pp, 4),
        "composite": round(comp, 4)
    }

ela_tabs = st.tabs(["SOW Extractor","Pricing & Schedule","Past Performance Helper","CO-style RFP Analyzer","Proposal Checklist"])

with ela_tabs[0]:
    st.subheader("Structured SOW details (ELA)")
    full_text = st.session_state.get("uploaded_full_text") or st.session_state.get("pws_text") or st.session_state.get("sol_text") or ""
    src_choice = st.selectbox("Source", ["PWS","Solicitation","Both"], key="ela_sow_src")
    if src_choice == "PWS":
        full_text = st.session_state.get("pws_text","")
    elif src_choice == "Solicitation":
        full_text = st.session_state.get("sol_text","")
    else:
        full_text = (st.session_state.get("sol_text","") + "\n" + st.session_state.get("pws_text","")).strip()
    pasted = st.text_area("Paste text (optional)", "", height=180, key="ela_sow_paste")
    if pasted.strip():
        full_text = pasted
    if not full_text:
        st.info("Upload/parse files in the main app (e.g., Auto extract) or paste text above.")
    else:
        sections = _ela_extract_sow_sections(full_text)
        c1, c2 = st.columns(2)
        with c1:
            st.markdown("**Scope of work**")
            st.write(sections["scope_of_work"] or "Not found")
            st.markdown("**Technical specifications**")
            st.write(sections["technical_specifications"] or "Not found")
            st.markdown("**Performance metrics**")
            st.write(sections["performance_metrics"] or "Not found")
        with c2:
            st.markdown("**Deliverables**")
            st.write(sections["deliverables"] or "Not found")
            st.markdown("**Timeline and milestones**")
            st.write(sections["timeline_milestones"] or "Not found")
            st.markdown("**Reporting requirements**")
            st.write(sections["reporting_requirements"] or "Not found")
        st.download_button("Download SOW JSON", data=json.dumps(sections, indent=2).encode("utf-8"), file_name="sow_extraction.json", mime="application/json")

with ela_tabs[1]:
    st.subheader("Schedule of Items and Pricing")
    price_file = st.file_uploader("Upload price sheet", type=["xlsx","xls","csv"], key="ela_price_upl")
    df = None
    if price_file is not None:
        try:
            if price_file.name.lower().endswith((".xlsx",".xls")):
    ...
    if df is not None and not df.empty:
        ...
        qty_col = cols.get("qty", cols.get("quantity"))
        price_col = cols.get("unit price", cols.get("unit_price", cols.get("price")))
        if not qty_col or not price_col:
            st.warning("Please include Quantity and Unit Price columns.")
        else:
            work = df.copy()
            work["__qty__"] = pd.to_numeric(work[qty_col], errors="coerce").fillna(0)
            work["__unit__"] = pd.to_numeric(work[price_col], errors="coerce").fillna(0)
            work["Extended"] = work["__qty__"] * work["__unit__"]
            base_total = float(work["Extended"].sum())
            c1, c2, c3, c4 = st.columns(4)
            with c1:
                markup = st.number_input("Prime markup %", value=10.0, min_value=0.0, step=0.5, key="ela_markup")
            with c2:
                overhead = st.number_input("Overhead %", value=5.0, min_value=0.0, step=0.5, key="ela_overhead")
            with c3:
                contingency = st.number_input("Contingency %", value=2.0, min_value=0.0, step=0.5, key="ela_cont")
            with c4:
                rounding = st.selectbox("Rounding", ["None","Nearest $","Nearest $100"], key="ela_round")
            subtotal = base_total * (1 + overhead/100) * (1 + contingency/100)
            proposed = subtotal * (1 + markup/100)
            if rounding == "Nearest $":
                proposed = round(proposed)
            elif rounding == "Nearest $100":
                proposed = round(proposed/100.0)*100.0
            st.markdown(f"**Base total** {_ela_currency(base_total)}")
            st.markdown(f"**Proposed total** {_ela_currency(proposed)}")
            work_out = work.drop(columns=["__qty__","__unit__"]).copy()
            work_out["Prime_markup_pct"] = markup
            work_out["Overhead_pct"] = overhead
            work_out["Contingency_pct"] = contingency
            st.download_button("Download pricing CSV", work_out.to_csv(index=False).encode("utf-8"), "ela_pricing_detail.csv", "text/csv")
    else:
        st.info("Upload a price sheet to begin.")

with ela_tabs[2]:
    st.subheader("Past Performance Helper")
    have_pp = st.selectbox("Do you have relevant past performance?", ["No","Yes"], index=0, key="ela_pp_have")
    if have_pp == "No":
        text_pp = (
            "Past performance statement\n\n"
            "ELA Management LLC acknowledges that we do not have recorded past performance for federal contracts within the "
            "last three years in this NAICS. We have an established safety culture, standard operating procedures, and a trained team "
            "to deliver the required services. We will manage quality through pre-task planning, checklists aligned to the PWS, and "
            "supervisor sign-off for each visit. Unknown past performance is considered acceptable for evaluation and we welcome the "
            "opportunity to establish a record of successful performance."
        )
        st.text_area("Copy for your Past Performance file", text_pp, height=220, key="ela_pp_text")
        st.download_button("Download statement", data=text_pp.encode("utf-8"), file_name="ela_past_performance_statement.txt", mime="text/plain")
    else:
        tpl = pd.DataFrame([
            {"Contract_number":"", "Title":"", "Role":"Prime or Sub", "Total_value":"", "Period_of_performance":"", "Location":"", "Customer_contact":"", "Relevance":"Similar scope and size"}
        ])
        st.dataframe(tpl, use_container_width=True)
        st.download_button("Download template CSV", data=tpl.to_csv(index=False).encode("utf-8"), file_name="ela_past_performance_template.csv", mime="text/csv")

with ela_tabs[3]:
    st.subheader("RFP Analyzer (CO-style weighting)")
    col1, col2, col3, col4, col5 = st.columns(5)
    with col1:
        tech_accept = st.selectbox("Technical acceptable", ["Yes","No"], index=0, key="ela_co_tech") == "Yes"
    with col2:
        service_plan_quality = st.slider("Service plan quality (1-5)", 1, 5, 4, key="ela_co_plan")
    with col3:
        past_perf_level = st.selectbox("Past performance", ["Acceptable","Unknown","Marginal","Unacceptable"], index=1, key="ela_co_pp")
    with col4:
        total_bidders = st.number_input("Total bidders", min_value=1, value=4, step=1, key="ela_co_bidders")
    with col5:
        price_rank = st.number_input("Your price rank (1=lowest)", min_value=1, value=2, step=1, key="ela_co_rank")
    res = _ela_rfp_score(tech_accept, service_plan_quality, past_perf_level, price_rank, total_bidders)
    cA, cB, cC = st.columns(3)
    cA.metric("Price component", res["price_component"])
    cB.metric("Technical component", res["tech_component"])
    cC.metric("Past perf component", res["past_component"])
    st.metric("Composite score", res["composite"])
    st.caption("Tip: Raise plan quality, keep price competitive, and confirm unknown past performance acceptance where applicable.")

with ela_tabs[4]:
    st.subheader("Proposal Checklist")
    checklist = [
        "Technical plan and staffing",
        "Past performance file or unknown statement",
        "Completed price sheet with totals",
        "Signed reps and certs (from SAM)",
        "Acknowledgment of any amendments",
        "Submission email prepared to POCs",
        "Calendar reminder for monthly reporting after award"
    ]
    done = []
    for item in checklist:
        if st.checkbox(item, key=f"ela_ck_{item}"):
            done.append(item)
    cmp_name = st.session_state.get("company_name", "ELA Management LLC")
    pkg = {"company": cmp_name, "completed_items": done, "timestamp": datetime.utcnow().isoformat() + "Z"}
    st.download_button("Download checklist JSON", data=json.dumps(pkg, indent=2).encode("utf-8"), file_name="ela_proposal_package_checklist.json", mime="application/json")
# === End Enhancements ===
