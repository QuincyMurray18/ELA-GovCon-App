...
# Data layer
# =========================================================
def _ensure_settings_schema(conn: sqlite3.Connection):
    try:
        cols = {row[1] for row in conn.execute("PRAGMA table_info(settings)").fetchall()}
        if "val" not in cols:
            try:
                conn.execute("ALTER TABLE settings ADD COLUMN val TEXT")
            except sqlite3.OperationalError:
                conn.execute("CREATE TABLE IF NOT EXISTS _settings_new(key TEXT PRIMARY KEY, val TEXT)")
                if "value" in cols:
                    conn.execute("INSERT OR IGNORE INTO _settings_new(key, val) SELECT key, value FROM settings")
                else:
                    conn.execute("INSERT OR IGNORE INTO _settings_new(key, val) SELECT key, '' FROM settings")
                conn.execute("DROP TABLE settings")
                conn.execute("ALTER TABLE _settings_new RENAME TO settings")
        conn.commit()
    except sqlite3.OperationalError:
        pass
