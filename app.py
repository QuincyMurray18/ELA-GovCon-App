...
with tabs[11]:
    ...
    # Sidebar branding + user picker (safe try/except)
    try:
...
with tabs[0]:
    ...
    if isinstance(sam_df, pd.DataFrame) and not sam_df.empty:
        ...
        with c1:
            if IN_SAM_TAB:
                ...
                if "select" not in sam_df.columns:
                    sam_df["select"] = False
                view_cols = ["select"] + [c for c in show_cols if c in sam_df.columns]
                grid = st.data_editor(
                    sam_df[view_cols],
                    use_container_width=True,
                    num_rows="fixed",
        ...
        if pick_id and isinstance(sam_df, pd.DataFrame) and "sam_notice_id" in sam_df.columns:
