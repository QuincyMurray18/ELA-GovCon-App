...
def _resolve_notice_id(notice: dict) -> str:
    """Return a SAM.gov notice ID from a notice dict. Accepts several field names and parses URLs."""
    ...
    for k in ["Notice ID","NoticeId","notice_id","noticeId","id","opp_id","Opp ID"]:
        ...
        if v and len(v) >= 32:
            return v
    # Parse from any link-like fields
    for k in ["SAM Link","SAM.gov Link","Link","URL","Sam Link","sam_url","sam"]:
        v = str(notice.get(k) or "").strip()
        if not v:
            continue
        # New SAM.gov pattern: /opp/<uuid>/view or query param noticeId=<uuid>
        m = _re.search(r"/opp/([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})", v, _re.I)
        if m:
            return m.group(1)
        m = _re.search(r"[?&](?:noticeId|notice_id)=([0-9a-f-]{36})", v, _re.I)
        if m:
            return m.group(1)
    return ""

def _ensure_rfp_for_notice(conn, notice: dict) -> int:
    """Ensure rfps row exists for given notice. Returns rfp_id or 0 if unresolved."""
    from contextlib import closing as _closing
    import streamlit as st
    nid = _resolve_notice_id(notice)
    if not nid:
        # Offer inline fix UI once
        st.warning("No Notice ID found. Select a notice first. Hint: set SAM Link on the row or open details so we can parse the id.")
        with st.expander("Paste SAM.gov link or Notice ID"):
            user_in = st.text_input("SAM.gov link or Notice ID", key="deal_notice_input")
            if user_in:
                # Try parse again
                nid2 = user_in.strip()
                m = _re.search(r"([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})", nid2, _re.I)
                if m:
                    nid = m.group(1)
                else:
                    # Not a uuid. keep empty
                    nid = ""
        if not nid:
            return 0
    ...
    with _closing(conn.cursor()) as cur:
        ...
        # Build minimal columns
        title = str(notice.get("Title") or notice.get("title") or "").strip()
        solnum = str(notice.get("Solicitation") or notice.get("Solicitation Number") or notice.get("solnum") or "").strip()
        sam_url = str(notice.get("SAM Link") or notice.get("SAM.gov Link") or notice.get("Link") or notice.get("URL") or "").strip()
        cur.execute(
...
def _x3_open_modal(conn, notice: dict):
    """Open the RFP Analyzer modal for a selected notice."""
    import streamlit as st
    st.session_state["x3_show_modal"] = True
    st.session_state["x3_notice"] = dict(notice or {})
    try:
        st.rerun()
    except Exception:
        pass
...
def _x3_render_modal(conn, notice: dict):
    ...
    if not rfp_id:
        return
    st.caption(f"RFP #{rfp_id} · {notice.get('Title','')}")
    # existing analyzer UI continues below...
