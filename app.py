# ===== app.py =====
# GovCon Copilot Pro — keysafe, GPT-5 fallbacks, NAICS/Goals, SubK finder (Google Places diag),
# SAM Watch (header+query key, diagnostics+raw), RFP tools, White Paper, Auto Extract,
# Ask-the-Doc, and Chat Assistant w/ context + file uploads.
...
def google_places_search(query, location="Houston, TX", radius_m=80000, strict=True):
    ...
    Returns (list_of_vendors, info_dict). If key missing or API errors, info explains why.
    strict=True: return [] on non-OK; strict=False: best-effort parse.
    """
    ...
    try:
        ...
        info = {
            "ok": (status_code == 200 and api_status in ("OK","ZERO_RESULTS")),
            "http": status_code,
            "api_status": api_status,
            "count": len(results),
            "note": "Enable billing + Places API; some queries require 'location bias' to return results.",
            "raw_preview": (r.text or "")[:800],
        }

        if status_code != 200:
            return ([] if strict else results), {**info, "ok": False, "reason": "http_error"}
        if api_status not in ("OK","ZERO_RESULTS"):
            return ([] if strict else results), {**info, "ok": False, "reason": api_status or "api_error"}
...
def sam_search(
    ...
    try:
        ...
        if df.empty:
            info["hint"] = "Try min_days=0–1, add a keyword, increase postedFrom, or clear noticeType for a broad test."
        return df, info
...
# ---------- SAM Watch (diagnostics + keyword + postedFrom + raw) ----------
with tabs[4]:
