...
def _strip_markdown_to_plain(txt: str) -> str:
    ...
    s = s.replace("`", ")# Bold/italic markers
    s = s.replace("***", ")s = s.replace("**", ")s = s.replace("*", ")s = s.replace("__", ")s = s.replace("_", ")# Strip heading markers at line starts
...
def _ensure_drafts_dir():
    base = os.path.join(os.getcwd(), "drafts", "proposals)os.makedirs(base, exist_ok=True)
    return base

def save_proposal_draft(title: str, content_md: str) -> str:
    base = _ensure_drafts_dir()
    safe = re.sub(r'[^A-Za-z0-9_.-]+', '_', title.strip() or "untitled)ts = datetime.now().strftime("%Y%m%d_%H%M%S)fname = f"{ts}__{safe}.md"
    path = os.path.join(base, fname)
    with open(path, "w", encoding="utf-8)as f:
        f.write(content_md or ")return path
...
def _render_markdown_to_docx(doc, md_text):
    ...
    def flush_bullets():
        ...
        for item in bullet_buf:
            _add_paragraph_with_inlines(doc, item, style="List Bullet)bullet_buf = []

    def flush_numbers():
        nonlocal num_buf
        for item in num_buf:
            _add_paragraph_with_inlines(doc, item, style="List Number)num_buf = []
...
def _mail_store_path():
    base = os.path.join(os.getcwd(), "secure_auth)os.makedirs(base, exist_ok=True)
    return os.path.join(base, "mail.json)def _load_mail_store():
    try:
...
def set_user_smtp_app_password(user: str, app_password: str):
    ...
    u["smtp_port"] = 587
    u["username"] = USER_EMAILS.get(user, ")u["app_password_b64"] = base64.b64encode((app_password or ").encode("utf-8)).decode("ascii)store[user] = u
...
def send_outreach_email(user: str, to_addrs, subject: str, body_html: str, cc_addrs=None, bcc_addrs=None, attachments=None):
    ...
    if not cfg or not cfg.get("username)or not cfg.get("password):
        raise RuntimeError(f"No email credentials configured for {user}. Set a Gmail App Password in the sidebar.)msg = EmailMessage()
    ...
    if not to_list:
        raise RuntimeError("Please provide at least one recipient in To.)msg["To"] = ", ".join(to_list)
    if cc_list: msg["Cc"] = ", ".join(cc_list)

    import re as _re
    plain = _re.sub("<[^<]+?>", "", body_html or ")if body_html else ""
    msg.set_content(plain or "(no content))if body_html:
        msg.add_alternative(body_html, subtype="html)attachments = attachments or []
    for att in attachments:
        try:
            content = att.getvalue()
            msg.add_attachment(content, maintype="application", subtype="octet-stream", filename=att.name)
        except Exception as e:
            raise RuntimeError(f"Failed to attach {getattr(att,'name','file')}: {e})all_rcpts = to_list + cc_list + bcc_list
...
def _pin_storage_path():
    base = os.path.join(os.getcwd(), "secure_auth)os.makedirs(base, exist_ok=True)
    return os.path.join(base, "pins.json)def _load_pin_store():
...
def _do_login():
    with st.sidebar:
        st.header("Sign in)user = st.selectbox("User", USERS, index=0, key="login_user_select)pin_ok = True
        if PINS:
            pin = st.text_input("PIN", type="password", key="login_pin_input)pin_ok = _verify_pin(user, pin)

        if st.button("Sign in", use_container_width=True, key="login_btn):
            if pin_ok:
                st.session_state["active_user"] = user
                st.session_state.setdefault("private_mode", True)
                st.success(f"Signed in as {user})else:
                st.error("Incorrect PIN)if "active_user" not in st.session_state:
        st.stop()
...
with st.sidebar:
    ...
    if st.session_state.get("active_user):
        st.caption(f"Signed in as {st.session_state['active_user']})if st.button("Sign out", use_container_width=True, key="logout_btn):
...
_selected = st.session_state.get("login_user_select)_active = st.session_state.get("active_user)if _active and _selected and _selected != _active:
    with st.sidebar:
        st.warning(f"You selected {_selected}. To switch from {_active}, click below then sign in.)if st.button(f"Switch to {_selected}", use_container_width=True, key="switch_user_btn):
...
with st.sidebar:
    st.subheader("Workspace)with st.sidebar:
    st.subheader("Security)with st.expander("Change My PIN", expanded=False):
        st.write("Update your sign-in PIN. New PIN must be 4–12 characters.)curr = st.text_input("Current PIN", type="password", key="pin_cur)new1 = st.text_input("New PIN", type="password", key="pin_new1)new2 = st.text_input("Confirm New PIN", type="password", key="pin_new2)if st.button("Update PIN", use_container_width=True, key="pin_update_btn):
            if not _verify_pin(ACTIVE_USER, curr or ''):
                st.error("Current PIN is incorrect.)elif not new1 or len(new1) < 4 or len(new1) > 12:
                st.error("New PIN must be 4–12 characters.)elif new1 != new2:
                st.error("New PINs do not match.)else:
                set_user_pin(ACTIVE_USER, new1)
                # Clear any cached login input
                st.session_state.pop("login_pin_input", None)
                st.success("Your PIN has been updated. It will be required next time you sign in.)st.session_state.setdefault(f"{ACTIVE_USER}::private_mode", True)
    NS["private_mode"] = st.toggle(
        "Private mode",
        value=NS.get("private_mode", True),
        help="When ON your changes stay private to you until you publish.)def queue_change(fn, *, label: str):
...
def write_or_queue(label, commit_fn):
    if NS.get("private_mode", True):
        ...
        st.info(f"Saved privately. Publish later. [{label}])else:
        commit_fn()
        st.success(f"Saved to team. [{label}])with st.sidebar:
    if st.button("Publish my changes", use_container_width=True, key="publish_btn):
        errs = publish_changes()
        if not errs:
            st.success("All your private changes are now published to the team data.)else:
            st.error("Some changes failed to publish. See below for details.)for label, e in errs:
...
def send_outreach_email(user: str, to_addrs, subject: str, body_html: str, cc_addrs=None, bcc_addrs=None, attachments=None):
    ...
    msg.set_content(plain or "(no content))if body_html:
        msg.add_alternative(body_html, subtype="html)# Attachments
...
def render_outreach_tools():
        ...
        for name in ("send_outreach_message", "send_gmail_message", "send_mail", "outreach_send):
            ...
            if callable(fn):
                try:
                    ...
                    last_err = e
        raise last_err or RuntimeError("No outreach sender is available)# ---------- Stable session keys ----------
    ...
    st.session_state.setdefault(SKEY_LASTSIG, ")from_addr = USER_EMAILS.get(ACTIVE_USER, ")# ---------- Header ----------
    with st.container(border=True):
        top_l, top_r = st.columns([3,2])
        with top_l:
            st.markdown("### ✉️ Outreach)st.caption(f"From: **{from_addr}**" if from_addr else "No email configured for this user.)with top_r:
            pass

    # ---- Attachments: Global uploader (available before generation) ----
    with st.container(border=True):
        st.markdown("#### )extra_files = st.file_uploader("", type=None, accept_multiple_files=True, key=ns_key("outreach::extra_files", label_visibility="collapsed", label_visibility="collapsed))
    ...
    with st.expander("Set/Update my Gmail App Password", expanded=False):
        ...
        if st.button("Save App Password", key=ns_key("outreach::save_app_pw)):
            try:
                ...
                st.success("Saved)except Exception as e:
                st.error(f"Failed to save: {e})st.divider()
    
    # ---------- Choose Generated Email & Attachments (required) ----------
    with st.container(border=True):
        st.markdown("#### Choose Generated Email)mb = st.session_state.get("mail_bodies)or []
        if not mb:
            st.info("Generate emails to select one for preview.", icon="ℹ️)else:
            ...
            st.caption(f"**To:** {sel.get('to','')})st.caption(f"**Subject:** {sel.get('subject','')})scope_disp = sel.get("scope_summary)or sel.get("scope)or ""
            due_disp = sel.get("quote_due)or sel.get("due)or ""
            meta_cols = st.columns(2)
            with meta_cols[0]:
                st.markdown(f"**Scope Summary:** {scope_disp})with meta_cols[1]:
                st.markdown(f"**Quote Due:** {due_disp})# Attachments uploader (REQUIRED) placed below Quote Due
            extra_files = st.file_uploader("", label_visibility="collapsed),  type=None, accept_multiple_files=True,
            ...
            if st.button("Generate preview", key=ns_key("outreach::gen_preview), use_container_width=True):
                ...
                if not files:
                    st.warning("Please upload at least one attachment before generating the preview.)else:
                    ...
                    st.success("Preview generated below.)actions2 = st.columns([1, 2, 2, 5])
            with actions2[1]:
                if st.button("Send selected now", key=ns_key("outreach::send_selected_now), use_container_width=True):
                    files = st.session_state.get(SKEY_ATTACH) or []
                    if not files:
                        st.warning("Please upload at least one attachment before sending.)else:
                        try:
                            ...
                            st.success("Selected email sent.)except Exception as e:
                            st.error(f"Failed to send selected: {e})with actions2[2]:
                if st.button("Send ALL generated now", key=ns_key("outreach::send_all_now), use_container_width=True):
                    files = st.session_state.get(SKEY_ATTACH) or []
                    if not files:
                        st.warning("Please upload at least one attachment before mass sending.)else:
    ...
    with st.container(border=True):
        st.markdown("#### Preview)if not snap:
            st.info("Select a generated email above, attach files if needed, and click Preview.", icon="ℹ️)else:
            # Header block similar to Gmail
            hdr_lines = []
            if snap.get("from_addr): hdr_lines.append(f"<div><b>From:</b> {snap['from_addr']}</div>)if snap.get("to):        hdr_lines.append(f"<div><b>To:</b> {snap['to']}</div>)if snap.get("cc):        hdr_lines.append(f"<div><b>Cc:</b> {snap['cc']}</div>)if snap.get("bcc):       hdr_lines.append(f"<div><b>Bcc:</b> {snap['bcc']}</div>)if snap.get("subject):   hdr_lines.append(f"<div style='font-size:16px;margin-top:4px;'><b>Subject:</b> {snap['subject']}</div>)# Meta row: Scope Summary & Quote Due
            meta_bits = []
            if snap.get("scope_summary):
                meta_bits.append("<div style='display:inline-block;border:1px solid #eee;"
                                 ...
                                 f"{snap['scope_summary']}</div>)if snap.get("quote_due):
                meta_bits.append("<div style='display:inline-block;border:1px solid #eee;"
                                 "padding:4px 8px;border-radius:8px;'><b>Quote due:</b> "
                                 f"{snap['quote_due']}</div>)# Attachments uploader (positioned below Quote Due)
            extra_files = st.file_uploader("", label_visibility="collapsed),  type=None, accept_multiple_files=True,
                                           key=ns_key("outreach::extra_files))
            if extra_files is not None:
                st.session_state[SKEY_ATTACH] = extra_files
            ...
            if atts:
                ...
                atts_html = ("<div style='margin-top:8px;'><b>Attachments:</b>"
                             f"<ul style='margin:6px 0 0 20px;'>{items}</ul></div>)components.html(f"""
            ...
            with a1:
                if st.button("Send email", key=ns_key("outreach::send_from_preview), use_container_width=True):
                    try:
                        ...
                        st.success("Email sent.)st.session_state[SKEY_PREVIEW] = None
                    except Exception as e:
                        st.error(f"Failed to send: {e})with a2:
...
def load_outreach_preview(to="", cc="", bcc="", subject="", html="):
    from_addr = USER_EMAILS.get(ACTIVE_USER, ")key = lambda k: f"{ACTIVE_USER}::outreach::{k}"
    ...
    st.subheader("Email – Outreach)from_addr = USER_EMAILS.get(ACTIVE_USER, ")if not from_addr:
        st.caption("No email configured for this user. Only Charles and Collin are set up.)else:
        st.caption(f"From: {from_addr})st.session_state.setdefault(ns_key("outreach::mail_preview_data), None)
    ...
    with st.expander("Set/Update my Gmail App Password", expanded=False):
        st.caption("Generate an App Password in your Google Account > Security > 2-Step Verification.)app_pw = st.text_input("Gmail App Password (16 chars, no spaces),  type="password", key=ns_key("outreach::gmail_app_pw))
        if st.button("Save App Password", key=ns_key("outreach::save_app_pw)):
            set_user_smtp_app_password(ACTIVE_USER, app_pw)
            st.success("Saved. You can now send emails from the Outreach composer.)with st.expander("Quick Outreach Composer", expanded=False):
        to = st.text_input("To (comma-separated),  key=ns_key("outreach::mail_to),
                           placeholder="recipient@example.com, another@domain.com)cc = st.text_input("Cc (optional, comma-separated),  key=ns_key("outreach::mail_cc))
        ...
        body = st.text_area("Message (HTML supported),  key=ns_key("outreach::mail_body), height=200,
                            placeholder="<p>Hello.</p>)files = st.file_uploader("", type=None, accept_multiple_files=True, key=ns_key("outreach::mail_files", label_visibility="collapsed", label_visibility="collapsed))
        ...
        with c2:
            if st.button("Send email", use_container_width=True, key=ns_key("outreach::mail_send_btn)):
                try:
                    ...
                    st.success("Email sent.)for k in ["outreach::mail_to","outreach::mail_cc","outreach::mail_bcc","outreach::mail_subj","outreach::mail_body","outreach::mail_files"]:
                        NS.pop(k, None)
                except Exception as e:
                    st.error(f"Failed to send: {e})preview = st.session_state.get(ns_key("outreach::mail_preview_data))
    if preview:
        import streamlit.components.v1 as components
        with st.container(border=True):
            st.markdown("#### Email preview)st.markdown(f"**From:** {preview.get('from_addr','')})if preview.get("to): st.markdown(f"**To:** {preview['to']})if preview.get("cc): st.markdown(f"**Cc:** {preview['cc']})if preview.get("bcc): st.markdown(f"**Bcc:** {preview['bcc']})st.markdown(f"**Subject:** {preview.get('subject','')})html = preview.get("body_html)or ""
            components.html(
                f"""
        ...
        with cc1:
            if st.button("Send this email", key=ns_key("outreach::mail_preview_confirm)):
                ...
                try:
                    ...
                    st.success("Email sent.)st.session_state[ns_key("outreach::mail_preview_data)] = None
                    for k in ["outreach::mail_to","outreach::mail_cc","outreach::mail_bcc","outreach::mail_subj","outreach::mail_body","outreach::mail_files"]:
                        NS.pop(k, None)
                except Exception as e:
                    st.error(f"Failed to send: {e})with cc2:
            if st.button("Close preview", key=ns_key("outreach::mail_preview_close)):
                st.session_state[ns_key("outreach::mail_preview_data)] = None
    from_addr = USER_EMAILS.get(ACTIVE_USER, ")if not from_addr:
        st.caption("No email configured for this user. Only Charles and Collin are set up.)else:
        st.caption(f"From: {from_addr})# Global preview state
    ...
    with st.expander("Set/Update my Gmail App Password", expanded=False):
        ...
        if st.button("Save App Password", key=ns_key("outreach::save_app_pw)):
            set_user_smtp_app_password(ACTIVE_USER, app_pw)
            st.success("Saved. You can now send emails from the Outreach composer.)# === Quick Outreach Composer ===
        ...
        with c2:
            if st.button("Send email", use_container_width=True, key=ns_key("outreach::mail_send_btn)):
                try:
                    ...
                    st.error(f"Failed to send: {e})# === Unified Preview Block (used by both header-level and composer-level triggers) ===
    preview = st.session_state.get(ns_key("outreach::mail_preview_data))
    if preview:
        import streamlit.components.v1 as components
        with st.container(border=True):
            st.markdown("#### Email preview)st.markdown(f"**From:** {preview.get('from_addr','')})if preview.get("to):
                st.markdown(f"**To:** {preview['to']})if preview.get("cc):
                st.markdown(f"**Cc:** {preview['cc']})if preview.get("bcc):
                st.markdown(f"**Bcc:** {preview['bcc']})st.markdown(f"**Subject:** {preview.get('subject','')})html = preview.get("body_html)or ""
            components.html(
                f"""
                <div style="border:1px solid #ddd;padding:16px;margin-top:8px;">
                    {html}
    ...
    from_addr = USER_EMAILS.get(ACTIVE_USER, ")if not from_addr:
        st.caption("No email configured for this user. Only Charles and Collin are set up.)else:
        st.caption(f"From: {from_addr})with st.expander("Set/Update my Gmail App Password", expanded=False):
        st.caption("Generate an App Password in your Google Account > Security > 2-Step Verification.)app_pw = st.text_input("Gmail App Password (16 chars, no spaces),  type="password", key=ns_key("outreach::gmail_app_pw))
        if st.button("Save App Password", key=ns_key("outreach::save_app_pw)):
            set_user_smtp_app_password(ACTIVE_USER, app_pw)
            st.success("Saved. You can now send emails from the Outreach composer.)# Preview state
    st.session_state.setdefault(ns_key("outreach::mail_preview_data), None)
        ...
        with c2:
            if st.button("Send email", use_container_width=True, key=ns_key("outreach::mail_send_btn)):
                try:
                    ...
                    st.error(f"Failed to send: {e})# If a preview has been requested, render it exactly like the HTML body will appear.
    preview = st.session_state.get(ns_key("outreach::mail_preview_data))
    if preview:
        import streamlit.components.v1 as components

        with st.container(border=True):
            st.markdown("#### Email preview)# Header preview
            st.markdown(f"**From:** {preview.get('from_addr','')})if preview.get("to):
                st.markdown(f"**To:** {preview['to']})if preview.get("cc):
                st.markdown(f"**Cc:** {preview['cc']})if preview.get("bcc):
                st.markdown(f"**Bcc:** {preview['bcc']})st.markdown(f"**Subject:** {preview.get('subject','')})# Render the HTML body using a component so styles and tags are honored
    ...
    from_addr = USER_EMAILS.get(ACTIVE_USER, ")if not from_addr:
        ...
        if st.button("Send email", use_container_width=True, key=ns_key("outreach::mail_send_btn)):
            try:
                ...
                st.error(f"Failed to send: {e})def outreach_send_from_active_user(to, subject, body_html, cc=None, bcc=None, attachments=None):
    return send_outreach_email(ACTIVE_USER, to, subject, body_html, cc_addrs=cc, bcc_addrs=bcc, attachments=attachments)
...
try:
    ...
    _openai_version = getattr(_openai_pkg, "__version__", "unknown)except Exception as e:
    st.warning("OpenAI SDK missing or too old. Chat features disabled until installed.)OpenAI = None

client = OpenAI(api_key=OPENAI_API_KEY) if OPENAI_API_KEY else None
OPENAI_MODEL = os.getenv("OPENAI_MODEL", _get_key("OPENAI_MODEL)or "gpt-5-chat-latest)_OPENAI_FALLBACK_MODELS = [
    OPENAI_MODEL,
...
def _send_via_gmail(to_addr: str, subject: str, body: str) -> str:
    ...
    """
    try:
        smtp_user = st.secrets.get("smtp_user)smtp_pass = st.secrets.get("smtp_pass)except Exception:
    ...
    if smtp_user and smtp_pass:
        ...
        try:
            ...
            try:
                st.warning(f"Gmail SMTP send failed: {e})except Exception:
                pass
    # Fallback to Graph or preview
    try:
        sender_upn = get_setting("ms_sender_upn", ")except Exception:
        sender_upn = ""
    try:
        ...
        try:
            ...
            _st.warning("Email preview mode is active. Configure SMTP or Graph to send.)except Exception:
            pass
        return "Preview"

st.set_page_config(page_title="GovCon Copilot Pro", page_icon="ðŸ§°", layout="wide)# ---- Date helpers for SAM search ----
...
try:
    ...
    def _us_date(dt):
        try:
            return dt.strftime("%m/%d/%Y)except Exception:
...
def send_via_graph(to_addr: str, subject: str, body: str, sender_upn: str = None) -> str:
    ...
    try:
        sender = sender_upn or os.getenv("MS_SENDER_UPN)or get_setting("ms_sender_upn", ")except Exception:
    ...
    try:
        _tenant = os.getenv("MS_TENANT_ID)or get_setting("MS_TENANT_ID", ")or get_setting("ms_tenant_id", ")except Exception:
        _tenant = os.getenv("MS_TENANT_ID)or ""
    try:
        _client_id = os.getenv("MS_CLIENT_ID)or get_setting("MS_CLIENT_ID", ")or get_setting("ms_client_id", ")except Exception:
        _client_id = os.getenv("MS_CLIENT_ID)or ""
    try:
        _client_secret = os.getenv("MS_CLIENT_SECRET)or get_setting("MS_CLIENT_SECRET", ")or get_setting("ms_client_secret", ")except Exception:
    ...
    try:
        token = token_r.json().get("access_token)except Exception:
        token = None
    if not token:
...
def usaspending_search_awards(naics: str = "", psc: str = "", date_from: str = "", date_to: str = "", keyword: str = "", limit: int = 200, st_debug=None):
    ...
    if not date_from or not date_to:
        from datetime import datetime, timedelta
        end = datetime.utcnow().date().strftime("%Y-%m-%d)start = (datetime.utcnow().date() - timedelta(days=365*2)).strftime("%Y-%m-%d)date_from, date_to = date_from or start, date_to or end
...
def google_places_search(query, location="Houston, TX", radius_m=80000, strict=True):
    ...
    try:
        ...
        api_status = data.get("status",")results = data.get("results", []) or []
        ...
        out = []
        for item in results:
            place_id = item.get("place_id)phone, website = "", ""
            if place_id:
...
def build_context(max_rows=6):
    ...
    if not g.empty:
        ...
        goals_line = (f"Bids target {int(rr['bids_target'])}, submitted {int(rr['bids_submitted'])}; "
                      f"Revenue target ${float(rr['revenue_target']):,.0f}, won ${float(rr['revenue_won']):,.0f}.)codes = pd.read_sql_query("select code from naics_watch order by code", conn)["code"].tolist()
...
try:
    ...
    def google_places_search(*args, **kwargs):
        ...
        """
        try:
            query = args[0] if len(args) >= 1 else kwargs.get("query",")loc = args[1] if len(args) >= 2 else kwargs.get("location",")radius_m = args[2] if len(args) >= 3 else kwargs.get("radius_meters", 1609)
        except Exception:
...
st.title("GovCon Copilot Pro)st.caption("SubK sourcing • SAM watcher • proposals • outreach • CRM • goals • chat with memory & file uploads)DB_PATH = "govcon.db"
...
def _ensure_outreach_log_columns(conn):
    cur = conn.cursor()
    try: cur.execute("alter table outreach_log add column error_text text)except Exception: pass
    try: cur.execute("alter table outreach_log add column try_count integer default 0)except Exception: pass
    conn.commit()
...
def normalize_vendor_website(website: str, display_link: str = None):
    ...
    try:
        ...
        if host in bad_hosts:
            return None
        return (u.scheme + "://" + u.netloc + u.path).rstrip("/)except Exception:
        return w

def ensure_indexes(conn):
    cur = conn.cursor()
    try: cur.execute("create index if not exists idx_opp_notice on opportunities(sam_notice_id))except Exception: pass
    try: cur.execute("create index if not exists idx_outreach_vendor on outreach_log(vendor_id))except Exception: pass
    try: cur.execute("create index if not exists idx_rfq_vendor on rfq_outbox(vendor_id))except Exception: pass
    try: cur.execute("create index if not exists idx_tasks_opp on tasks(opp_id))except Exception: pass
    conn.commit()
...
def run_migrations():
    ...
    try: cur.execute("alter table compliance_items add column owner text)except Exception: pass
    try: cur.execute("alter table compliance_items add column snippet text)except Exception: pass
    try: cur.execute("alter table opportunities add column assignee text)except Exception: pass
    try: cur.execute("alter table opportunities add column quick_note text)except Exception: pass
    # vendors table expansions
    try: cur.execute("alter table vendors add column distance_miles real)except Exception: pass
    conn.commit()
...
def ensure_schema():
    ...
    # seed goals
    cur.execute("select count(*) from goals)if cur.fetchone()[0] == 0:
        cur.execute(
...
ELA Management LLC
""))
    cur.execute("select count(*) from naics_watch)if cur.fetchone()[0] == 0:
    ...
    if suffix == "pdf":
        try:
            ...
            try:
                ...
                pass
            return ""
    return uploaded_file.read().decode("utf-8", errors="ignore)def llm(system, prompt, temp=0.2, max_tokens=1400):
    ...
    for model_name in _OPENAI_FALLBACK_MODELS:
        try:
            ...
            if model_name != OPENAI_MODEL:
                try: st.toast(f"Using fallback model: {model_name}", icon="âš™ï¸)except Exception: pass
...
def _validate_text_for_guardrails(md_text: str, page_limit: int = None, require_font: str = None, require_size_pt: int = None,
    ...
    if _re.search(r'\bINSERT\b', text, flags=_re.IGNORECASE):
        issues.append("Placeholder text 'INSERT' detected. Remove before export.)if _re.search(r'\bTBD\b|\bTODO\b', text, flags=_re.IGNORECASE):
        issues.append("Unresolved 'TBD/TODO' placeholders present.)if "<>" in text or "[ ]" in text:
        issues.append("Bracket placeholders found. Replace with final content.)# Page length heuristic: ~450 words per page at 11pt single-space
    words = _re.findall(r'\w+', text)
    est_pages = max(1, math.ceil(len(words) / 450)) if words else 1

    if page_limit and est_pages > page_limit:
        issues.append(f"Estimated length is {est_pages} pages which exceeds the {page_limit}-page limit.)return issues, est_pages
    ...
    for ln in lines:
        ...
        if m:
            ...
            if prev_heading and curr == prev_heading:
                ...
                continue
            out.append(f"{hashes} {text})prev_heading = curr
        else:
            out.append(ln)
...
def _docx_title_if_needed(md_text: str, proposed_title: str) -> str:
    ...
    return "" if re.match(r'^#\s+.+', first) else (proposed_title or ")def _md_to_docx_bytes(md_text: str, title: str = "", base_font: str = "Times New Roman", base_size_pt: int = 11,
                      margins_in: float = 1.0) -> bytes:
    """
    ...
    for raw in lines:
        ...
        # Numbered list approx (e.g., "1. step)if re.match(r"^\d+\.\s+", line):
            flush_bullets()
    ...
    if not body:
        issues.append("No content assembled for export.)return issues, 0

    # Placeholder checks
    placeholders = ["TBD", "INSERT", "[BRACKET]", "{PLACEHOLDER}", "lorem ipsum"]
    for ph in placeholders:
        if ph.lower() in body.lower():
            issues.append(f"Placeholder text '{ph}' detected. Remove before export.)# Word/page estimate
    words = body.split()
    est_pages = max(1, int(len(words) / 500))  # heuristic ~500 words/page
    if page_limit and est_pages > page_limit:
        issues.append(f"Estimated pages {est_pages} exceed limit {page_limit}.)# Font/size are enforced during DOCX build; here we flag if requested but not standard
    if require_font and require_font.lower() not in ("times new roman","arial","calibri","garamond","helvetica):
        warnings.append(f"Requested font '{require_font}' is uncommon for federal proposals.)if require_size_pt and (require_size_pt < 10 or require_size_pt > 13):
        warnings.append(f"Requested font size {require_size_pt}pt is atypical for body text.)# Margins/spacing advisory
    if margins_in is not None and (margins_in < 0.5 or margins_in > 1.5):
        warnings.append(f"Margin {margins_in}\" may violate standard 1\" requirement.)if line_spacing is not None and (line_spacing < 1.0 or line_spacing > 2.0):
        warnings.append(f"Line spacing {line_spacing} looks unusual.)# Filename pattern
    if filename_pattern:
        # Very simple validation tokens
        tokens = ["{company}", "{solicitation}", "{section}", "{date}"]
        if not any(t in filename_pattern for t in tokens):
            warnings.append("Filename pattern lacks tokens like {company} or {date}.)return issues, est_pages
...
def _render_saved_vendors_manager(_container=None):
    import pandas as pd
    _c = _container or st
    _c.markdown("### Saved vendors)try:
        conn = get_db()
    except Exception as e:
        _c.error(f"DB error: {e})return
    try:
        _v = pd.read_sql_query("select * from vendors order by updated_at desc, company", conn)
    except Exception as e:
        _c.warning("Vendors table missing. Creating it now...)try:
            cur = conn.cursor()
            cur.execute("""
            create table if not exists vendors(
            ...
            _c.error(f"Could not create/read vendors table: {ce})return

    if _v.empty:
        _c.info("No vendors saved yet. Use your import above or add one manually below.)# Show empty editor with columns for manual add
        _v = pd.DataFrame([{
    ...
    editor = _c.data_editor(
        ...
        num_rows="dynamic",
        key="vendors_grid_tab1)c1, c2, c3 = _c.columns([1,1,2])
    with c1:
            ...
            try:
                ...
                for _, r in editor.iterrows():
                    ...
                    if vid is None or (isinstance(vid, float) and pd.isna(vid)) or str(vid).strip()=="" :
                        ...
                        updated += 1
                conn.commit()
                _c.success(f"Saved {saved} new, updated {updated} existing)except Exception as se:
                _c.error(f"Save failed: {se})with c2:
        try:
            all_ids = [int(x) for x in editor.get("id", pd.Series(dtype=float)).dropna().astype(int).tolist()]
        except Exception:
            all_ids = []
        del_ids = _c.multiselect("Delete vendor IDs", options=all_ids, key="vendors_del_ids_tab1)if _c.button("Delete selected", key="vendors_del_btn_tab1):
            try:
                if del_ids:
                    cur = conn.cursor()
                    for vid in del_ids:
                    ...
                    conn.commit()
                    _c.success(f"Deleted {len(del_ids)} vendor(s))except Exception as de:
                _c.error(f"Delete failed: {de})with c3:
        _c.caption("Tip: Add a new row at the bottom to create a vendor manually.)TAB_LABELS = [
...
def compute_win_score_row(opp_row, past_perf_df):
    ...
    # Time runway
    try:
        due = _parse_date_any(opp_row.get("response_due)or ")except Exception:
        due = None
...
try:
    with legacy_tabs[-3]:
        st.subheader("Past Performance Library)st.caption("Create reusable blurbs linked by NAICS and agency. Insert into Proposal Builder later.)conn = get_db()
        ...
        with st.form("pp_form", clear_on_submit=True):
            ...
            with col1:
                title = st.text_input("Project title)agency = st.text_input("Agency)naics = st.text_input("NAICS", value=")psc = st.text_input("PSC", value=")period = st.text_input("Period", value=")with col2:
                value_amt = st.number_input("Contract value", min_value=0.0, step=1000.0)
                role = st.text_input("Role", value="Prime)location = st.text_input("Location", value=")highlights = st.text_area("Highlights bullets", height=120, value="• Scope coverage\n• Key metrics\n• Outcomes)contact_name = st.text_input("POC name", value=")contact_email = st.text_input("POC email", value=")contact_phone = st.text_input("POC phone", value=")submit = st.form_submit_button("Save record)if submit:
    ...
    st.caption(f"[Past Performance tab init note: {_e_pp}])# Quote Comparison tab body (last-2)
try:
    with legacy_tabs[-2]:
        st.subheader("Subcontractor Quote Comparison)conn = get_db()
        ...
        if opp_pick:
            ...
            with st.form("qc_add):
                ...
                with cols[1]:
                    lead_time = st.text_input("Lead time", value=")notes = st.text_area("Notes", height=120, value=")files = st.text_input("Files list", value=")add_btn = st.form_submit_button("Save quote line)if add_btn and v_pick:
                ...
                conn.commit()
                st.success("Saved)dfq = pd.read_sql_query("select * from vendor_quotes where opp_id=? order by total asc", conn, params=(opp_id,))
            if dfq.empty:
                st.info("No quotes yet)else:
                ...
                if pick_winner and st.button("Pick Winner):
                    ...
                    st.success(f"Winner selected {pick_winner}. Open Pricing Calculator to model markup.)except Exception as _e_qc:
    st.caption(f"[Quote Comparison tab init note: {_e_qc}])st.markdown("### Vendor ranking (scorecards))try:
        conn = get_db()
        ...
        st.caption(f"[Vendor ranking note: {_e_vs}])# Win Probability tab body (last-1)
try:
    with legacy_tabs[-1]:
        st.subheader("Win Probability)conn = get_db()
        df_opp = pd.read_sql_query("select * from opportunities order by posted desc", conn)
        df_pp = get_past_performance_df()
        if df_opp.empty:
            st.info("No opportunities in pipeline)else:
            rows = []
    ...
    st.caption(f"[Win Probability tab init note: {_e_win}])# === End injected ===

with legacy_tabs[0]:
    st.subheader("Opportunities pipeline)conn = get_db()
    df_opp = pd.read_sql_query("select * from opportunities order by posted desc", conn)
    ...
    edit = st.data_editor(
        ...
        use_container_width=True, num_rows="dynamic", key="opp_grid)if st.button("Save pipeline changes):
        cur = conn.cursor()
        ...
        conn.commit()
        __ctx_pipeline = True
        st.success(f"Saved — updated {updated} row(s), deleted {deleted} row(s).)# Analytics mini-dashboard (scoped to Pipeline tab)
with legacy_tabs[0]:
    ...
    try:
        ...
        if not df_all.empty:
            st.markdown("### Pipeline analytics)st.bar_chart(df_all.set_index("status))
        ...
        try:
            ...
            st.caption(f"[Win score analytics note: {_e_wa}])except Exception as _e_dash:
        st.caption(f"[Analytics dash note: {_e_dash}])with legacy_tabs[0]:

    if globals().get("__ctx_pipeline", False):


        st.markdown("### Tasks for selected opportunity)try:
            ...
            if sel_id:
                ...
                grid_tasks = st.data_editor(df_tasks, use_container_width=True, num_rows="dynamic", key="tasks_grid)if st.button("Save tasks):
                    ...
                    st.success("Tasks saved.)except Exception as _e_tasks:

            st.caption(f"[Tasks panel note: {_e_tasks}])with legacy_tabs[1]:
    st.subheader("Find subcontractors and rank by fit)trade = st.text_input("Trade", value=get_setting("default_trade", "Janitorial))
    ...
    with colA:
        ...
        if st.button("Google Places import):
            ...
            if places_diag:
                st.write("Places diagnostics:", info); st.code((info or {}).get("raw_preview",")or "", language="json)results = st.session_state.get("vendor_results)or []
        ...
        if results:
            ...
            name_filter = st.text_input("Filter by company name contains", ")if name_filter:
            ...
            edited = st.data_editor(
                ...
                use_container_width=True,
                num_rows="fixed",
                key="vendor_import_grid)# Save only selected rows
            ...
            with col_save_a:
                st.caption(f"Selected to save: {len(save_sel)} of {len(edited) if isinstance(edited, pd.DataFrame) else 0})save_btn = st.button("Save selected to vendors)with col_save_b:
                st.caption("Tip: Click a link to review a site before saving.)if save_btn and not save_sel.empty:
                ...
                for _, r in save_sel.rename(columns={
                    ...
                    saved += 1
                conn.commit()
                st.success(f"Saved {saved} vendor(s).)else:
            msg = "No results"
    ...
    with colB:
        st.markdown("LinkedIn quick search)st.link_button("Open LinkedIn", f"https://www.linkedin.com/search/results/companies/?keywords={quote_plus(trade + ' ' + loc)})with colC:
        st.markdown("Google search)st.link_button("Open Google", f"https://www.google.com/search?q={quote_plus(trade + ' ' + loc)})st.divider()
    _render_saved_vendors_manager()  # show manager only inside Subcontractor Finder
with legacy_tabs[2]:



    st.subheader("POC and networking hub)st.caption("Add or clean up government POCs and vendor contacts. Link key contacts to opportunities in your notes.)conn = get_db()
    df_c = pd.read_sql_query("select * from contacts order by created_at desc", conn)
    grid = st.data_editor(df_c, use_container_width=True, num_rows="dynamic", key="contacts_grid)if st.button("Save contacts):
        cur = conn.cursor()
        ...
        conn.commit(); st.success("Saved)with legacy_tabs[3]:
    st.subheader("Outreach and mail merge)st.caption("Use default templates, personalize for distance, capability and past performance. Paste replies to track status.)# Render Outreach tools here (moved from sidebar)
    ...
    pick_t = st.selectbox("Template", options=names, key="tpl_pick_name)tpl = pd.read_sql_query("select subject, body from email_templates where name=?", get_db(), params=(pick_t,))
    subj_default = tpl.iloc[0]["subject"] if not tpl.empty else get_setting("outreach_subject", ")body_default = tpl.iloc[0]["body"] if not tpl.empty else get_setting("outreach_scope", ")subj = st.text_input("Subject", value=subj_default, key="tpl_subject)body = st.text_area("Body with placeholders {company} {scope} {due}", value=body_default, height=220, key="tpl_body)colA, colB, colC, colD = st.columns([1,1,1,2])
    ...
    with colA:
        if st.button("Update selected", key="tpl_btn_update):
            ...
            _conn.commit()
            st.success(f"Updated '{pick_t}')st.rerun()

    with colB:
        new_name = st.text_input("New name", value="", placeholder="e.g., RFQ Follow-up", key="tpl_new_name)if st.button("Save as new", key="tpl_btn_save_new)and new_name.strip():
            _conn = get_db()
            _conn.execute(
                """
                INSERT INTO email_templates(name, subject, body)
            ...
            _conn.commit()
            st.success(f"Saved as '{new_name.strip()}')st.rerun()

    with colC:
        confirm_del = st.checkbox("Confirm delete", key="tpl_confirm_delete)if st.button("Delete selected", key="tpl_btn_delete", help="Requires confirm)and confirm_del:
            _conn = get_db()
            _conn.execute("DELETE FROM email_templates WHERE name=?", (pick_t,))
            _conn.commit()
            st.warning(f"Deleted '{pick_t}')st.rerun()

    with colD:
        st.caption("Tips: Use placeholders like {company}, {scope}, {due}.)picks = st.multiselect("Choose vendors to email", options=df_v["company"].tolist(), default=df_v["company"].tolist()[:10])
    ...
    if st.button("Generate emails):
        ...
        for name in picks:
            ...
            to_addr = row.get("email",")body_filled = body.format(company=name, scope=scope_hint, due=due)
            st.session_state["mail_bodies"].append({"to": to_addr, "subject": subj, "body": body_filled, "vendor_id": int(row["id"])})
        st.success(f"Prepared {len(st.session_state['mail_bodies'])} emails)# SMTP email sender helpers
        ...
        def _send_via_gmail(to_addr, subject, body):
            ...
            smtp_user = st.secrets.get("smtp_user)smtp_pass = st.secrets.get("smtp_pass)if not smtp_user or not smtp_pass:
                raise RuntimeError("Missing smtp_user/smtp_pass in Streamlit secrets)from_addr = st.secrets.get("smtp_from", smtp_user)
...
def score_opportunity(row, keywords=None, watched_naics=None):
    try:
        ...
        # Days until due
        due = row.get("response_due)days_to_due = None
...
def _ensure_opportunity_columns():
    ...
    try: cur.execute("alter table opportunities add column status text default 'New')except Exception: pass
    try: cur.execute("alter table opportunities add column assignee text)except Exception: pass
    try: cur.execute("alter table opportunities add column quick_note text)except Exception: pass
    conn.commit()
...
def _get_table_cols(name):
    conn = get_db(); cur = conn.cursor()
    cur.execute(f"pragma table_info({name}))return [r[1] for r in cur.fetchall()]
...
def _to_sqlite_value(v):
    ...
    try:
        ...
        if isinstance(v, (bytes, bytearray)):
            try:
                return v.decode("utf-8", "ignore)except Exception:
                return str(v)
...
def save_opportunities(df, default_assignee=None):
    ...
    for _, r in df.iterrows():
        nid = r.get("sam_notice_id)if not nid:
            continue
        ...
        if row:
            ...
            if "status" in cols:
                insert_cols.append("status); insert_vals.append("New)if "assignee" in cols:
                insert_cols.append("assignee); insert_vals.append(_to_sqlite_value(default_assignee or "))
            if "quick_note" in cols:
                insert_cols.append("quick_note); insert_vals.append(")placeholders = ",".join("?" for _ in insert_cols)
...
def sam_live_monitor(run_now: bool = False, hours_interval: int = 3, email_digest: bool = False, min_score_digest: int = 70):
    ...
    try:
        ...
        last_run = get_setting(key_last, ")now_utc = pd.Timestamp.utcnow()
        do_run = run_now
        if not do_run:
            if last_run:
                try:
        ...
        try:
            _raw = get_setting(_defaults_key, ")_saved = json.loads(_raw) if _raw else {}
        except Exception:
            _saved = {}
        ...
        if email_digest and isinstance(df, pd.DataFrame) and not df.empty:
            ...
            if not best.empty and USER_EMAILS.get(ACTIVE_USER, "):
                ...
                for _, r in best.iterrows():
                    lines.append(f"• [{int(r['Score'])}] {str(r.get('title',''))[:90]} — {str(r.get('agency',''))[:40]} (due {str(r.get('response_due',''))[:16]})<br>{str(r.get('url',''))})try:
    ...
    def build_proposal_md_from_row(row: dict) -> str:
        ...
        naics = str(row.get("naics","))
        url = str(row.get("url","))
        company = get_setting("company_name", "ELA Management LLC)uei = get_setting("uei", ")cage = get_setting("cage", ")phone = get_setting("company_phone", ")email = USER_EMAILS.get(ACTIVE_USER, get_setting("company_email","))
...
with legacy_tabs[4]:
    st.subheader("SAM.gov auto search with attachments)st.markdown("> **Flow:** Set All active → apply filters → open attachments → choose assignee → **Search** then **Save to pipeline**)conn = get_db()
    codes = pd.read_sql_query("select code from naics_watch order by code", conn)["code"].tolist()
    st.caption(f"Using NAICS codes: {', '.join(codes) if codes else 'none'})auto_on = st.checkbox("Enable auto-monitor", value=bool(get_setting(f"sam_auto_{ACTIVE_USER}", "true)!= "false))
    ...
    if st.button("Save monitor settings):
        set_setting(f"sam_auto_{ACTIVE_USER}", "true" if auto_on else "false)set_setting(f"sam_interval_{ACTIVE_USER}", str(int(interval_hours)))
        set_setting(f"sam_digest_{ACTIVE_USER}", "true" if digest else "false)set_setting(f"sam_digestmin_{ACTIVE_USER}", str(int(digest_min)))
        st.success("Saved monitor settings)# Kick the monitor if interval elapsed
    try:
        if auto_on:
            _res = sam_live_monitor(False, int(interval_hours), digest, int(digest_min))
            if _res and _res.get("ok)and not _res.get("skipped):
                st.info(f"Auto-monitor: inserted {_res.get('inserted',0)}, updated {_res.get('updated',0)})except Exception as _e_mon:
        st.caption(f"[Monitor note: {_e_mon}])# --- Per-user default filters
_defaults_key = f"sam_default_filters_{ACTIVE_USER}"
try:
    _raw = get_setting(_defaults_key, ")_saved_defaults = json.loads(_raw) if _raw else {}
except Exception:
    _saved_defaults = {}
    ...
    with col3:
        ...
        st.markdown("**Defaults**)if st.button("Save as my default):
            ...
            st.success("Saved your defaults)if st.button("Reset my default):
            set_setting(_defaults_key, ")st.info("Cleared your saved defaults)cA, cB, cC = st.columns(3)
    ...
    with cA:
        ...
        if st.button("Run search now):
            df, info = sam_search(
                ...
                notice_types="Combined Synopsis/Solicitation,Solicitation", active="true)st.session_state["sam_results_df"] = df
                ...
                if email_top and isinstance(df, pd.DataFrame) and not df.empty and email_to_self:
                    ...
                    if not best.empty:
                        ...
                        for _, r in best.iterrows():
                            ...
                            st.info(f"Emailed {len(best)} matches to {email_to_self})except Exception as _e_mail:
                            st.caption(f"[Email note: {_e_mail}])except Exception as _e_email:
                st.caption(f"[Email block note: {_e_email}])if diag:
                st.write("Diagnostics:", info)
                st.code(f"naics={','.join(codes[:20])} | keyword={keyword or ''} | postedFrom={info.get('filters',{}).get('postedFrom')} -> postedTo={info.get('filters',{}).get('postedTo')} | min_days={min_days} | limit=150", language="text)# Optional: email top results once computed below after scoring
            if raw:
                st.code((info or {}).get("raw_preview",")or "", language="json)# Show results from session (if any)
    df = st.session_state.get("sam_results_df)info = st.session_state.get("sam_results_info", {}) or {}
    if info and not info.get("ok", True):
        st.error(f"SAM API error: {info})elif isinstance(df, pd.DataFrame) and not df.empty:
        ...
        if "response_due" in grid_df.columns:
            try:
                _dt = pd.to_datetime(grid_df["response_due"], errors="coerce)grid_df = grid_df.assign(_due=_dt).sort_values(["Score","_due"], ascending=[False, True]).drop(columns=["_due"])
            except Exception:
        ...
        with st.expander("Quick select options):
            ...
            if st.button("Mark top N for Save):
                try:
                    ...
                    st.success(f"Selected {int(n_top)} rows)except Exception as e:
                    st.warning(f"Could not select top rows: {e})edited = st.data_editor(
            grid_df,
            column_config={
                "Link": st.column_config.LinkColumn("Link", display_text="Open in SAM)},
            use_container_width=True,
            num_rows="fixed",
            key="sam_watch_grid)# Save only selected rows
        save_sel = edited[edited.get("Save", False)==True] if "Save" in edited.columns else edited.iloc[0:0]
        st.caption(f"Selected to save: {len(save_sel)} of {len(edited)})if st.button("Save selected to pipeline):
            ...
            st.success(f"Saved to pipeline — inserted {ins}, updated {upd}.)# Proposal drafts for selected
            if len(save_sel) > 0:
                st.markdown("#### Auto Proposal Prep)for _i, _r in save_sel.iterrows():
                    _md = build_proposal_md_from_row(_r)
                    _bytes = md_to_docx_bytes_rich(_md, title=str(_r.get('title','')))
                    st.download_button("Prep Proposal DOCX: " + str(_r.get('sam_notice_id','') or _r.get('title',''))[:40], data=_bytes, file_name=f"proposal_{str(_r.get('sam_notice_id','') or _i)}.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document)try:
                        cur = get_db().cursor()
                        cur.execute("insert into sam_history(ts_utc,user,action,sam_notice_id,title,agency,naics,response_due,score) values(?,?,?,?,?,?,?,?,?), 
            ...
            if len(save_sel) > 0:
                try:
                    st.markdown("#### Prep outreach drafts for CO (email placeholders))if st.button("Create outreach drafts in Outreach tab):
                        bods = []
                        ...
                        st.success("Drafts prepared — open the Outreach tab to review and send.)except Exception as _e_prep:
                    st.caption(f"[CO outreach prep note: {_e_prep}])else:
        st.info("No active results yet. Click **Run search now**.)with cB:
        if st.button("Broad test (keyword only)):
            ...
            df, info = sam_search(
                ...
                notice_types="Combined Synopsis/Solicitation,Solicitation", active="true)st.session_state["sam_results_df"] = df
            st.session_state["sam_results_info"] = info
            st.success(f"Test search complete for keyword: {kw})with cC:
        if st.button("Test SAM key only):
            try:
                ...
                st.error(f"Key test failed: {e})# Removed RFP mini-analyzer from SAM Watch
    ...
    with st.expander("SAM Analytics):
        ...
        try:
            ...
            if not hist.empty and "ts_utc" in hist.columns:
                ...
                agg = _h.groupby(["month","action"]).size().reset_index(name="n)st.write("Activity by month)st.dataframe(agg.sort_values(["month","action"]))
        except Exception as _e_ana:
            st.caption(f"[Analytics note: {_e_ana}])with legacy_tabs[6]:
    st.subheader("Capability statement builder)company = get_setting("company_name", "ELA Management LLC)tagline = st.text_input("Tagline", key="cap_tagline_input_capability_builder", value="Responsive project management for federal facilities and services)core = st.text_area("Core competencies", key="cap_core_textarea_capability_builder", value="Janitorial Landscaping Staffing Logistics Construction Support IT Charter buses Lodging Security Education Training Disaster relief)diff = st.text_area("Differentiators", key="cap_diff_textarea_capability_builder", value="Fast mobilization • Quality controls • Transparent reporting • Nationwide partner network)past_perf = st.text_area("Representative experience", key="cap_past_textarea_capability_builder", value="Project A: Custodial support, 100k sq ft. Project B: Grounds keeping, 200 acres.)contact = st.text_area("Contact info", key="cap_contact_textarea_capability_builder", value="ELA Management LLC • info@elamanagement.com • 555 555 5555 • UEI XXXXXXX • CAGE XXXXX)c1, c2, c3 = st.columns([1,1,2])
...
Certifications Small Business"""
    ...
    cap_md = st.session_state.get("capability_md", ")cap_md = _normalize_markdown_sections(cap_md)
    if cap_md:
        st.markdown("#### Preview)st.markdown(cap_md)
        issues, est_pages = _validate_text_for_guardrails(cap_md, page_limit=2, require_font="Times New Roman", require_size_pt=11, margins_in=1.0, line_spacing=1.0, filename_pattern="{company}_{section}_{date})if issues:
            st.warning("Before export, fix these items: " + "; ".join(issues))
        logo_file = st.file_uploader("", type=["png", "jpg", "jpeg"], key="cap_logo_upload", label_visibility="collapsed)_logo = logo_file.read() if logo_file else None
        ...
        st.download_button("Export Capability Statement (DOCX),  data=docx_bytes, file_name="Capability_Statement.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document)else:
        st.info("Click Generate one page to draft, then export to DOCX.)with legacy_tabs[7]:
    st.subheader("White paper builder)title = st.text_input("Title", key="wp_title_input_whitepaper_builder", value="Improving Facility Readiness with Outcome based Service Contracts)thesis = st.text_area("Thesis", key="wp_thesis_textarea_whitepaper_builder", value="Outcome based service contracts reduce total cost and improve satisfaction when paired with clear SLAs and transparent data.)audience = st.text_input("Audience", key="wp_audience_input_whitepaper_builder", value="Facility Managers • Contracting Officers • Program Managers)col_w1, col_w2, col_w3 = st.columns([1,1,2])
    with col_w1:
    ...
    wp_md = st.session_state.get("whitepaper_md", ")wp_md = _normalize_markdown_sections(wp_md)
    if wp_md:
        st.markdown("#### Preview)st.markdown(wp_md)
        issues, est_pages = _validate_text_for_guardrails(wp_md, page_limit=4, require_font="Times New Roman", require_size_pt=11, margins_in=1.0, line_spacing=1.0, filename_pattern="{company}_{section}_{date})if issues:
            st.warning("Before export, fix these items: " + "; ".join(issues))
        wp_logo_file = st.file_uploader("", type=["png", "jpg", "jpeg"], key="wp_logo_upload", label_visibility="collapsed)_wp_logo = wp_logo_file.read() if wp_logo_file else None
        ...
        st.download_button("Export White Paper (DOCX),  data=wp_bytes, file_name="White_Paper.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document)else:
        st.info("Click Draft white paper to create a draft, then export to DOCX.)with legacy_tabs[8]:
    st.subheader("Export to Excel workbook)conn = get_db()
    ...
    st.download_button("Download Excel workbook", data=bytes_xlsx, file_name="govcon_hub.xlsx",
                       mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet)with legacy_tabs[9]:
    st.subheader("Auto extract key details)up = st.file_uploader("", type=["pdf", "docx", "doc", "txt"], accept_multiple_files=True, key="auto_up", label_visibility="collapsed)if up and st.button("Extract):
...
with legacy_tabs[10]:
    st.subheader("Ask questions over the uploaded docs)up2 = st.file_uploader("", type=["pdf", "docx", "doc", "txt"], accept_multiple_files=True, key="qna_up", label_visibility="collapsed)q = st.text_input("Your question)if up2 and q and st.button("Answer):
...
with legacy_tabs[11]:
    st.subheader("Chat Assistant (remembers context; accepts file uploads))conn = get_db()
    ...
    if pick == "➤ New chat":
        ...
        if st.button("Start chat):
            ...
            conn.commit()
            st.rerun()
        st.caption("Pick an existing chat from the dropdown above to continue.)else:
        # Parse session id
        session_id = parse_pick_id(pick)
        if session_id is None:
            st.info("Select a valid session to continue.)else:
            cur_title = sessions[sessions["id"] == session_id]["title"].iloc[0] if not sessions.empty else "(untitled)"
            st.caption(f"Session #{session_id} — {cur_title})# File uploads for this chat session
            up_files = st.file_uploader("", label_visibility="collapsed),  type=["pdf","docx","doc","txt"],
                                        accept_multiple_files=True, key=f"chat_up_{session_id})if up_files and st.button("Add files to this chat):
                added = 0
                for up in up_files:
                    try:
                    ...
                    added += 1
                conn.commit()
                st.success(f"Added {added} file(s).)st.rerun()
            ...
            if not files_df.empty:
                st.caption("Attached files)st.dataframe(files_df.rename(columns={"chars":"chars_of_text"}), use_container_width=True)
            ...
            user_msg = st.chat_input("Type your message)if user_msg:
                # Save user's message
                ...
                msgs_db = pd.read_sql_query(
                    ...
                    conn, params=(session_id,)
                ).to_dict(orient="records)# Keep last ~12 user/assistant turns
...
with legacy_tabs[__tabs_base + 0]:
    st.subheader("Deadline tracker)conn = get_db()
    colA, colB = st.columns(2)
    with colA:
        st.caption("From opportunities table)o = pd.read_sql_query("select id, title, agency, response_due, status from opportunities order by response_due asc nulls last", conn)
        if not o.empty:
            ...
            st.info("No opportunities yet)with colB:
        st.caption("Manual deadlines)m = pd.read_sql_query("select * from deadlines order by due_date asc", conn)
        st.dataframe(m)
        with st.form("add_deadline):
            title = st.text_input("Title)due = st.date_input("Due date", datetime.now().date())
            source = st.text_input("Source or link", ")notes = st.text_area("Notes", ")if st.form_submit_button("Add):
                ...
                conn.commit()
                st.success("Added)st.markdown("### Due today)due_today = pd.read_sql_query("select * from deadlines where date(due_date)=date('now') and status='Open'", conn)
    if not due_today.empty:
        st.dataframe(due_today[["title","due_date","source","notes"]])
        # Email reminders via Microsoft Graph
        st.markdown("#### Send email reminders)to_addr = st.text_input("Send reminders to email", value=")if st.button("Email reminders for items due today):
            if to_addr:
                body_lines = ["The following items are due today:"]
                for _, r in due_today.iterrows():
                    body_lines.append(f"- {r['title']} (source: {r.get('source','')}))status = send_via_graph(to_addr, "Reminders: items due today", "\n".join(body_lines))
                st.info(f"Email status: {status})else:
                st.info("Enter an email address to send reminders.)else:
        st.write("No items due today.)with legacy_tabs[__tabs_base + 1]:
    st.subheader("Section L and M checklist)conn = get_db()
    ...
    opp_sel = st.selectbox("Link checklist to opportunity", options=opp_opt, index=0, key="lm_opp_sel)opp_id_val = int(opp_sel.split(":)[0]) if opp_sel else None

    up = st.file_uploader("", type=["pdf", "docx", "doc", "txt"], accept_multiple_files=True, key="lm_up", label_visibility="collapsed)if up and st.button("Generate checklist):
        items = []
        for f in up:
            name = f.name
            ...
            def _snip(text, pat):
                try:
                    ...
                    m = rx.search(text or ")if not m: return ""
        ...
        conn.commit()
        st.success("Checklist saved with page anchors, owners and snippets)st.markdown("#### Existing items)items = pd.read_sql_query("select * from compliance_items order by created_at desc limit 200", conn)
    st.dataframe(items, use_container_width=True)

with legacy_tabs[__tabs_base + 2]:
    pass
    st.subheader("RFQ generator to subcontractors)conn = get_db()
    vendors = pd.read_sql_query("select id, company, email, phone, trades from vendors order by company", conn)
    st.caption("Compose RFQ)with st.form("rfq_form):
        ...
        qty = st.text_input("Quantities or CLIN list", ")due = st.date_input("Quote due by", datetime.now().date() + timedelta(days=3))
        files = st.text_input("File names to reference", ")subject = st.text_input("Email subject", "Quote request for upcoming federal project)body = st.text_area("Email body preview", height=240,
            value=(f"Hello, \n\nELA Management LLC requests a quote.\n\nScope\n{scope}\n\nQuantities\n{qty}\n\nDue by {due.strftime('%Y-%m-%d')}\n\nFiles\n{files}\n\nPlease reply with price lead time and any exclusions.\n\nThank you.))
        submit = st.form_submit_button("Generate drafts)if submit:
        ...
        conn.commit()
        st.success(f"Created {len(recs)} RFQ draft(s))st.markdown("#### Drafts)drafts = pd.read_sql_query("select * from rfq_outbox order by created_at desc", conn)
    st.dataframe(drafts)
    ...
    if pick:
        ...
        if row:
            ...
            doc.add_paragraph(f"To: {row[0]})for para in row[2].split("\n\n):
            ...
            st.download_button("Download RFQ.docx", data=bio.getvalue(), file_name="RFQ.docx",
                               mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document)with legacy_tabs[__tabs_base + 3]:
    st.subheader("Pricing calculator)with st.form("price_calc):
        ...
        terms_days = st.number_input("Payment terms (days),  min_value=0, step=1, value=30, help="Net terms for government payment)advance_pct = st.number_input("Factoring advance (%),  min_value=0.0, max_value=100.0, step=1.0, value=85.0)
        fac_rate = st.number_input("Factoring fee per 30 days (%),  min_value=0.0, max_value=10.0, step=0.1, value=2.0)
        run = st.form_submit_button("Calculate)if run:
        total = base_cost * (1 + overhead/100.0) * (1 + gna/100.0) * (1 + profit/100.0)
        note = _lpta_note(total, budget_hint=igce if igce > 0 else None)
        st.metric("Total price", f"${total:,.2f})st.info(f"LPTA note: {note})# Cash flow factoring model (simple)
        ...
        try:
            ...
            try: cur.execute("alter table pricing_scenarios add column terms_days integer)except Exception: pass
            try: cur.execute("alter table pricing_scenarios add column factoring_rate real)except Exception: pass
            try: cur.execute("alter table pricing_scenarios add column advance_pct real)except Exception: pass
            ...
            st.caption(f"[Pricing save note: {_e_pc}])st.markdown("### Scenario comparison)conn = get_db()
    try:
        ...
        if not dfp.empty:
            ...
            st.caption("No scenarios yet.)except Exception as _e_cmp:
        st.caption(f"[Scenario table note: {_e_cmp}])with st.expander("Market data assist", expanded=True):
        colm1, colm2 = st.columns(2)
        with colm1:
            naics_q = st.text_input("NAICS for history lookup", value="", key="md_naics)psc_q = st.text_input("PSC for history lookup", value="", key="md_psc)kw_q = st.text_input("Optional keyword", value="", key="md_kw)with colm2:
            lookback_months = st.number_input("Look back months", min_value=1, step=1, value=24, key="md_months)limit_rows = st.number_input("Max awards to pull", min_value=10, step=10, value=200, key="md_limit)want_calc = st.checkbox("Also pull GSA CALC labor rates", value=False, key="md_calc)if st.button("Fetch market data", key="md_fetch):
            from datetime import datetime as _dt, timedelta as _td
            date_to = _dt.utcnow().date().strftime("%Y-%m-%d)date_from = (_dt.utcnow().date() - _td(days=int(lookback_months)*30)).strftime("%Y-%m-%d)debug_box = st.container()
            ...
            if not df_awards.empty and "error" not in df_awards.columns:
                st.caption(f"USAspending awards from {date_from} to {date_to})st.dataframe(df_awards.head(50), use_container_width=True)
                ...
                if "start" in df_awards.columns and "end" in df_awards.columns and "amount" in df_awards.columns:
                    ...
                    st.markdown("#### Diagnostics: term and monthly spend)# Save selected awards as benchmarks with your annotations
                    st.markdown("#### Save selected awards to your benchmark library)try:
                        _choices = _df["award_id"].dropna().astype(str).unique().tolist()
                    except Exception:
                        _choices = []
                    _sel_awards = st.multiselect("Pick award IDs to tag", _choices, key="md_pick_awards)with st.form("md_bench_form):
                        _sqft = st.number_input("Facility size sqft", min_value=0, step=1000, value=0, key="md_bench_sqft)_freq = st.number_input("Visits per week", min_value=0, max_value=14, step=1, value=5, key="md_bench_freq)_facility = st.text_input("Facility type", value="", key="md_bench_facility)_scope = st.text_input("Scope tags comma separated", value="daily, restrooms, trash, floors", key="md_bench_scope)_cpi = st.number_input("Inflation adjust percent per year", min_value=0.0, max_value=20.0, value=3.0, step=0.5, key="md_bench_cpi)_note = st.text_area("Notes", value="", key="md_bench_notes)_save = st.form_submit_button("Save to benchmarks)if _save and _sel_awards:
                        import pandas as _pd, math as _math
                        from datetime import datetime as _dtd
                        _rows = _df[_df["award_id"].astype(str).isin(_sel_awards)].to_dict("records)for r in _rows:
                            ...
                            try:
                                ...
                                st.warning(f"Save failed for {r.get('award_id')}: {_e})st.success(f"Saved {len(_sel_awards)} benchmark rows)# View and use your benchmarks
                    with st.expander("Your benchmark library", expanded=False):
                        ...
                        if _bench is None or _bench.empty:
                            st.caption("No benchmarks yet. Save from the table above.)else:
                            ...
                            try:
                                ...
                                _med_month = None
                            if _med_sqft:
                                st.markdown(f"**Median dollars per sqft per year across benchmarks: ${_med_sqft:,.2f}**)if _med_month:
                                st.markdown(f"**Median monthly spend across benchmarks: ${_med_month:,.0f}**)_apply_sqft = st.number_input("Use sqft to apply median $ per sqft", min_value=0, step=1000, value=0, key="md_apply_sqft)if _apply_sqft and _apply_sqft > 0 and _med_sqft:
                                ...
                                if st.button("Set base cost from benchmark median", key="md_bench_setbase):
                                    ...
                                    st.success(f"Base cost set to ${_hint:,.2f} from benchmark median. Recalculate above.)st.dataframe(_df[["award_id","recipient","agency","start","end","amount","term_months","monthly_spend"]].head(50), use_container_width=True)

                    with st.expander("Implied $/sqft/year calculator", expanded=False):
                        sqft = st.number_input("Approx facility size (sqft),  min_value=0, step=1000, value=0, key="md_sqft)per_week = st.number_input("Service frequency (visits per week),  min_value=0, max_value=14, step=1, value=5, key="md_freq)if sqft and sqft > 0:
                            _df2 = _df.copy()
                            ...
                            st.caption("Based on your sqft input, here are implied $/sqft/year figures:)st.dataframe(_df2[["award_id","agency","annualized_amount","dollars_per_sqft_year"]].head(50), use_container_width=True)

                            _vals = _pd.to_numeric(_df2["dollars_per_sqft_year"], errors="coerce).dropna()
                            if not _vals.empty:
                                _med = float(_vals.median())
                                st.markdown(f"**Median implied $/sqft/year across results: ${_med:,.2f}**)if st.button("Set pricing hint from $/sqft median", key="md_set_sqft):
                                    st.session_state["pricing_base_cost"] = _med * float(sqft)
                                    st.success(f"Base cost set to ${st.session_state['pricing_base_cost']:,.2f} from implied $/sqft median. Recalculate above.)if want_calc:
                    df_rates = gsa_calc_rates(kw_q or naics_q or psc_q or "janitorial)if df_rates is not None and not df_rates.empty:
                        st.caption("GSA CALC sample labor rates)st.dataframe(df_rates.head(50), use_container_width=True)
                        ...
                        if not rate_series.empty:
                            with st.expander("Crew cost estimate (GSA CALC),  expanded=False):
                                crew_size = st.number_input("Crew size (people),  min_value=1, max_value=50, value=3, step=1, key="md_crew)hrs_per_week = st.number_input("Hours per week (crew),  min_value=1, max_value=168, value=40, step=1, key="md_hours)rate_med = float(_np.median(rate_series))
                                est_monthly = rate_med * float(crew_size) * float(hrs_per_week) * 4.33
                                st.markdown(f"Estimated crew cost at CALC median rate: **${est_monthly:,.0f} / month**)else:
                        st.info("No CALC rates returned. Try a simpler keyword.)else:
                st.info("No award results returned. Try broadening filters or increasing look back.)st.caption(diag)

# ---------- Dates (US format for SAM) ----------
def _us_date(d: datetime.date) -> str:
    return d.strftime("%m/%d/%Y)def _parse_sam_date(s: str):
...
def _extract_emails(text: str) -> set:
    ...
    for m in EMAIL_REGEX.finditer(text or "):
        e = m.group(0).strip().strip(".,;:))if not e.lower().endswith((".png",".jpg",".gif",".svg",".jpeg)):
            emails.add(e)
    return emails
...
def crawl_site_for_emails(seed_url: str, max_pages=5, delay_s=0.7, same_domain_only=True) -> dict:
    ...
    while queue and visited < max_pages:
        ...
        visited += 1
        try:
            soup = BeautifulSoup(html, "html.parser)for a in soup.find_all("a", href=True):
                href = a["href"].strip()
...
# ---------- UI ----------
with st.sidebar:
    st.subheader("Configuration)company_name = st.text_input("Company name", value=get_setting("company_name", "ELA Management LLC))
    ...
    if st.button("Save configuration):
        ...
        st.success("Saved)st.subheader("API Key Status)def _ok(v): return "✔" if v else "✘"
    st.markdown(f"**OpenAI Key:** {_ok(bool(OPENAI_API_KEY))})st.markdown(f"**Google Places Key:** {_ok(bool(GOOGLE_PLACES_KEY))})st.markdown(f"**SAM.gov Key:** {_ok(bool(SAM_API_KEY))})st.caption(f"OpenAI SDK: {_openai_version} • Model: {OPENAI_MODEL})if st.button("Test model):
        st.info(llm("You are a health check.", "Reply READY.", max_tokens=5))
    
    # Company identifiers (ELA Management LLC)
    st.subheader("Company identifiers)st.code("DUNS: 14-483-4790\nCAGE: 14ZP6\nUEI: U32LBVK3DDF7", language=None)
    ...
    if st.button("Test SAM key):
        try:
            ...
            st.error(f"Request failed: {e})if st.button("Test Google Places key):
    ...
    st.subheader("Watch list NAICS)conn = get_db()
    ...
    st.multiselect("Choose or type NAICS codes then Save", options=naics_options,
                   default=saved_codes if saved_codes else sorted(set(NAICS_SEEDS[:20])), key="naics_watch)new_code = st.text_input("Add a single NAICS code)col_n1, col_n2 = st.columns(2)
    with col_n1:
        if st.button("Add code):
            val = (new_code or ").strip()
            if val:
                conn.execute("insert or ignore into naics_watch(code,label) values(?,?),  (val, val)); conn.commit(); st.success(f"Added {val})with col_n2:
        if st.button("Clear all saved codes):
            conn.execute("delete from naics_watch); conn.commit(); st.success("Cleared saved codes)if st.button("Save NAICS list):
        keep = sorted(set([c.strip() for c in st.session_state.naics_watch if str(c).strip()]))
        cur = conn.cursor(); cur.execute("delete from naics_watch)for c in keep: cur.execute("insert into naics_watch(code,label) values(?,?),  (c, c))
        conn.commit(); st.success("Saved NAICS watch list)naics_csv = st.file_uploader("", label_visibility="collapsed),  type=["csv"])
    if naics_csv and st.button("Import NAICS from CSV):
        ...
        if "code" in df_in.columns:
            ...
            conn.commit(); st.success("Imported)else:
            st.info("CSV must have a column named code)st.subheader("Goals)g = pd.read_sql_query("select * from goals limit 1", conn)
    if g.empty:
    ...
    with st.form("goals_form", clear_on_submit=False):
        ...
        if st.form_submit_button("Save goals):
            ...
            conn.commit(); st.success("Goals updated)colq1, colq2 = st.columns(2)
    with colq1:
        if st.button("Log new bid):
            conn.execute("update goals set bids_submitted = bids_submitted + 1 where id=?", (goal_id,)); conn.commit(); st.success("Bid logged)with colq2:
        add_amt = st.number_input("Add award amount", min_value=0.0, step=1000.0, value=0.0, key="award_add_amt)if st.button("Log award):
            if add_amt > 0:
                conn.execute("update goals set revenue_won = revenue_won + ? where id=?", (float(add_amt), goal_id)); conn.commit()
                st.success(f"Award logged for ${add_amt:,.0f})else:
                st.info("Enter a positive amount)g = pd.read_sql_query("select * from goals limit 1", conn); row = g.iloc[0]
    st.metric("Bids target", int(row["bids_target"]))
    st.metric("Bids submitted", int(row["bids_submitted"]))
    st.metric("Revenue target", f"${float(row['revenue_target']):,.0f})st.metric("Revenue won", f"${float(row['revenue_won']):,.0f})def render_rfp_analyzer():
    try:
        st.subheader("RFP Analyzer)st.caption("Upload RFP package and chat with memory. Use quick actions or ask your own questions.)conn = get_db()
        ...
        if not pick:
            st.info("Select a chat session to continue.)pass

        session_id = parse_pick_id(pick)
        if session_id is None:
            st.info("Select a valid session to continue.)pass
        cur_title = sessions[sessions["id"] == session_id]["title"].iloc[0]
        st.caption(f"RFP thread #{session_id}  {cur_title})# File uploader with persistence
        uploads = st.file_uploader("", type=["pdf", "docx", "doc", "txt"], accept_multiple_files=True, key=f"rfp_up_{session_id}", label_visibility="collapsed)if uploads and st.button("Add files to RFP thread):
            added = 0
            for up in uploads:
                text = read_doc(up)[:800_000]
                ...
                added += 1
            conn.commit()
            st.success(f"Added {added} file(s) to this thread.)st.rerun()
        ...
        if files_df.empty:
            st.caption("No files yet.)else:
            st.caption("Attached files)st.dataframe(files_df.rename(columns={"chars":"chars_of_text"}), use_container_width=True)
            del_id = st.number_input("Delete attachment by ID", min_value=0, step=1, value=0, key=f"rfp_del_{session_id})if st.button("Delete selected RFP file):
                if del_id > 0:
                    conn.execute("delete from rfp_files where id=?", (int(del_id),))
                    conn.commit()
                    st.success(f"Deleted file id {del_id}.)st.rerun()
        ...
        if hist.empty:
            st.info("No messages yet. Use the quick actions or ask a question below.)else:
            for _, row in hist.iterrows():
                if row["role"] == "user":
                    st.chat_message("user).markdown(row["content"])
                elif row["role"] == "assistant":
                    ...
                    st.caption(f"System updated at {row['created_at']})# Helper to build doc context
        ...
        user_q = st.chat_input("Ask a question about the RFP or use a quick action above)pending_prompt = qa or user_q
    ...
    {doc_snips if doc_snips else ""}"""
            ...
            msgs_db = pd.read_sql_query(
                ...
                conn, params=(session_id,)
            ).to_dict(orient="records)# Keep up to 12 user turns
            pruned, user_turns = [], 0
            for m in msgs_db[::-1]:
                if m["role"] == "assistant":
                    pruned.append(m)
        ...
        st.warning(f"RFP Analyzer error: {e})def render_proposal_builder():
    try:
        st.subheader("Proposal Builder)st.caption("Draft federal proposal sections using your RFP thread and files. Select past performance. Export to DOCX with guardrails.)conn = get_db()
        sessions = pd.read_sql_query("select id, title, created_at from rfp_sessions order by created_at desc", conn)
        if sessions.empty:
            st.warning("Create an RFP thread in RFP Analyzer first.)return

        opts = [f"{r['id']}: {r['title'] or '(untitled)'}" for _, r in sessions.iterrows()]
        pick = st.selectbox("Select RFP thread", options=opts, index=0, key="pb_session_pick)session_id = parse_pick_id(pick)
        if session_id is None:
            st.info("Select a valid session to continue.)st.markdown("**Attach past performance to include**)df_pp = get_past_performance_df()
        selected_pp_ids = []
        if not df_pp.empty:
            df_pp["pick"] = False
            edited_pp = st.data_editor(df_pp[["id","title","agency","naics","period","value","role","highlights","pick"]], use_container_width=True, num_rows="fixed", key="pp_pick_grid)selected_pp_ids = [int(x) for x in edited_pp[edited_pp["pick"]==True]["id"].tolist()]
        else:
            st.caption("No past performance records yet. Add some in Past Performance tab.)col1, col2, col3 = st.columns(3)
        with col1:
        ...
        with colA:
            regenerate = st.button("Generate selected sections)if regenerate and not any(actions.values()):
            st.warning("Pick at least one section above, then click Generate selected sections.)regenerate = False

        with colB:
            save_all = st.button("Save edited drafts)export_md = st.button("Assemble full proposal (Markdown))export_docx = st.button("Export Proposal DOCX (guardrails))# === Generate selected sections ===
        if regenerate:
            # Diagnostics: show which sections are selected
            try:
                _on = [k for k,v in actions.items() if v]
                st.info(f"Generating sections: {', '.join(_on) if _on else 'none'})except Exception:
                pass
            ...
            if selected_pp_ids:
                ...
                for _, r in df_sel.iterrows():
                    lines.append(f"- {r['title']} — {r['agency']} ({r['role']}); NAICS {r['naics']}; Period {r['period']}; Value ${float(r['value'] or 0):,.0f}. Highlights: {r['highlights']})pp_text = "\n".join(lines)
            ...
            for sec, on in actions.items():
                ...
                user_prompt = section_prompts.get(sec, f"Draft the section titled: {sec}.)out = _gen_with_fallback(system_text, user_prompt)
                ...
                conn.commit()
            try:
                st.success("Generated drafts. Scroll down to 'Drafts' to review and edit.)except Exception:
                pass
            st.rerun()


        # Compliance validation settings
        st.markdown("#### Compliance validation settings)colv1, colv2, colv3 = st.columns(3)
        with colv1:
            pb_page_limit = st.number_input("Page limit (estimated),  min_value=0, step=1, value=0)
            pb_font = st.text_input("Required font", value="Times New Roman)with colv2:
        ...
        with colv3:
            ...
            pb_file_pat = st.text_input("Filename pattern", value="{company}_{solicitation}_{section}_{date})# Assemble full proposal in Markdown
        if export_md:
            parts = []
            for sec in order:
            ...
            st.markdown("#### Assembled Proposal (Markdown preview))st.code(assembled, language="markdown)st.download_button("Download proposal.md", data=assembled.encode("utf-8),
                               file_name="proposal.md", mime="text/markdown)# Export DOCX with guardrails
        if export_docx:
            from docx import Document
            ...
            if issues:
                st.warning("Proceeding with export:)for x in issues:
                    st.markdown(f"- {x})doc = Document()
            ...
            company = get_setting("company_name","ELA Management LLC)today = datetime.now().strftime("%Y%m%d)safe_title = (sessions[sessions["id"] == session_id]["title"].iloc[0] if not sessions.empty else "RFP).replace(" ", "_)fname = (pb_file_pat or "{company}_{solicitation}_{date}).format(
            ...
            st.download_button("Download Proposal DOCX", data=bio.getvalue(), file_name=fname,
                               mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document)st.markdown("### Drafts)order = ["Executive Summary","Technical Approach","Management & Staffing Plan","Past Performance","Pricing Assumptions/Notes","Compliance Narrative"]
        ...
        for sec in order:
            if not actions.get(sec, False):
                continue
            st.markdown(f"**{sec}**)txt = existing.get(sec, {}).get("content", ")edited_blocks[sec] = st.text_area(f"Edit {sec}", value=txt, height=240, key=f"pb_{sec})if save_all and edited_blocks:
            cur = conn.cursor()
            ...
            conn.commit()
            st.success("Drafts saved.)except Exception as e:
        st.error(f"Proposal Builder error: {e})# === End new features ===
...
try:
    ...
    st.caption(f"[RFP Analyzer tab note: {e}])try:
    with legacy_tabs[12]:
        render_proposal_builder()
except Exception as e:
    st.caption(f"[Proposal Builder tab note: {e}])with conn:
    conn.execute("""
...
with legacy_tabs[8]:
    st.subheader("Proposal export and drafts)# === Deals (CRM pipeline) helpers ===
DEAL_STAGES = [
    "No Contact Made",
    "CO Contacted",
    "Quote",
...
def ensure_deals_table(conn):
    ...
    "")cur.execute("create index if not exists deals_stage_idx on deals(stage))cur.execute("create index if not exists deals_updated_idx on deals(updated_at))conn.commit()
...
def list_deals(stage: str | None = None, q: str | None = None):
    ...
    if q:
        where.append("(title like ? or notes like ? or agency like ?))params += [f"%{q}%", f"%{q}%", f"%{q}%"]
    if where:
...
def update_deal(id_: int, **fields):
    ...
    sets.append("updated_at = datetime('now'))sql = "update deals set " + ", ".join(sets) + " where id = ?"
    vals.append(id_)
...
try:
    with legacy_tabs[13]:
        st.subheader("Deals Pipeline)st.caption("Track opportunities by stage, assign owners, record amounts, and manage the pipeline.)# Filters
        c1,c2,c3 = st.columns([1,1,2])
        with c1:
            stage_filter = st.selectbox("Stage", options=["All"] + DEAL_STAGES, index=0, key="deals_stage_filter)with c2:
            q = st.text_input("Search", key="deals_search)with c3:
            st.markdown(" )# Data
        df = list_deals(stage_filter, q)

        # Totals by stage (above grid)
        st.markdown("#### Totals by stage)import pandas as _pd
        ...
        for _i, _stage in enumerate(DEAL_STAGES):
            with _cols[_i]:
                st.metric(_stage, f"${float(_stage_totals.get(_stage, 0.0)):,.2f})# Add a new deal
        st.markdown("#### Add a new deal)with st.form("new_deal_form", clear_on_submit=True):
            nc1,nc2,nc3,nc4 = st.columns([2,1,1,1])
            with nc1:
                new_title = st.text_input("Opportunity title*", placeholder="e.g., USDA Athens Roof Repair RFQ)with nc2:
                new_stage = st.selectbox("Stage*", options=DEAL_STAGES, index=0)
            with nc3:
                new_owner = st.text_input("Owner", placeholder="e.g., Latrice)with nc4:
                new_amount = st.number_input("Amount", min_value=0.0, step=100.0, value=0.0, format="%.2f)nc5,nc6 = st.columns([1,1])
            with nc5:
                new_agency = st.text_input("Agency", placeholder="e.g., USDA ARS)with nc6:
                new_due = st.text_input("Due date (YYYY-MM-DD),  placeholder="2025-11-01)new_notes = st.text_area("Notes", height=80, placeholder="Key details, next actions...)submitted = st.form_submit_button("Create deal)if submitted:
                if not new_title.strip():
                    st.warning("Please enter a title.)else:
                    create_deal(
                        new_title.strip(),
                        new_stage,
        ...
        st.markdown("#### Pipeline editor)if df.empty:
            st.info("No deals yet. Add your first deal above.)else:
            edited = st.data_editor(
            ...
            if changes:
                st.success(f"Saved {len(changes)} change(s).)# Kanban board
        st.divider()
        st.markdown("### Board view)st.caption("Column per stage with quick add, inline edits, and move.)df_board = list_deals(stage=None, q=q)
        ...
        st.markdown(f"**Total pipeline value:** ${grand_total:,.2f})cols = st.columns(len(DEAL_STAGES))
        for i, stage_name in enumerate(DEAL_STAGES):
            with cols[i]:
                st.markdown(f"#### {stage_name})st.caption(f"{int(_counts.get(stage_name, 0))} deals • ${float(_totals.get(stage_name, 0.0)):,.2f})# Quick add in this stage
                with st.container(border=True):
                    _new_title = st.text_input("New deal title", key=f"quick_new_title_{i})qa1, qa2 = st.columns([1,1])
                    with qa1:
                        _new_owner = st.text_input("Owner", key=f"quick_new_owner_{i})with qa2:
                        _new_amount = st.number_input("Amount", min_value=0.0, step=100.0, value=0.0, format="%.2f", key=f"quick_new_amt_{i})if st.button("New in this stage", key=f"quick_new_btn_{i}):
                        if _new_title.strip():
                            ...
                            st.warning("Enter a title first)# Cards for deals in this stage
                stage_rows = df_board[df_board["stage"] == stage_name]
                for _, row in stage_rows.iterrows():
                    with st.container(border=True):
                        st.markdown(f"**{row['title']}**)st.caption(f"Owner: {row['owner'] or 'Unassigned'}  •  Amount: ${float(row['amount'] or 0):,.2f})kc1, kc2 = st.columns([1,1])
                        with kc1:
                            new_owner = st.text_input("Owner", value=row["owner"] or "", key=f"owner_{row['id']})with kc2:
                            new_amt = st.number_input("Amount", value=float(row["amount"] or 0.0), step=100.0, format="%.2f", key=f"amt_{row['id']})km1, km2 = st.columns([2,1])
                        with km1:
                            new_stage = st.selectbox("Move to", options=DEAL_STAGES, index=DEAL_STAGES.index(stage_name), key=f"mv_{row['id']})with km2:
                            if st.button("Save", key=f"save_{row['id']}):
                                ...
                                if changes:
                                    ...
                                    if ok:
                                        ...
                                        st.error("No changes saved)# Danger zone
        with st.expander("Danger zone: delete a deal):
            ...
            if st.button("Delete deal):
                if delete_deal(int(del_id)):
                    st.warning(f"Deleted deal {int(del_id)}.)st.rerun()
                else:
                    st.error("Delete failed or ID not found.)except Exception as _e_deals:
    try:
        st.caption(f"[Deals tab note: {_e_deals}])except Exception:
        pass
...
def mount_compliance_assistant():
    ...
    with st.expander("Proposal Compliance Assistant", expanded=False):
        sow = st.text_area("Paste Solicitation or SOW text", height=200, key="co_sow_text)draft = st.text_area("Paste Proposal Draft text", height=200, key="co_draft_text)if st.button("Run Compliance Check):
            result = compliance_assess(draft, sow)
            st.write("Flesch Kincaid grade:", result["fk_grade"])
            st.write("Missing sections:", result["missing_sections"] or "None)st.write("Mirrored terms to use:", result["mirrored_terms_sample"] or "None)if result["risks"]:
                st.write("Detected risks and mitigations:)for r, m in result["risks"]:
                    st.write(f"- {r}: {m})st.write("Evaluator style scores:", result["scores"])
        if st.button("Suggest Outline from SOW):
            st.code(propose_outline_with_mirrored_terms(sow), language="markdown)if st.button("Clean Placeholders in Draft):
            st.text_area("Cleaned Draft", clean_placeholders(draft), height=200, key="co_cleaned_draft_out)# Attempt to mount automatically if Streamlit is present
try:
