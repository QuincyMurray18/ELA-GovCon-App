...
def ensure_schema():
    ...
    # ensure assignee column
    try:
        cur.execute("alter table opportunities add column assignee text")
    except Exception:
        pass
    # seed default users
    try:
...
def google_places_search(query, location="Houston, TX", radius_m=80000, strict=True):
    """
    Google Places Text Search + Details (phone + website) with optional distance from a 'place of performance' string.
    Returns (list_of_vendors, info). Emails are NOT provided by Places.
    Each vendor includes distance_km if geocoding succeeded.
    """
    if not GOOGLE_PLACES_KEY:
        return [], {"ok": False, "reason": "missing_key", "detail": "GOOGLE_PLACES_API_KEY is empty."}
    try:
        # Geocode the location string to bias search and compute distance later
        loc_lat, loc_lng = None, None
        try:
            geo_url = "https://maps.googleapis.com/maps/api/geocode/json"
            gr = requests.get(geo_url, params={"address": location, "key": GOOGLE_PLACES_KEY}, timeout=20)
            gj = gr.json() if gr.headers.get("Content-Type","").startswith("application/json") else {}
            if gj.get("results"):
                geom = gj["results"][0]["geometry"]["location"]
                loc_lat, loc_lng = geom.get("lat"), geom.get("lng")
        except Exception:
            pass

        # 1) Text Search with location bias if available
        search_url = "https://maps.googleapis.com/maps/api/place/textsearch/json"
        search_params = {"query": f"{query}", "radius": radius_m, "key": GOOGLE_PLACES_KEY}
        if loc_lat is not None and loc_lng is not None:
            search_params["location"] = f"{loc_lat},{loc_lng}"
        ...
        if status_code != 200 or api_status not in ("OK","ZERO_RESULTS"):
            return ([] if strict else results), {
                ...
                "note": "Enable billing + 'Places API' and 'Geocoding API' in Google Cloud."
            }
        ...
        for item in results:
            ...
            dist_km = None
            if loc_lat is not None and loc_lng is not None and lat is not None and lng is not None:
                try:
                    dist_km = round(hav_km(loc_lat, loc_lng, lat, lng), 2)
                except Exception:
                    dist_km = None
            ...
            out.append({
                ...
                "certifications": "",
                "set_asides": "",
                "notes": (item.get("formatted_address","") or "") + (f" | distance_km={dist_km}" if dist_km is not None else ""),
                "source": "GooglePlaces",
                "distance_km": dist_km,
                "lat": lat, "lng": lng
            })
...
tabs = st.tabs([
    "Pipeline","Subcontractor Finder","Contacts","Outreach","SAM Watch","RFP Analyzer","Capability Statement","White Paper Builder","Pricing & Quote Builder","Data Export","Auto extract","Chat Assistant",
])
...
with tabs[0]:
    ...
    conn = get_db()
    # Filter by assignee
    team = ["","Quincy","Charles","Collin"]
    assignee_filter = st.selectbox("Filter by assignee", options=team, index=team.index(get_current_user() or ""))
    query = "select * from opportunities order by posted desc"
    df_opp = pd.read_sql_query(query, conn)
    if assignee_filter:
        df_opp = df_opp[df_opp["assignee"].fillna("") == assignee_filter]

    if df_opp.empty:
        st.info("No opportunities yet. Use SAM Watch to pull new items.")
    else:
        page_size = st.number_input("Rows per page", min_value=10, max_value=500, value=50, step=10, key="opp_page_size")
        page = st.number_input("Page", min_value=1, value=1, step=1, key="opp_page")
        start = (page-1)*page_size
        end = start + page_size
        # Provide an editable assignee column with a select widget in a separate form
        edit = st.data_editor(df_opp.iloc[start:end], use_container_width=True, num_rows="dynamic", key="opp_grid",
                              column_config={"url": st.column_config.LinkColumn("Link", display_text="open")})
        if st.button("Save pipeline changes"):
            cur = conn.cursor()
            for _, r in edit.iterrows():
                cur.execute("""update opportunities set status=?, response_due=?, title=?, agency=?, assignee=? where id=?""",
                            (r["status"], r["response_due"], r["title"], r["agency"], r.get("assignee","") or "", int(r["id"])))
            conn.commit(); st.success("Saved"); audit("save_pipeline", "opportunities", payload=f"rows={len(edit)}")
...
with tabs[1]:
    ...
    with colA:
        if st.button("Google Places import"):
            ...
            if results:
                ...
                name_filter = st.text_input('Filter by name contains', value='')
                if name_filter:
                    df_new = df_new[df_new['company'].astype(str).str.contains(name_filter, case=False, na=False)]
                sort_by = st.selectbox('Sort by', options=['distance_km','company','website','phone'])
                df_new = df_new.sort_values(by=sort_by, na_position='last')
                # drop dupes by company and website
                df_new = df_new.drop_duplicates(subset=['company','website'], keep='first')
                st.session_state["places_df"] = df_new  # persist for Save button
        ...
        if st.button("Save to vendors"):
            ...
            if df_new is None or df_new.empty:
                ...
                for _, r in df_new.iterrows():
                    ...
                    try:
                        get_db().execute("""insert or ignore into vendors(company,naics,trades,phone,email,website,city,state,certifications,set_asides,notes,source)
                                            values(?,?,?,?,?,?,?,?,?,?,?,?)""",
                                         (r.get("company"), ",".join(naics_choice) if naics_choice else "",
                                          trade, phone, email, website,
                                          r.get("city",""), r.get("state",""), "", "", extra_note, r.get("source","GooglePlaces")))
                        get_db().commit(); saved += 1
                    except Exception:
                        skipped += 1
                st.success(f"Saved {saved} vendors (skipped {skipped} duplicates or errors)")
                audit("save_vendors", "vendors", payload=f"saved={saved}, skipped={skipped}")
        ...
        if vendor_csv:
            ...
            if st.button("Append uploaded vendors"):
                ...
                for _, r in df_u.fillna("").iterrows():
                    conn.execute("""insert or ignore into vendors(company,naics,trades,phone,email,website,city,state,certifications,set_asides,notes,source)
                                 values(?,?,?,?,?,?,?,?,?,?,?,?)""",
    ...
    v_page_size = st.number_input("Rows per page", min_value=10, max_value=500, value=50, step=10, key="vendor_page_size")
    v_page = st.number_input("Page", min_value=1, value=1, step=1, key="vendor_page")
    v_start = (v_page-1)*v_page_size
    v_end = v_start + v_page_size
    grid = st.data_editor(df_v.iloc[v_start:v_end], use_container_width=True, num_rows="dynamic", key="vendor_grid")
...
with tabs[4]:
    st.subheader("SAM.gov — Active Opportunities (with attachments)")
    conn = get_db()

    # Current watchlist for context
    watch_codes = pd.read_sql_query("select code from naics_watch order by code", conn)["code"].tolist()
    st.caption(f"NAICS watchlist: {', '.join(watch_codes) if watch_codes else '(none)'}")

    # Controls
    col1, col2, col3 = st.columns(3)
    with col1:
        search_all = st.checkbox("All active opportunities (ignore NAICS watch)", value=True)
    ...
    with col2:
        manual_naics = st.text_input("Filter by NAICS (comma-separated, optional)", value="")
    ...
    notice_types = ",".join(picked_notice)

    # Build NAICS list: use manual if given; else watchlist; else empty (for 'all')
    naics_for_query = []
    ...
    with run_col1:
        if st.button("Search SAM now"):
            df, info = sam_search(naics_for_query, min_days=min_days, limit=200,
            ...
            if not info.get("ok"):
                ...
                # Enhance display: add attachments count, clickable link, and show key dates
                # Optional default assignee
                default_assignee = st.selectbox("Default assignee for saved items", options=["","Quincy","Charles","Collin"], index=(["","Quincy","Charles","Collin"].index(get_current_user()) if get_current_user() in ["Quincy","Charles","Collin"] else 0))
                if "attachments_json" in df.columns:
                    try:
                        df["attachments_count"] = df["attachments_json"].apply(lambda s: len(json.loads(s)) if isinstance(s, str) and s.strip() else 0)
                    except Exception:
                        df["attachments_count"] = 0
                show_cols = ["title","agency","naics","psc","place_of_performance","posted","response_due","type","attachments_count","url"]
                show_cols = [c for c in show_cols if c in df.columns]
                st.dataframe(df[show_cols], use_container_width=True,
                             column_config={"url": st.column_config.LinkColumn("Link", display_text="open")})
                # Save to pipeline
                save_opportunities(df)
                # Update assignee if chosen
                auto_assignee = default_assignee or (get_current_user() if get_current_user() in ["Quincy","Charles","Collin"] else "")
                if auto_assignee:
                    ids = df["sam_notice_id"].tolist()
                    if ids:
                        qmarks = ",".join(["?"]*len(ids))
                ...
                # Attachment viewer
                st.markdown("**View attachments for a specific notice**")
                pick_id = st.text_input("Enter SAM Notice ID from results", value="")
                if pick_id:
                    row = df[df["sam_notice_id"] == pick_id].head(1)
                    if row.empty:
                        st.info("Notice ID not found in this result set.")
                    else:
                        try:
                            atts = json.loads(row.iloc[0]["attachments_json"]) if row.iloc[0]["attachments_json"] else []
                        except Exception:
                            atts = []
                        if atts:
    ...
    st.dataframe(pd.read_sql_query(
        "select sam_notice_id, title, agency, naics, response_due, posted, type, url, attachments_json from opportunities order by posted desc limit 100", conn
    ), use_container_width=True, column_config={"url": st.column_config.LinkColumn("Link", display_text="open")})
...
with tabs[5]:
    ...
    cur_title = sessions[sessions["id"] == rfp_session_id]["title"].iloc[0]
    st.caption(f"RFP Session #{rfp_session_id} — {cur_title}")

    # Draft editor (stored on session)
    ...
    draft = st.text_area("Optional: paste your proposal draft to grade and correct", height=220, value=draft_text, key="rfp_draft")
    ...
    # File attachments persisted per session
    st.markdown("**Attach RFP documents (PDF/DOCX/TXT)**")
    uploads = st.file_uploader("Drop files", type=["pdf","docx","doc","txt"], accept_multiple_files=True, key="rfp_uploads")
    if uploads and st.button("Add files to this RFP"):
        ...
        for up in uploads:
            ...
            conn.execute("""insert into rfp_files(session_id, filename, mimetype, content_text)
                            values(?,?,?,?)""", (rfp_session_id, up.name, getattr(up, "type", ""), text))
            added += 1
    ...
    if files_df.empty:
        ...
        del_id = st.number_input("Delete attachment by ID", min_value=0, step=1, value=0, key="rfp_del_id")
    ...
    # Conversation history for this RFP session
    hist = pd.read_sql_query("select role, content, created_at from rfp_messages where session_id=? order by id asc",
                             conn, params=(rfp_session_id,))
    if hist.empty:
    ...
    if prompt:
        ...
        # Build doc context from RFP files
        ...
        # Draft grading block if present
        grade_block = ""
        draft_row = pd.read_sql_query("select draft_text from rfp_sessions where id=?", conn, params=(rfp_session_id,))
        draft_text = (draft_row.iloc[0]["draft_text"] if not draft_row.empty else "") or ""
        if draft_text:
            grade_block = f"""
...
with tabs[6]:
    ...
    contact = st.text_area("Contact info", value=f"ELA Management LLC • elamgmtllc@gmail.com • 555 555 5555 • UEI {COMPANY_UEI} • CAGE {COMPANY_CAGE} • DUNS {COMPANY_DUNS} • {COMPANY_EMAIL}")
...
Identifiers UEI {COMPANY_UEI} CAGE {COMPANY_CAGE} DUNS {COMPANY_DUNS}
...
with tabs[8]:
    ...
    # Reuse helper from earlier patch if present, else define a minimal version
        ...
        try:
            ...
            if r.status_code != 200:
                try:
                    ...
                    hint = r.text[:300]
                info.update(ok=False, reason=f"http_{r.status_code}", hint=hint)
                return rows, info
            ...
            for it in results:
                ...
                rows.append({
                    ...
                    "start": start,
                    "end": end,
                    "agency": it.get("Awarding Agency Name",""),
                    "naics": it.get("NAICS",""),
                })
            info.update(ok=True, count=len(rows))
            return rows, info
        except Exception as e:
            info.update(ok=False, reason="exception", hint=str(e)[:200])
            return rows, info
    ...
    if comp_rows:
        ...
        if not df_comp.empty:
            ...
            st.metric("Comparable awards", len(df_comp))
            st.metric("Median award $", f"${med_award:,.0f}")
            st.metric("Median avg per year $", f"${med_per_year:,.0f}")
            ...
            if your_price > 0:
                ...
                if bench_total and bench_total > 0:
                    ...
                    st.metric("Bench total for duration", f"${bench_total:,.0f}")
                    st.metric("Your price vs bench", f"{pct:+.1f}% {verdict}")
    ...
    if comp_csv is not None:
        ...
        if "Award Amount" in df_up.columns:
            st.caption("Stats from uploaded awards")
            st.metric("Rows", len(df_up))
            st.metric("Median award $", f"${_pd.to_numeric(df_up['Award Amount'], errors='coerce').median():,.0f}")
    ...
    def estimate_ftes(unit, size, shifts):
        if unit == "Square Feet":
            return max(0.5, (size / 22000.0) * shifts)
        if unit == "Acres":
            return max(0.5, (size / 13.5) * shifts)
        return max(0.1, size * shifts)

    ftes = estimate_ftes(size_unit, size_value, shifts)
    st.caption(f"Estimated FTEs: {ftes:.2f} (heuristic)")

    hours_per_year = days_per_week * 52 * 8
    years_total = (base_years + option_years) or 1
    rows = []
    labor_rate_loaded = wage_rate * (1 + burden_pct/100.0)
