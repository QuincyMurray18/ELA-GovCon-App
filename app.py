...
def run_rfp_analyzer(conn: sqlite3.Connection) -> None:
    ...
    if _rfp_flow in ("Deliverables", "Intake"):
        try:
            _ctx = pd.read_sql_query(
                "SELECT id, title, sam_url FROM rfps ORDER BY id DESC;",
                conn, params=()
            )
        except Exception:
            _ctx = pd.DataFrame()
        ...
        if _ctx is not None and not _ctx.empty:
            ...
            with cA:
                sam_url = st.text_input(
                    "SAM URL (for amendment tracking)",
                    value=sam_default or "",
                    key="p3_sam_url"
                )
            with cB:
                ttl_hours = st.number_input(
                    "Cache TTL (hours)", min_value=1, max_value=168,
                    value=72, step=1, key="p3_ttl"
                )

            if _rfp_flow == "Intake" and sam_url:
                render_amendment_sidebar(conn, int(rid_p3), sam_url, ttl_hours=int(ttl_hours))

            if _rfp_flow == "Deliverables":
                with st.expander(
                    "Requirements Traceability Matrix (RTM)",
                    expanded=_expand_for("Requirements Traceability Matrix (RTM)")
                ):
