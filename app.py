...
with tabs[0]:
    IN_SAM_TAB = True
    if st.button("Search SAM now", key="sam_search_btn"):
# --- SAM Watch safe defaults (injected) ---
        manual_naics = ' ' if 'manual_naics' not in locals() and 'manual_naics' not in globals() else manual_naics
        keyword = '' if 'keyword' not in locals() and 'keyword' not in globals() else keyword
        search_all = False if 'search_all' not in locals() and 'search_all' not in globals() else search_all

        open_browse = (not manual_naics.strip()) and (not keyword.strip()) and search_all
        kword = (keyword or "").strip()
        if open_browse:
            kword = "*"
        # SAM Watch: build query inputs safely
        watch_codes = st.session_state.get('sam_watch_naics', []) or []
        naics_for_query = []
        if isinstance(manual_naics, str) and manual_naics.strip():
            naics_for_query = [c.strip() for c in manual_naics.split(',') if c.strip()]
        elif not search_all and isinstance(watch_codes, list) and watch_codes:
            naics_for_query = [str(x).strip() for x in watch_codes if str(x).strip()]
        # defaults for other controls if not set
        min_days = st.session_state.get('sam_min_days', 30)
        posted_from_days = st.session_state.get('sam_posted_from_days', 60)
        picked_notice = st.session_state.get('sam_notice_types', ['Solicitation','Combined Synopsis/Solicitation','Presolicitation','Sources Sought','Special Notice'])
        ...
        if not info.get("ok"):
            ...
            st.caption(f"Results loaded: {len(df)}")
# --- Quick filters ---
    sam_df_all = st.session_state.get("sam_results_df")
    if isinstance(sam_df_all, pd.DataFrame) and not sam_df_all.empty:
        _df = sam_df_all.copy()
        if "response_due" in _df.columns:
            try:
                _df["_due_dt"] = pd.to_datetime(_df["response_due"], errors="coerce")
            except Exception:
                _df["_due_dt"] = pd.NaT
        else:
            _df["_due_dt"] = pd.NaT
        agencies = sorted([a for a in _df.get("agency", pd.Series(dtype=str)).dropna().unique().tolist() if str(a).strip()])
        states = []
        if "place_of_performance" in _df.columns:
            def _state_heur(x):
                try:
                    ...
                    return None
            states = sorted(list({st for st in _df["place_of_performance"].map(_state_heur).dropna().tolist()}))
        with st.container():
            cA, cB, cC, cD = st.columns([2,2,2,2])
            with cA:
                pick_agencies = st.multiselect("Agency", agencies, default=[], key="sam_filter_agency")
            with cB:
                pick_states = st.multiselect("State (POP)", states, default=[], key="sam_filter_state")
            with cC:
                due_from = st.date_input("Due from", value=None, key="sam_filter_due_from")
            with cD:
                due_to = st.date_input("Due to", value=None, key="sam_filter_due_to")
        _mask = pd.Series([True] * len(_df))
        if pick_agencies:
            _mask &= _df["agency"].isin(pick_agencies)
        if pick_states:
            def _state_only(x):
                try:
                    t = str(x).strip().split()[-1]
                    return t if len(t) == 2 and t.isalpha() else ""
                except Exception:
                    return ""
            _mask &= _df["place_of_performance"].map(_state_only).isin(pick_states)
        if due_from is not None:
            try:
                _df_from = pd.to_datetime(pd.Timestamp(due_from))
                _mask &= (_df["_due_dt"] >= _df_from)
            except Exception:
                pass
        if due_to is not None:
            try:
                _df_to = pd.to_datetime(pd.Timestamp(due_to)) + pd.Timedelta(days=1)
                _mask &= (_df["_due_dt"] < _df_to)
            except Exception:
                pass
        st.caption(f"Showing {int(_mask.sum())} of {len(_df)} notices after filters")
        st.session_state["sam_results_df_filtered"] = _df[_mask].drop(columns=["_due_dt"], errors="ignore")
    else:
        st.session_state["sam_results_df_filtered"] = None
try:
    ...
    # --- Safe defaults for search selectors ---
    manual_naics = st.session_state.get("manual_naics", st.session_state.get("sam_manual_naics", ""))
    search_all = st.session_state.get("sam_search_all", False)
    watch_codes = st.session_state.get("sam_watch_naics", []) or []
    keyword = st.session_state.get("sam_keyword", "")
    # NAICS selection start
    naics_for_query = []
    if isinstance(manual_naics, str) and manual_naics.strip():
        naics_for_query = [c.strip() for c in manual_naics.split(",") if c.strip()]
    elif not search_all and isinstance(watch_codes, list) and watch_codes:
...
"""
    ...
    # >>> inserted_capability_statement_builder_marker
    st.subheader('Capability statement builder')
    st.write('Fill in company info and export a PDF/DOCX from this page. If you do not see the export buttons below, scroll.')
    # <<< inserted_capability_statement_builder_marker
    ...
    # >>> inserted_white_paper_builder_marker
    st.subheader('White paper builder')
    st.write('Enter title, audience, abstract, and sections; click Export DOCX to download.')
    # <<< inserted_white_paper_builder_marker
    ...
    # >>> inserted_data_export_marker
    st.subheader('Data Export')
    st.write('Pick tables and click Export to Excel.')
    # <<< inserted_data_export_marker
    ...
    # >>> inserted_auto_extract_key_details_marker
    st.subheader('Auto extract key details')
    st.write('Upload PDFs/DOCX/TXT to extract text and export a CSV.')
    # <<< inserted_auto_extract_key_details_marker
...
with tabs[10]:
    ...
    # >>> inserted_chat_assistant_(remembers_context;_accepts_file_uploads)_marker
    st.subheader('Chat Assistant (remembers context; accepts file uploads)')
    st.write('Type in the chat input at the bottom; uploads are supported.')
    # <<< inserted_chat_assistant_(remembers_context;_accepts_file_uploads)_marker
