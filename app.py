# ===== app.py =====
# GovCon Copilot Pro — keysafe, GPT-5 fallbacks, NAICS/Goals, SubK finder (Google Places + phone),
# SAM Watch (header+query key, postedFrom+postedTo, diagnostics+raw), RFP tools, White Paper,
# Auto Extract, Ask-the-Doc, and Chat Assistant w/ context + file uploads.
...
# ---------- Safe key loader: env first, then st.secrets ----------
...
# Trim critical keys (avoid hidden whitespace/newline issues)
OPENAI_API_KEY    = (OPENAI_API_KEY or "").strip()
GOOGLE_PLACES_KEY = (GOOGLE_PLACES_KEY or "").strip()
SAM_API_KEY       = (SAM_API_KEY or "").strip()

# ---------- OpenAI SDK import guard & client ----------
try:
    import openai as _openai_pkg
    from openai import OpenAI  # openai>=1.0.0 (recommend >=1.40.0)
    _openai_version = getattr(_openai_pkg, "__version__", "unknown")
except Exception as e:
    raise RuntimeError(
        "OpenAI SDK is missing or too old. "
        "Fix: in requirements.txt set 'openai>=1.40.0'. "
        f"Original import error: {e}"
    )

client = OpenAI(api_key=OPENAI_API_KEY) if OPENAI_API_KEY else None

# ---------- Model selection: your GPT-5 models + safe fallbacks ----------
...
SCHEMA = {
    ...
    """,
    # Chat file attachments (drag & drop)
    "chat_files": """
...
def llm(system, prompt, temp=0.2, max_tokens=1400):
    ...
    for model_name in _OPENAI_FALLBACK_MODELS:
        try:
            ...
            last_err = e; continue
    err_name = type(last_err).__name__ if last_err else "UnknownError"
    return f"LLM error ({err_name}). Tip: set OPENAI_MODEL to a model you have (e.g. 'gpt-5-chat-latest')."
...
# ---------- Context Builder for Chat Assistant ----------
...
# ---------- External integrations (LinkedIn, Google Places, Microsoft Graph) ----------
...
def google_places_search(query, location="Houston, TX", radius_m=80000, strict=True):
    """
    Google Places Text Search + Details (for phone + website).
    Returns (list_of_vendors, info_dict). Emails are NOT provided by Google.
    """
    ...
    try:
        ...
        if status_code != 200 or api_status not in ("OK","ZERO_RESULTS"):
            return ([] if strict else results), {
                ...
                "note": "Enable billing + Places API; ensure 'Places API' is ON in Google Cloud."
            }

        # 2) For each result, hit Details API for phone + website
        out = []
        for item in results:
        ...
        info = {
            ...
            "http": status_code, "api_status": api_status,
            "raw_preview": (rs.text or "")[:800]
        }
        return out, info
    except Exception as e:
...
def send_via_graph(to_addr, subject, body):
    ...
    try:
        ...
        if not token:
            return f"Graph token error: {token_r.text[:200]}"
        user_id = "me"
        r = requests.post(
...
def save_chat_files(session_id: int, uploads):
    if not uploads: return 0
    conn = get_db(); cur = conn.cursor(); added = 0
    for up in uploads:
        text = _extract_text_from_upload(up)[:800_000]
        cur.execute("""insert into chat_files(session_id, filename, mimetype, content_text)
                       values(?,?,?,?)""",
                    (session_id, up.name, getattr(up, "type", ""), text))
        added += 1
    conn.commit(); return added

def list_chat_files(session_id: int) -> pd.DataFrame:
    conn = get_db()
...
def delete_chat_file(file_id: int):
    conn = get_db()
    conn.execute("delete from chat_files where id=?", (file_id,))
    conn.commit()

def build_doc_context_for_question(session_id: int, question: str, max_snips=8, snip_chars=1200):
    conn = get_db()
    ...
    for _, r in rows.iterrows():
        file_chunks = chunk_text(r["content_text"], max_chars=snip_chars, overlap=200)
        chunks.extend(file_chunks)
        labels.extend([r["filename"]] * len(file_chunks))
    vec, X = embed_texts(chunks)
    top = search_chunks(question, vec, X, chunks, k=min(max_snips, len(chunks)))
    out, used = ["Attached document snippets (most relevant first):"], set()
    for sn in top:
        ...
        used.add(key)
        out.append(f"\n--- {fname} ---\n{sn.strip()}\n")
    return "\n".join(out[: (max_snips * 2) ])

# ---------- Robust SAM search (returns (df, info)) ----------
def sam_search(
    naics_list,
    min_days=3,
    limit=100,
    ...
    posted_from = (today - timedelta(days=posted_from_days)).isoformat()
    posted_to   = today.isoformat()  # SAM requires both ends

    params = {
        "api_key": SAM_API_KEY,            # also in query
        "limit": str(limit),
        "response": "json",
        "sort": "-publishedDate",
        "active": active,
        "postedFrom": posted_from,
        "postedTo": posted_to,            # NEW
    }
    if notice_types:
        params["noticeType"] = notice_types
    if naics_list:
        params["naics"] = ",".join([c for c in naics_list if c][:20])
    if keyword:
        params["keywords"] = keyword

    try:
        headers = {"X-Api-Key": SAM_API_KEY}  # and in header
        ...
        for opp in items:
            ...
            try:
                due_ok = (not due) or (datetime.fromisoformat(due.replace("Z","")).date() >= min_due_date)
            except Exception:
                due_ok = True
            if not due_ok:
                continue
        ...
        info = {
            "ok": True, "status": status, "count": len(df),
        ...
        if df.empty:
            info["hint"] = "Try min_days=0–1, add a keyword, increase postedFrom window, or clear noticeType for a broad test."
        return df, info
...
with st.sidebar:
    ...
    st.caption(f"Requested model: {OPENAI_MODEL}")
    if st.button("Test model"):
        msg = llm("You are a health check.", "Reply with READY.", max_tokens=5)
        st.info(msg)
    ...
    if st.button("Test SAM key"):
        try:
            test_params = {"api_key": SAM_API_KEY, "limit": "1", "response": "json",
                           "postedFrom": datetime.utcnow().date().isoformat(), "postedTo": datetime.utcnow().date().isoformat()}
    ...
    if st.button("Test Google Places key"):
        ...
        st.write("Places diagnostics:", info)
        st.write("Sample results:", vendors[:3])
    ...
    with col_n1:
        if st.button("Add code"):
            ...
            if val:
                ...
                conn.commit(); st.success(f"Added {val}")
    with col_n2:
    ...
    if naics_csv and st.button("Import NAICS from CSV"):
        ...
        if "code" in df_in.columns:
            ...
            conn.commit(); st.success("Imported NAICS codes")
        else:
    ...
    if g.empty:
        conn.execute(
            "insert into goals(year,bids_target,revenue_target,bids_submitted,revenue_won) values(?,?,?,?,?)",
            (datetime.now().year, 156, 600000, 1, 0)
        ); conn.commit(); g = pd.read_sql_query("select * from goals limit 1", conn)
...
with tabs[1]:
    ...
    with colA:
        if st.button("Google Places import"):
            ...
            if places_diag:
                st.write("Places diagnostics:", info)
                st.code(info.get("raw_preview","") or "", language="json")
            if results:
...
# ---------- SAM Watch (diagnostics + keyword + postedFrom+postedTo + raw) ----------
with tabs[4]:
    ...
    with cA:
        if st.button("Run search now"):
            ...
            if diag:
                ...
                st.code(f"naics={','.join(codes[:20])} | keyword={keyword or ''} | postedFrom={int(posted_from_days)}d | min_days={min_days} | limit=150", language="text")
                if raw:
                    st.code(info.get("raw_preview","") or "", language="json")
            if not info.get("ok"):
    ...
    with cB:
        if st.button("Broad test (keyword only)"):
            ...
            df, info = sam_search(
                [], min_days=0, limit=100, keyword=kw,
                posted_from_days=60, notice_types="", active="true"
            )
            if diag:
                st.write("Diagnostics:", info)
                st.code(f"keyword={kw} | postedFrom=60d | min_days=0 | limit=100 | noticeTypes=(none)", language="text")
                if raw:
                    st.code(info.get("raw_preview","") or "", language="json")
            if not info.get("ok"):
                st.error(f"SAM API error: {info}")
    ...
    with cC:
        if st.button("Test SAM key only"):
            try:
                test_params = {
                    "api_key": SAM_API_KEY, "limit": "1", "response": "json",
                    "postedFrom": datetime.utcnow().date().isoformat(),
                    "postedTo": datetime.utcnow().date().isoformat()
                }
                headers = {"X-Api-Key": SAM_API_KEY}
                r = requests.get("https://api.sam.gov/opportunities/v2/search", params=test_params, headers=headers, timeout=20)
                st.write("HTTP", r.status_code)
    ...
    st.dataframe(
        pd.read_sql_query("select title, agency, naics, response_due, url from opportunities order by posted desc limit 50", conn),
...
# ---------- Chat Assistant (multi-turn + context + file uploads) ----------
with tabs[11]:
    st.subheader("Chat Assistant (remembers context; accepts file uploads)")

    conn = get_db()

    # Create or pick a chat session
    ...
    if pick == "➕ New chat":
        ...
        if st.button("Start chat"):
            cur = conn.cursor(); cur.execute("insert into chat_sessions(title) values(?)", (new_title,))
    ...
    # System prompt (auto-includes company + goals)
    g = pd.read_sql_query("select * from goals limit 1", conn)
    goals_line = ""
    if not g.empty:
        rr = g.iloc[0]
...
Keep responses concise and actionable. Use bullet points when helpful. Ask clarifying questions only when necessary."""
    ...
    if uploads and st.button("Add to chat"):
        added = save_chat_files(session_id, uploads)
        st.success(f"Added {added} file(s) to this chat.")

    # List attachments
    files_df = list_chat_files(session_id)
    if files_df.empty:
        ...
        if st.button("Delete selected file") and del_id > 0:
            delete_chat_file(int(del_id)); st.success(f"Deleted file id {del_id}."); st.rerun()

    # Show history
    hist = pd.read_sql_query("select role, content, created_at from chat_messages where session_id=? order by id asc", conn, params=(session_id,))
    if hist.empty:
    ...
    if user_msg:
        ...
        msgs = pd.read_sql_query(
            "select role, content from chat_messages where session_id=? order by id asc", conn, params=(session_id,)
        ...
        # Inject business snapshot + relevant doc snippets
        context_snap = build_context(max_rows=6)
        doc_snips = build_doc_context_for_question(session_id, user_msg, max_snips=8, snip_chars=1200)
    ...
    with st.expander("Utilities"):
        colA, colB, colC = st.columns(3)
        with colA:
            newt = st.text_input("New title", value=cur_title, key="rename_title")
            if st.button("Save new title"):
                conn.execute("update chat_sessions set title=? where id=?", (newt.strip() or cur_title, session_id)); conn.commit(); st.success("Renamed"); st.rerun()
        with colB:
            if st.button("Export chat to txt"):
                msgs = pd.read_sql_query("select role, content from chat_messages where session_id=? order by id asc", conn, params=(session_id,))
                text = "\n\n".join([f"{m['role'].upper()}:\n{m['content']}" for _, m in msgs.iterrows()])
                st.download_button("Download", data=text.encode("utf-8"), file_name=f"chat_{session_id}.txt")
        with colC:
            if st.button("Clear chat"):
                conn.execute("delete from chat_messages where session_id=?", (session_id,)); conn.commit(); st.success("Cleared messages. Type to start again.")
# ===== end app.py =====
