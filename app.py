# ===== app.py =====
# GovCon Copilot Pro — safe key loading, GPT-5 fallbacks, saved NAICS, goals, subcontractor finder,
# SAM Watch, RFP tools, White Paper, Auto Extract, Ask-the-Doc, and a Chat Assistant with context memory.
...
# ---------- Safe key loader: env first, then st.secrets (if present) ----------
...
_OPENAI_FALLBACK_MODELS = [
    OPENAI_MODEL,            # your override/default
    "gpt-5-chat-latest",
    "gpt-5",
    "gpt-5-2025-08-07",
    "gpt-5-mini",
    ...
    # broad safety nets if account access changes
    "gpt-4o-mini",
    "gpt-4o",
]
...
def llm(system, prompt, temp=0.2, max_tokens=1400):
    """Single-turn LLM with model fallbacks."""
    if not client:
...
def llm_messages(messages, temp=0.2, max_tokens=1400):
    """Multi-turn chat with fallbacks. messages = list of {role, content}."""
    if not client:
        return "Set OPENAI_API_KEY to enable drafting."
    last_err = None
    for model_name in _OPENAI_FALLBACK_MODELS:
...
def build_context(max_rows=6):
    """Create a short snapshot of current pipeline/goals/vendors for the assistant."""
    conn = get_db()
    ...
    opp_lines = ["- " + " | ".join(filter(None, [
        str(r["title"])[:80],
        str(r["agency"])[:40],
        f"due {str(r['response_due'])[:16]}",
        f"NAICS {str(r['naics'])[:18]}",
    ])) for _, r in opp.iterrows()]

    # Vendor snapshot (top NAICS by count)
...
# ---------- Chat Assistant (multi-turn with context) ----------
with tabs[11]:
    st.subheader("Chat Assistant (remembers context)")
    ...
    if pick == "➕ New chat":
        ...
        if st.button("Start chat"):
            cur = conn.cursor()
            cur.execute("insert into chat_sessions(title) values(?)", (new_title,))
            conn.commit()
            st.rerun()
        st.stop()
...
Keep responses concise and actionable. Use bullet points when helpful. Ask clarifying questions only when necessary."""
    ...
    # Input + response
    user_msg = st.chat_input("Ask a question… e.g., 'Draft a 1-page technical approach for landscaping at a VA clinic'")
    if user_msg:
        # Save/refresh system message at top if changed
        ...
        if last_sys.empty or last_sys.iloc[0]["content"] != sys_prompt:
            conn.execute("insert into chat_messages(session_id, role, content) values(?,?,?)", (session_id, "system", sys_prompt))
            conn.commit()

        # Save user message
        conn.execute("insert into chat_messages(session_id, role, content) values(?,?,?)", (session_id, "user", user_msg))
        conn.commit()

        # Build a token-friendly message window (system + last ~12 user turns)
        ...
        for m in msgs[::-1]:
            ...
            if m["role"] == "system" and not sys_seen:
                pruned.append(m); sys_seen = True
        msgs_window = list(reversed(pruned))

        # Inject a fresh context snapshot at the top
        context_snap = build_context(max_rows=6)
        msgs_with_ctx = [{"role":"system","content": f"Context snapshot (keep answers consistent with this):\n{context_snap}"}] + msgs_window

        # Call model and save
        assistant_out = llm_messages(msgs_with_ctx, temp=0.2, max_tokens=1200)
        conn.execute("insert into chat_messages(session_id, role, content) values(?,?,?)", (session_id, "assistant", assistant_out))
        conn.commit()
    ...
    with st.expander("Utilities"):
        ...
        with colA:
            ...
            if st.button("Save new title"):
                conn.execute("update chat_sessions set title=? where id=?", (newt.strip() or cur_title, session_id))
                conn.commit()
                st.success("Renamed")
                st.rerun()
        with colB:
        ...
        with colC:
            if st.button("Clear chat"):
                ...
                conn.commit()
                st.success("Cleared messages. Type to start again.")
# ===== end app.py =====
