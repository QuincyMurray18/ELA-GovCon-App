...
def s1_render_places_panel(conn, default_addr=None):
    import streamlit as st, pandas as pd, json as _json
    ...
    ss.setdefault("s1_results", None)         # list of row dicts for table
    ss.setdefault("s1_raw_results", None)     # raw API results keyed by place_id
    ss.setdefault("s1_saved_ids", set())      # set of place_ids saved this session
    ...
    if (do_search or do_next) and key_addr:
        ...
        # Build table rows and raw results map
        rows = []
        raw = {}
        for r in data.get("results", []):
            ...
            # Respect hide_saved by checking DB and this-session saved
    ...
    # Render any existing table from session even after rerun
    ...
    with st.form("s1_save_form", clear_on_submit=False):
        ...
        submit = st.form_submit_button("Save selected to vendors")
    ...
    if submit and to_save:
        ...
        # Refresh the rows shown, removing any that are now saved when hide_saved is on
        if hide_saved:
