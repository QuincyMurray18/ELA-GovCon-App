...
vendor_import_grid = st.data_editor(
    df_new.assign(Link=df_new.get("website","")).assign(Save=False) if "website" in df_new.columns else df_new.assign(Save=False),
    column_config={
...
if st.button("Save to vendors"):
                ...
                saved = 0
                to_save = vendor_import_grid if isinstance(vendor_import_grid, pd.DataFrame) else None
                    ...
                    if vid:
                        ...
                        cur.execute(
                            ...
                            (company, r.get("naics",""), trade, phone, email, website, city, state, "", "", extra_note, source)
                                        ...
                                        # Upsert a matching Contact entry for this vendor row
                    try:
                        ...
                        pass
                    saved += 1                    # Mirror saved vendors into Contacts
                    conn.commit()
