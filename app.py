...
# ---- Profile Picker (Quincy / Charles / Collin) ----
...
st.session_state["active_profile"] = st.radio(
    "Choose a profile", _profiles,
    index=_profiles.index(st.session_state["active_profile"]),
    horizontal=True
)
...
with tabs[4]:
    ...
    if info and not info.get("ok", True):
        ...
        if st.button("Save selected to pipeline"):
            ...
            ins, upd = save_opportunities(to_save, default_assignee=(assignee_default or st.session_state.get('active_profile','')))
