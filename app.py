...
def sam_search(naics_list, min_days=3, limit=100, keyword=None, posted_from_days=30,
    ...
    """Query SAM.gov. If no NAICS and no keyword, return all active within the posted window."""
    if not SAM_API_KEY:
    ...
    try:
        ...
        return pd.DataFrame(), {"ok": False, "reason": "request_error", "detail": str(e)}

    if status != 200 or (isinstance(data, dict) and data.get("message")):
        msg = data.get("message") if isinstance(data, dict) else ""
        return pd.DataFrame(), {"ok": False, "reason": "api_error", "status": status, "message": msg, "raw_preview": raw_preview}
    ...
    for opp in items:
        ...
        rows.append({
            ...
            "naics": ",".join(opp.get("naicsCodes", []) or []),
            "psc": ",".join(opp.get("productOrServiceCodes", []) or []),
    ...
    info = {
        ...
        "filters": {
            ...
            "postedFrom": posted_from,
            "postedTo": posted_to,
            "notice_types": params.get("noticeType",""),
...
with cA:
    if st.button("Run search now"):
        df, info = sam_search(
            ...
            notice_types=",".join(notice_types) if notice_types else "", active="true"
        )
