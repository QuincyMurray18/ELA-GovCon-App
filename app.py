...
# ---------- SAM search ----------
def sam_search(
    naics_list, min_days=3, limit=100, pages=1, keyword=None, posted_from_days=30,
    ...
    try:
        ...
        if "sam_notice_id" in df.columns:
            df = df.drop_duplicates(subset=["sam_notice_id"]).reset_index(drop=True)
...
with tabs[4]:
    ...
    with cA:
        if st.button("Run search now"):
            df, info = sam_search(codes, min_days=min_days, limit=150, pages=int(pages_to_fetch),
    ...
    with cB:
        if st.button("Broad test (keyword only)"):
            ...
            df, info = sam_search([], min_days=0, limit=100, pages=int(pages_to_fetch), keyword=kw, posted_from_days=60,
                notice_types="Combined Synopsis/Solicitation,Solicitation", active="true"
            )
            st.session_state["sam_results_df"] = df
            st.session_state["sam_results_info"] = info
