...
def ensure_schema():
    ...
    try:
        ...
        pass
    try:
        cur.execute("create index if not exists idx_opp_posted on opportunities(posted)")
        cur.execute("create index if not exists idx_opp_due on opportunities(response_due)")
    except Exception:
        pass
    try:
        cur.execute("create unique index if not exists idx_vendors_unique on vendors(company, ifnull(phone,''), ifnull(website,''))")
        cur.execute("create index if not exists idx_vendors_updated on vendors(updated_at)")
    except Exception:
        pass
    try:
        cur.execute("create index if not exists idx_contacts_created on contacts(created_at)")
    except Exception:
        pass
    try:
        cur.execute("create index if not exists idx_rfp_sessions_created on rfp_sessions(created_at)")
        cur.execute("create index if not exists idx_rfp_messages_session on rfp_messages(session_id)")
        cur.execute("create index if not exists idx_rfp_files_session on rfp_files(session_id)")
    except Exception:
        pass
    # seed goals
...
def save_opportunities(df):
    ...
    for _, r in df.iterrows():
        cur.execute("""insert or ignore into opportunities
...
tabs = st.tabs([
    ...
    "Pricing & Quote Builder","Data Export","Auto extract","Chat Assistant",
])
    ...
    with colA:
        if st.button("Google Places import"):
            ...
            if results:
                ...
                sort_by = st.selectbox('Sort by', options=['company','website','phone'])
    ...
    with cA:
        if st.button("Run search now"):
            ...
            if not info.get("ok"):
                ...
                st.dataframe(df, use_container_width=True, column_config={'url': st.column_config.LinkColumn('Link', display_text='open')})
...
with tabs[5]:
    ...
    if prompt:
        conn.execute("insert into rfp_messages(session_id, role, content) values(?,?,?)", (rfp_session_id, "user", prompt)); conn.commit()
        ...
        if draft_text:
            grade_block = f"""
You also have the vendor draft below. Act like a contracting officer. Derive a scoring rubric from evaluation factors and Section M if present. Score on a 0 to 5 scale per factor and provide strengths, weaknesses, risks, and specific corrections. Then produce 'Targeted Improvements by Section' with concrete rewrites or bullet changes mapped to section IDs. Quote section IDs and page numbers when visible.
Vendor draft
{draft_text[:120000]}
"""
        ...
        conn.execute("insert into rfp_messages(session_id, role, content) values(?,?,?)", (rfp_session_id, "assistant", assistant_out)); conn.commit()
...
with tabs[8]:
    ...
    def _try_fetch_awards(naics, keywords, place, years_back, low, high):
        # Attempt to query open data (USASpending/FPDS). If it fails, return empty with diagnostics.
        import requests, json, datetime
        info = {"ok": False, "reason": "", "hint": ""}
        rows = []
        try:
            # Simple attempt: USASpending search/spending_by_award requires POST; build a narrow query.
            ...
            if r.status_code != 200:
                info.update(ok=False, reason=f"http_{r.status_code}", hint=r.text[:300])
                return rows, info
            data = r.json()
            results = data.get("results", []) or []
            for it in results:
    ...
    if comp_rows:
        ...
        # Duration calc (years) and yearly avg
        ...
        if not df_comp.empty:
            ...
            st.caption("Heuristic: If 'years' ~ 1, likely one-time or one-year; ~5 suggests base + 4 option years.")
            st.metric("Comparable awards", len(df_comp))
            st.metric("Median award $", f"${df_comp['amount'].median():,.0f}")
            st.metric("Median avg per year $", f"${df_comp['avg_per_year'].median():,.0f}")

    if comp_csv:
        df_up = pd.read_csv(comp_csv)
        ...
        if "Award Amount" in df_up.columns:
            ...
            st.metric("Median award $", f"${pd.to_numeric(df_up['Award Amount'], errors='coerce').median():,.0f}")
    ...
    # Simple production estimate: FTEs as function of size and shifts (heuristic for janitorial)
    def estimate_ftes(unit, size, shifts):
        if unit == "Square Feet":
            # 20k-25k sq ft per FTE per shift range; use 22k
            return max(0.5, (size / 22000.0) * shifts)
        if unit == "Acres":
            # 12-15 acres per FTE per shift; use 13.5
            return max(0.5, (size / 13.5) * shifts)
        return max(0.1, size * shifts)  # if size given as FTEs baseline

    ftes = estimate_ftes(size_unit, size_value, shifts)
    st.caption(f"Estimated FTEs: {ftes:.2f} (heuristic)")

    hours_per_year = days_per_week * 52 * 8  # 8-hr shifts
    base_plus_options = [1]*base_years + [0]*option_years  # marker list

    # Build per-year cost table
    rows = []
    labor_rate_loaded = wage_rate * (1 + burden_pct/100.0)
    for y in range(1, base_years + option_years + (1 if base_years+option_years==0 else 0) + 1):
        # If no years entered, treat as one-time
    ...
    if not df_quote.empty:
        ...
        if size_value > 0 and size_unit in ("Square Feet","Acres"):
            per_unit = df_quote["total"].sum() / size_value / max(1, (base_years+option_years or 1))
