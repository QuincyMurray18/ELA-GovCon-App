...
def run_outreach(conn):
    import streamlit as st
    # Single, guarded Outreach UI. removes duplicate calls that caused NameError/UnboundLocalError.
    st.header("Outreach")

    # O4 — multi-sender, opt-outs, audit
    ...
    try:
        ...
        with st.expander("Templates", expanded=False):
            try:
                ...
                st.warning(f"Templates UI unavailable: {e}")
    except Exception as e:
        st.warning(f"Templates not available: {e}")

    # Subject and body inputs (prefilled by template picker)
