        ...
        if row:
            cur.execute(
                """update opportunities set title=?, agency=?, naics=?, psc=?, place_of_performance=?,
                   ...
                   where sam_notice_id=?""",
                (vals["title"], vals["agency"], vals["naics"], vals["psc"], vals["place_of_performance"],
                 vals["response_due"], vals["posted"], vals["type"], vals["url"], vals["attachments_json"], nid)
            )
            updated += 1
        else:
            cur.execute(
                """insert into opportunities
                   ...
                   values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (nid, vals["title"], vals["agency"], vals["naics"], vals["psc"], vals["place_of_performance"],
                 vals["response_due"], vals["posted"], vals["type"], vals["url"], vals["attachments_json"],
                 "New", default_assignee or "", "")
            )
            inserted += 1
    conn.commit()
        ...
        for _, r in edit.iterrows():
            cur.execute(
                ...
                (r.get("status","New"), r.get("response_due"), r.get("title"), r.get("agency"),
                 r.get("assignee",""), r.get("quick_note",""), int(r["id"]))
            )
