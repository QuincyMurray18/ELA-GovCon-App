...
# === SAM WATCH V2 (MINIMAL EMBED) START ===
# Lightweight SAM.gov watcher embedded without touching existing tabs.
# Opens from sidebar -> dialog (or expander fallback).

from __future__ import annotations
import sqlite3, json, os, datetime as _dt
...
def _log(msg: str):
    try:
        st.session_state.setdefault("_samv2_logs", []).append(msg)
    except Exception:
        pass
...
def _conn():
    ...
    c.execute("CREATE TABLE IF NOT EXISTS samv2_opportunities("
              "id INTEGER PRIMARY KEY AUTOINCREMENT,"
...
def samv2_search(params: dict) -> list[dict]:
    if requests is None or not SAM_API_KEY:
        _log("Requests or SAM_API_KEY missing."); return []
    headers = { "Accept": "application/json", "X-Api-Key": SAM_API_KEY }
    q = params.copy()
    nt_map = { "Solicitation":"opp", "Combined Synopsis/Solicitation":"comsynopp",
               "Presolicitation":"presolicitation", "Sources Sought":"sources_sought" }
    nt = q.pop("notice_type", "") or q.get("noticeType","")
    if isinstance(nt, str) and nt:
        parts = [p.strip() for p in nt.split(",") if p.strip()]
        q["noticeType"] = ",".join([nt_map.get(p,p) for p in parts])
    if "postedFrom" in q: q["postedFrom"] = _to_us_date(q["postedFrom"])
    if "postedTo" in q: q["postedTo"] = _to_us_date(q["postedTo"])
    q.setdefault("limit", 100)
    q["api_key"] = SAM_API_KEY
    try:
        r = requests.get(SAM_API_BASE, params=q, headers=headers, timeout=30)
        if r.status_code != 200:
            _log(f"SAM error {r.status_code}: {r.text[:160]}"); return []
        data = r.json()
        out = []
        for attrs in data.get("opportunitiesData", []):
            pop = attrs.get("placeOfPerformance") or {}
            out.append({
                "notice_id": str(attrs.get("noticeId") or attrs.get("id") or attrs.get("solicitationNumber") or ""),
                "sol_number": attrs.get("solicitationNumber") or "",
                "title": attrs.get("title") or "",
                "agency": (attrs.get("agency") or {}).get("name") if isinstance(attrs.get("agency"), dict) else (attrs.get("agency") or ""),
                "naics": ", ".join(attrs.get("naicsCodes", []) or attrs.get("naics", []) or []),
                "psc": ", ".join(attrs.get("pscCodes", []) or attrs.get("productServiceCode", []) or []),
                "set_aside": attrs.get("typeOfSetAside") or attrs.get("setAside") or "",
                "posted_date": attrs.get("publishDate") or attrs.get("postedDate") or "",
                "due_date": attrs.get("responseDate") or attrs.get("archiveDate") or "",
                "place_of_performance": pop.get("address","") if isinstance(pop, dict) else "",
                "sam_detail_url": attrs.get("uiLink") or attrs.get("url") or "",
                "description": attrs.get("description") or ""
            })
        return out
    except Exception as ex:
        _log("SAM exception: " + str(ex)); return []

def samv2_upsert(rows: list[dict]) -> tuple[int,int]:
    conn = _conn(); cur = conn.cursor()
    ins = upd = 0
    for r in rows:
        cur.execute("SELECT id FROM samv2_opportunities WHERE notice_id=?", (r.get("notice_id",""),))
        hit = cur.fetchone()
        if hit:
            cur.execute("UPDATE samv2_opportunities SET sol_number=?, title=?, agency=?, naics=?, psc=?, set_aside=?, posted_date=?, due_date=?, place_of_performance=?, sam_detail_url=?, description=?, updated_at=datetime('now') WHERE notice_id=?",
                        (r.get("sol_number"), r.get("title"), r.get("agency"), r.get("naics"), r.get("psc"), r.get("set_aside"),
                         r.get("posted_date"), r.get("due_date"), r.get("place_of_performance"), r.get("sam_detail_url"), r.get("description"), r.get("notice_id")))
            upd += 1
        else:
            cur.execute("INSERT INTO samv2_opportunities (notice_id, sol_number, title, agency, naics, psc, set_aside, posted_date, due_date, place_of_performance, sam_detail_url, description) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
                        (r.get("notice_id"), r.get("sol_number"), r.get("title"), r.get("agency"), r.get("naics"),
                         r.get("psc"), r.get("set_aside"), r.get("posted_date"), r.get("due_date"),
                         r.get("place_of_performance"), r.get("sam_detail_url"), r.get("description")))
            ins += 1
    conn.commit(); conn.close()
    return ins, upd

def render_sam_watch_v2():
    st.title("SAM Watch V2 (Minimal)")
    st.caption("Pull SAM.gov, list opportunities, and hand off to your analyzer/pipeline.")
    with (st if st.session_state.get("_samv2_in_dialog") else st.sidebar):
        st.subheader("Filters")
        kw = st.text_input("Keywords", value=st.session_state.get("_sam_kw",""))
        naics = st.text_input("NAICS (comma-separated)", value=st.session_state.get("_sam_naics",""))
        types = st.multiselect("Notice types",
            ["Solicitation","Combined Synopsis/Solicitation","Presolicitation","Sources Sought"],
            default=st.session_state.get("_sam_types", ["Solicitation","Combined Synopsis/Solicitation","Presolicitation","Sources Sought"])
        )
        set_aside = st.selectbox("Set-Aside", ["Any","Total Small Business","WOSB","SDVOSB","8(a)","HUBZone"], index=0)
        d_from = st.date_input("Posted from", value=_dt.date.today() - _dt.timedelta(days=60))
        d_to = st.date_input("Posted to", value=_dt.date.today())
        pull = st.button("Pull SAM Data")

    if pull:
        params = {
            "q": kw or "",
            "notice_type": ",".join(types),
            "postedFrom": d_from,
            "postedTo": d_to,
        }
        if naics.strip(): params["naics"] = naics
        if set_aside != "Any": params["setAside"] = set_aside
        rows = samv2_search(params)
        ins, upd = samv2_upsert(rows)
        st.success(f"Fetched {len(rows)}; inserted {ins}, updated {upd}.")

    conn = _conn()
    cur = conn.cursor()
    cur.execute("SELECT notice_id, sol_number, title, agency, naics, psc, set_aside, posted_date, due_date, sam_detail_url FROM samv2_opportunities ORDER BY COALESCE(posted_date, created_at) DESC LIMIT 300")
    recs = cur.fetchall()
    conn.close()

    if not recs:
        st.info("No records yet. Use the filters and click Pull SAM Data.")
    else:
        for r in recs:
            nid, sol, title, agency, naics_v, psc, sa, posted, due, link = r
            with st.container(border=True):
                ...
                st.caption(f"Posted: {posted or 'N/A'} | Due: {due or 'N/A'}")
                cols = st.columns([0.25, 0.25, 0.25, 0.25])
                with cols[0]:
                    if link: st.markdown(f"[Open in SAM.gov]({link})")
                    else: st.text("No SAM link")
                with cols[1]:
                    if st.button("Ask RFP Analyzer", key=f"ask_{nid}"):
                        st.session_state["sam_watch_selected_notice_id"] = nid
                        st.session_state["sam_watch_selected_title"] = title
                        st.success("Ready — switch to your Analyzer tab/workflow.")
                with cols[2]:
                    if st.button("Save to Pipeline", key=f"save_{nid}"):
                        st.info("TODO: hook to your pipeline/deals table here.")
...
def _sidebar_launcher():
    ...
    try:
        with st.sidebar:
            if st.button("🚀 Launch SAM Watch V2", key="__samv2_launch_min"):
        ...
        if st.session_state.get("_samv2_open"):
            try:
                ...
                def _dlg():
                    ...
                    if st.button("Close", key="__samv2_close_min"):
                ...
                with st.expander("SAM Watch V2", expanded=True):
                    ...
                    try:
                        ...
                        st.session_state["_samv2_in_dialog"] = False
                    if st.button("Close", key="__samv2_close_fallback_min"):
                        st.session_state["_samv2_open"] = False
                        try: st.rerun()
                        except Exception: pass
    except Exception as ex:
        _log("Sidebar launcher error (minimal): " + str(ex))

try:
    _sidebar_launcher()
except Exception as ex:
    _log("SAM V2 init error (minimal): " + str(ex))
# === SAM WATCH V2 (MINIMAL EMBED) END ===
