...
def sam_search(
    ...
    # Set noticeType only if provided (blank means mirror SAM.gov and includes all notice types)
if notice_types:
...
mirror_sam = st.checkbox(
    "Mirror SAM.gov (all notice types)",
    value=False,
    help="Show all active notice types like the website."
)
with col2:
    ...
    notice_types=("" if mirror_sam else "Combined Synopsis/Solicitation,Solicitation"),
    active="true"
)
    ...
    if info and not info.get("ok", True):
        ...
        # Only restrict to Sol/Combined if not mirroring SAM.gov
if not mirror_sam:
...
Keep responses concise and actionable. Use bullet points when helpful. Ask clarifying questions only when necessary."""
    ...
    if hist.empty:
        ...
        for _, row in hist.iterrows():
            ...
            else: st.caption(f"Ã°Å¸Â§  System updated at {row['created_at']}")
