...
try:
    ...
    # If Streamlit not available at import, defer errors to runtime
    pass

def _pb_safe_key(label: str) -> str:
    """Sanitize section label into a stable Streamlit key tail."""
    base = re.sub(r"[^a-z0-9]+", "_", (label or "").lower()).strip("_")
    # Map common aliases to a single token so UI keys stay consistent
    alias = {
...
def _pb_build_prompt(section_title: str, ctx: dict, target_words: int) -> str:
    ...
    try:
        blueprint = _section_blueprint(section_title)
    except Exception:
        blueprint = f"Write only the {section_title} content. Do not include other sections."
    # Hard guard against cross-section bleed
    guard = (
        ...
        "(procedures, metrics, artifacts, timelines, acceptance criteria) rather than adding other sections."
    )
    # Build governing context from RFP Analyzer
    # Prefer parsed text and analyzer summaries over generic snippets
    rfp_bits = []
    try:
...
def _pb_strip_cross_section_bleed(section_title: str, text: str) -> str:
    """Remove content that looks like unrelated top-level sections creeping in."""
    try:
        return _finalize_section(section_title, text or "")
    except Exception:
        # Fallback: simple heading filter
        headings = [
            "Executive Summary","Technical Approach","Management Approach","Staffing","Quality Control",
            "Risk","Risks and Mitigations","Transition Plan","Price Approach","Past Performance",
            "Subcontractor Plan","Compliance Crosswalk","Cover Letter"
        ...
        for ln in (text or "").splitlines():
            m = re.match(r"^\s*([A-Z][A-Za-z ]{2,40})\s*:?\s*$", ln.strip())
            if m and m.group(1) in headings:
                # Stop if this is not our section
                norm = _normalize_section_name(m.group(1))
                if norm != _normalize_section_name(section_title):
                    break
            cur.append(ln)
        out = "\n".join(cur).strip()
...
def _pb_enforce_word_target(text: str, target: int, tol: float = 0.93) -> str:
    """Trim or lightly extend to land close to target words without adding other sections."""
    ...
    if n > target * 1.05:
        ...
        for s in sents:
            acc.append(s)
            w = len(re.findall(r\"\\b\\w+\\b\", \" \".join(acc)))
            if w >= target:
                break
        return \" \".join(acc).strip()
    # If short, leave as-is; streaming/top-off helper may handle the rest
    return text

def _pb_get_context(conn, rfp_id: int) -> dict:
    # Prefer existing app helper
    try:
        return _load_rfp_context(conn, int(rfp_id)) or {}
    except Exception:
        pass
    # Minimal fallback context from rfps_t if present
    try:
        import pandas as _pd
...
def _pb_draft_one(conn, rfp_id: int, section_title: str, target_words: int, k: int = 3) -> str:
    ...
    drafted = ""
    # Prefer existing streaming/top-off helpers if available
    try:
        drafted = y3_stream_draft(conn, int(rfp_id), section_title, prompt, int(target_words or 500), int(k))
    except Exception:
        pass
    # Optional top-off
    try:
        if drafted and len(re.findall(r\"\\b\\w+\\b\", drafted)) < int(target_words * 0.93):
            drafted = _y3_top_off_precise(drafted, int(target_words))
    except Exception:
        pass
    # Clip and finalize
...
def run_proposal_builder(conn: \"sqlite3.Connection\") -> None:
    ...
    with st.expander(\"Sections\", expanded=True):
        ...
        for i, label in enumerate(sel):
            with cols[i % 2]:
                ...
                # Initialize target word slider per section
                ...
                # Text area without any post-write mutation to session_state
        ...
        if st.button(\"Draft selected\", type=\"primary\", key=\"pb_draft_sel\"):
            for label in st.session_state.get(\"pb_sel\", []):
                try:
                    ...
                    st.session_state[_pb_ta_key(label)] = t  # safe: assign before next render cycle
                except Exception as e:
    ...
    with c3:
        if st.button(\"Export .docx\", key=\"pb_export\"):
            # Collect sections in the chosen order
            sections = []
...
# Provide a no-op for pb_phase_v_section_library if missing so router hook stays safe
