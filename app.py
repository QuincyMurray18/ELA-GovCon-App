                ...
                if st.button("Save to vendors"):
                    ...
                    for _, r in df_new.iterrows():
                        ...
                        if vid:
                            ...
                            cur.execute(
                                '''insert into vendors(company,naics,trades,phone,email,website,city,state,certifications,set_asides,notes,source)
                                   values(?,?,?,?,?,?,?,?,?,?,?,?)''',
    ...
    with cB:
        if st.button("Broad test (keyword only)"):
            ...
            df, info = sam_search([], min_days=0, limit=100, keyword=kw,
                                  posted_from_days=60, notice_types="", active=("true" if active_only else ""))
            if diag:
                st.write("Diagnostics:", info)
