...
# ==============================
# SAM WATCH 10/10 MODULE (ELA)
# Safe to append: self-contained namespaced functions and classes.
# ==============================
import os
import json as _json
...
def ela_sam_db_path() -> str:
    return os.environ.get("ELA_DB_PATH", os.path.join(os.getcwd(), "ela_app.db"))

def ela_sam_get_conn():
    conn = _sqlite3.connect(ela_sam_db_path(), timeout=30, check_same_thread=False)
    conn.execute("PRAGMA journal_mode=WAL;")
    conn.execute("PRAGMA foreign_keys=ON;")
    return conn

def ela_sam_init_schema(conn):
    cur = conn.cursor()
    cur.executescript("""
    CREATE TABLE IF NOT EXISTS opportunities (
      id INTEGER PRIMARY KEY,
    ...
    CREATE TABLE IF NOT EXISTS attachments (
      ...
      hash TEXT,
      downloaded_at TEXT,
      UNIQUE(notice_id, file_name)
    );
    CREATE TABLE IF NOT EXISTS pipeline_items (
      id INTEGER PRIMARY KEY,
      notice_id TEXT NOT NULL UNIQUE,
      stage TEXT,
      win_prob REAL DEFAULT 0.0,
      owner TEXT,
      notes TEXT,
      created_at TEXT DEFAULT CURRENT_TIMESTAMP,
      updated_at TEXT DEFAULT CURRENT_TIMESTAMP
    ...
    CREATE INDEX IF NOT EXISTS idx_opps_keys ON opportunities(naics, psc, set_aside, notice_type, status);
    """)
    conn.commit()

try:
    _c = ela_sam_get_conn(); ela_sam_init_schema(_c); _c.close()
except Exception:
    pass

# ---- API client (requests + retries) ----
class SamClient:
    def __init__(self, api_key: str = None, timeout: int = 30):
        import os, requests
        from requests.adapters import HTTPAdapter
        from urllib3.util import Retry
        self.base = "https://api.sam.gov/opportunities/v2"
    ...
    def map_query(self, fdict: dict) -> dict:
        q = {}
        if fdict.get("keywords"): q["searchText"] = fdict["keywords"]
        if fdict.get("naics"): q["naics"] = fdict["naics"]
        if fdict.get("set_aside"): q["setAside"] = fdict["set_aside"]
        if fdict.get("notice_type"): q["noticeType"] = fdict["notice_type"]
        if fdict.get("posted_from"): q["postedFrom"] = fdict["posted_from"]
        if fdict.get("posted_to"): q["postedTo"] = fdict["posted_to"]
        q["inactive"] = "no"; q["limit"] = "100"
        return q

    def search(self, filters: dict) -> list:
        params = self.map_query(filters)
        if self.api_key: params["api_key"] = self.api_key
        url = f"{self.base}/search/"
        try:
            r = self.s.get(url, params=params, timeout=self.timeout); r.raise_for_status()
            data = r.json()
            return data.get("opportunitiesData", data.get("data", [])) or []
        except Exception:
            return []

    def detail(self, notice_id: str) -> dict:
        try:
            res = self.s.get(f"{self.base}/opportunities/{notice_id}", params={"api_key": self.api_key}, timeout=self.timeout)
    ...
    def list_attachments(self, notice_id: str) -> list:
        det = self.detail(notice_id) or {}
        return det.get("attachments") or det.get("documents") or []

    def download_attachment(self, url: str, dest_path: str) -> str:
        try:
            resp = self.s.get(url, timeout=self.timeout); resp.raise_for_status()
...
# ---- Service layer ----
def ela_sam_upsert_opportunity(conn, record: Dict) -> None:
    keys = ["notice_id","sol_number","title","notice_type","agency","office","naics","psc","set_aside",
            "place_of_performance","posted_date","due_date","status","is_archived","sam_url"]
    rec = {k: record.get(k) for k in keys}
    rec["is_archived"] = int(rec.get("is_archived") or 0)
    rec["updated_at"] = _dt.now(_tz.utc).isoformat()
    placeholders = ", ".join("?" for _ in keys)
    update_clause = ", ".join(f"{k}=excluded.{k}" for k in keys if k != "notice_id")
    conn.execute(f"""
        INSERT INTO opportunities ({", ".join(keys)})
        VALUES ({placeholders})
        ON CONFLICT(notice_id) DO UPDATE SET
            {update_clause},
            updated_at=?
    """, [rec[k] for k in keys] + [rec["updated_at"]])
    conn.commit()

def ela_sam_upsert_contacts(conn, notice_id: str, contacts: List[Dict]) -> None:
    for c in contacts or []:
        conn.execute("""
            INSERT INTO opportunity_contacts (notice_id, name, role, email, phone, address)
            VALUES (?, ?, ?, ?, ?, ?)
            ON CONFLICT(notice_id, email, phone) DO NOTHING
        """, (notice_id, c.get("name"), c.get("role"), c.get("email"), c.get("phone"), c.get("address")))
    conn.commit()

def ela_sam_upsert_attachments_meta(conn, notice_id: str, files: List[Dict]) -> None:
    for f in files or []:
        conn.execute("""
            INSERT INTO attachments (notice_id, file_name, file_url, file_type, local_path, hash, downloaded_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ...
        """, (notice_id, f.get("file_name"), f.get("file_url"), f.get("file_type"),
              f.get("local_path"), f.get("hash"), f.get("downloaded_at")))
    conn.commit()

def ela_sam_add_to_pipeline(conn, notice_id: str, initial_stage: str = "No Contact") -> Tuple[bool, str]:
    try:
        conn.execute("""
            INSERT INTO pipeline_items (notice_id, stage)
            VALUES (?, ?)
        """, (notice_id, initial_stage))
        conn.commit()
        return True, "Added"
    except Exception as e:
        msg = str(e)
        if "UNIQUE constraint failed: pipeline_items.notice_id" in msg:
            return False, "Already in pipeline"
        return False, msg

def ela_sam_prevent_duplicates(conn, notice_id: str) -> bool:
    cur = conn.execute("SELECT 1 FROM pipeline_items WHERE notice_id = ?", (notice_id,))
    return cur.fetchone() is not None

def ela_sam_compute_diff(old: Dict, new: Dict) -> List[Dict]:
    changes = []
    for key in ("status","due_date","posted_date","title","set_aside"):
        if (old or {}).get(key) != (new or {}).get(key):
            changes.append({"field": key, "old": (old or {}).get(key), "new": (new or {}).get(key)})
    return changes

def ela_sam_log_change(conn, notice_id: str, change_type: str, payload: Dict) -> None:
    conn.execute("INSERT INTO change_log (notice_id, change_type, change_payload) VALUES (?, ?, ?)",
                 (notice_id, change_type, _json.dumps(payload)))
    conn.commit()

def ela_sam_generate_ai_summary(conn, notice_id: str, meta: Dict) -> Dict:
    overview = f"{meta.get('title','Opportunity')} for {meta.get('agency','unknown agency')} under NAICS {meta.get('naics','N/A')}."
    key_reqs = "- Provide required services per SOW.\n- Meet submission requirements in Sections L & M.\n- Comply with set-aside and NAICS eligibility."
    extracted = {
        "overview": overview, "key_requirements": key_reqs, "section_C": "",
        "section_L": "Format, page limits, submission portal or email, due date.",
        "section_M": "Evaluation factors such as Technical Approach, Past Performance, Price.",
        "deliverables": "Per SOW; confirm in attachments.", "compliance_flags": ""
    }
    conn.execute("""
        INSERT INTO rfp_summaries (notice_id, overview, key_requirements, section_C, section_L, section_M, deliverables, compliance_flags, extracted_json, model_name)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(notice_id) DO UPDATE SET
            ...
            updated_at=CURRENT_TIMESTAMP
    """, (notice_id, extracted["overview"], extracted["key_requirements"], extracted["section_C"],
          extracted["section_L"], extracted["section_M"], extracted["deliverables"], extracted["compliance_flags"],
          _json.dumps(extracted), "stub"))
    conn.commit()
    return extracted

def ela_sam_get_proposal_context(conn, notice_id: str) -> Dict:
    cur = conn.execute("SELECT * FROM opportunities WHERE notice_id = ?", (notice_id,))
    opp = cur.fetchone()
    if not opp: return {}
    colnames = [d[0] for d in cur.description]
    opp_dict = dict(zip(colnames, opp))
    cur2 = conn.execute("SELECT * FROM rfp_summaries WHERE notice_id = ?", (notice_id,))
    summ = cur2.fetchone()
    summ_dict = {}
    if summ:
        col2 = [d[0] for d in cur2.description]; summ_dict = dict(zip(col2, summ))
    cur3 = conn.execute("SELECT name, role, email, phone FROM opportunity_contacts WHERE notice_id = ?", (notice_id,))
    contacts = [{"name": r[0], "role": r[1], "email": r[2], "phone": r[3]} for r in cur3.fetchall()]
    return {"opportunity": opp_dict, "summary": summ_dict, "contacts": contacts}

def ela_sam_ui_toast_add_results(outcomes: List[Tuple[bool, str]]):
    try:
        import streamlit as st
        added = sum(1 for ok, _ in outcomes if ok)
        skipped = sum(1 for ok, msg in outcomes if not ok and "Already" in msg)
        failed = [msg for ok, msg in outcomes if not ok and "Already" not in msg]
        st.success(f"Added {added} deal(s). Skipped {skipped} duplicate(s).")
        if failed: st.warning("Some failed: " + "; ".join(failed)[:500])
    except Exception:
        pass
# End base
...
def ela_sam__detect_notice_id_col(conn):
    ...
    if "notice_id" in cols: return "notice_id"
...
def ela_sam_fetch_opportunities(filters: dict, limit: int = 200):
    ...
    if filters.get("keywords"):
        ...
        wh += ["(title LIKE ? OR agency LIKE ? OR office LIKE ?)"]; params += [kw, kw, kw]
    if filters.get("naics"): wh += ["naics LIKE ?"]; params += [f"%{filters['naics']}%"]
    if filters.get("set_aside"): wh += ["set_aside LIKE ?"]; params += [f"%{filters['set_aside']}%"]
    if filters.get("notice_type"): wh += ["notice_type LIKE ?"]; params += [f"%{filters['notice_type']}%"]
    if filters.get("posted_from"): wh += ["date(posted_date) >= date(?)"]; params += [filters["posted_from"]]
    if filters.get("posted_to"): wh += ["date(posted_date) <= date(?)"]; params += [filters["posted_to"]]
    where = ("WHERE " + " AND ".join(wh)) if wh else ""
    sql = f"""
      SELECT {nid_col} as notice_id, title, agency, naics, psc, set_aside, posted_date, due_date, status, sam_url
      FROM opportunities
      {where}
    ...
    try:
        cur2 = conn.execute("SELECT notice_id FROM pipeline_items"); in_pipe = {r[0] for r in cur2.fetchall()}
    except Exception:
        in_pipe = set()
    for r in rows: r["in_pipeline"] = 1 if r["notice_id"] in in_pipe else 0
    return rows
...
def ela_sam_ui_render_filters():
    try:
        ...
        with st.container(border=True):
            ...
            with c1: keywords = st.text_input("Keywords", key="sam_kw", placeholder="janitorial, HVAC, landscaping")
            with c2: naics = st.text_input("NAICS", key="sam_naics", placeholder="561720")
            with c3: set_aside = st.text_input("Set-aside", key="sam_setaside", placeholder="Total Small Business")
            c4, c5, c6 = st.columns([1,1,1])
            with c4: notice_type = st.selectbox("Notice type", ["","Solicitation","Combined Synopsis/Solicitation","Presolicitation","Sources Sought"], key="sam_nt")
            with c5: posted_from = st.date_input("Posted from", value=None, key="sam_from")
            with c6: posted_to = st.date_input("Posted to", value=None, key="sam_to")
            st.caption("Tip: saved searches and email alerts coming from subscriptions table.")
        return {"keywords": (keywords or "").strip(), "naics": (naics or "").strip(), "set_aside": (set_aside or "").strip(),
                "notice_type": notice_type or "", "posted_from": str(posted_from) if posted_from else None, "posted_to": str(posted_to) if posted_to else None}
    except Exception:
        return {}
...
def ela_sam_ui_render_results_table(rows):
    try:
        ...
        if not rows: st.info("No opportunities matched your filters."); return []
        df = pd.DataFrame(rows); st.write("### Results")
        selected_ids = []
        for idx, r in df.iterrows():
            with st.expander(f"{r['title']} • {r.get('agency','')} • Due: {r.get('due_date','N/A')}", expanded=False):
                ...
                if r.get("sam_url"): st.write(f"[Open in SAM.gov]({r['sam_url']})")
                add_col, analyze_col, start_col = st.columns([1,1,1])
                with add_col:
                    if st.checkbox("Select", key=f"sel_{r['notice_id']}"): selected_ids.append(r["notice_id"])
                with analyze_col:
                    if st.button("Analyze", key=f"an_{r['notice_id']}"): st.session_state["sam_modal_open"] = r["notice_id"]
                with start_col:
                    if st.button("Start Proposal", key=f"sp_{r['notice_id']}"):
                        conn = ela_sam__get_conn_safe(); ctx = ela_sam_get_proposal_context(conn, r["notice_id"])
                        st.session_state["proposal_context"] = ctx; st.session_state["active_tab"] = "Proposal Builder"; st.rerun()
                if r.get("in_pipeline"): st.caption("✅ Already in Pipeline")
        return selected_ids
    except Exception:
        return []
...
def ela_sam_render_embedded_panel():
    try:
        ...
        if selected and st.button("Add selected to Pipeline", type="primary"):
            conn = ela_sam__get_conn_safe(); outcomes = [ela_sam_add_to_pipeline(conn, nid) for nid in selected]
        ...
        if notice_id:
            with st.dialog("RFP Analyzer"):
                ...
                if st.button("Close", key="close_rfp_modal"): st.session_state["sam_modal_open"] = None
    except Exception as _e:
        try: import streamlit as st; st.warning(f"[SAM Watch UI error] {str(_e)[:300]}")
        except Exception: pass
# End UI helpers
...
def ela_sam_transform_record(api_row: dict) -> dict:
    ...
    naics = ""
    try: naics = ",".join(str(x) for x in (get("naicsCodes") or [])) if isinstance(get("naicsCodes"), list) else (get("naics") or "")
    except Exception: naics = get("naics") or ""
    psc = get("productServiceCode") or get("psc") or ""
    set_aside = get("setAside") or ""
    pop = ""
    try:
        popd = get("placeOfPerformance") or {}
        city = popd.get("city") or ""; state = popd.get("state") or ""; country = popd.get("country") or ""
    ...
    return {"notice_id": str(notice_id), "sol_number": sol, "title": title, "notice_type": notice_type, "agency": agency,
            "office": office, "naics": naics, "psc": psc, "set_aside": set_aside, "place_of_performance": pop,
            "posted_date": posted_date, "due_date": due_date, "status": status, "sam_url": sam_url, "is_archived": 0}

def ela_sam_pull_new(filters: dict, limit: int = 200) -> dict:
    conn = ela_sam__get_conn_safe(); client = SamClient(); items = client.search(filters) or []
    added = 0; updated = 0
    for row in items[:limit]:
        ...
        try:
            ...
            if prev:
                ...
                if diffs: ela_sam_log_change(conn, rec["notice_id"], "updated", {"diffs": diffs}); updated += 1
            else:
                added += 1
        except Exception:
            pass
...
def ela_sam_render_embedded_panel_with_pull():
    try:
        ...
        if notice_id:
            with st.dialog("RFP Analyzer"):
                ela_sam_ui_render_rfp_modal(notice_id)
                if st.button("Close", key="close_rfp_modal2"): st.session_state["sam_modal_open"] = None
    except Exception as _e:
        try: import streamlit as st; st.warning(f"[SAM Pull UI error] {str(_e)[:300]}")
        except Exception: pass
# End pull-new
...
def ela_sam__notice_dir(notice_id: str) -> str:
    ...
    os.makedirs(base, exist_ok=True); os.makedirs(os.path.join(base, "originals"), exist_ok=True); os.makedirs(os.path.join(base, "parsed"), exist_ok=True)
    return base

def ela_sam_download_all_attachments(notice_id: str) -> dict:
    conn = ela_sam__get_conn_safe(); client = SamClient()
    files = client.list_attachments(notice_id) or []
    base = ela_sam__notice_dir(notice_id); originals = os.path.join(base, "originals")
    downloaded = 0
    from datetime import datetime as _dt
    for f in files:
        ...
        if not url or not fname: continue
        dest = os.path.join(originals, fname); dest = client.download_attachment(url, dest)
        h = ""
        try:
            import hashlib; 
            with open(dest, "rb") as fp: h = hashlib.sha256(fp.read()).hexdigest()
        except Exception: pass
        meta = {"file_name": fname, "file_url": url, "file_type": ftype, "local_path": dest, "hash": h, "downloaded_at": _dt.utcnow().isoformat()}
        ela_sam_upsert_attachments_meta(conn, notice_id, [meta]); downloaded += 1
    return {"count": downloaded, "listed": len(files)}

def ela_sam_parse_local_attachments(notice_id: str) -> dict:
    base = ela_sam__notice_dir(notice_id); originals = os.path.join(base, "originals"); parsed_dir = os.path.join(base, "parsed"); os.makedirs(parsed_dir, exist_ok=True)
    conn = ela_sam__get_conn_safe(); cur = conn.execute("SELECT file_name, local_path, file_type FROM attachments WHERE notice_id=?", (notice_id,))
    rows = cur.fetchall(); parsed = 0
    for fname, local_path, ftype in rows:
        ...
        try:
            if os.path.exists(out_txt) and os.path.getmtime(out_txt) >= os.path.getmtime(local_path): continue
        except Exception: pass
        text = ""
        try:
            ext = (ftype or "").lower()
            if ext == "pdf" or local_path.lower().endswith(".pdf"):
                try:
                    ...
                    with open(local_path, "rb") as fp:
                        reader = PyPDF2.PdfReader(fp); text_chunks = []
                        for page in reader.pages:
                            try: text_chunks.append(page.extract_text() or "")
                            except Exception: text_chunks.append("")
                        text = "\n".join(text_chunks)
                except Exception: text = ""
            elif ext in ("docx","doc") or local_path.lower().endswith(".docx"):
                try:
                    import docx; d = docx.Document(local_path); text = "\n".join(p.text for p in d.paragraphs)
                except Exception: text = ""
            elif ext in ("txt","csv") or local_path.lower().endswith(".txt"):
                try:
                    with open(local_path, "r", encoding="utf-8", errors="ignore") as fp: text = fp.read()
                except Exception: text = ""
            else:
                text = ""
        except Exception: text = ""
        try:
            with open(out_txt, "w", encoding="utf-8", errors="ignore") as w: w.write(text or ""); parsed += 1
        except Exception: pass
    return {"parsed": parsed}

def ela_sam_refresh_summary_from_files(notice_id: str) -> dict:
    conn = ela_sam__get_conn_safe(); base = ela_sam__notice_dir(notice_id); parsed_dir = os.path.join(base, "parsed")
    combined = []
    try:
        import glob
        ...
        for path in txts[:8]:
            try:
                with open(path, "r", encoding="utf-8", errors="ignore") as fp: combined.append(fp.read())
            except Exception: pass
    except Exception: pass
    corpus = "\n\n".join(combined)[:150000]
    cur = conn.execute("SELECT title, agency, naics, set_aside, due_date FROM opportunities WHERE notice_id=?", (notice_id,))
    r = cur.fetchone(); meta = {"title": "", "agency": "", "naics": "", "set_aside": "", "due_date": ""}
    if r: meta = dict(zip(["title","agency","naics","set_aside","due_date"], r))
    import re as _re
    bullets = []
    for line in (corpus or "").splitlines():
        s = line.strip()
        if not s: continue
        if _re.search(r"(shall|must|provide|deliver|submit|include)", s, _re.I):
            bullets.append("- " + s)
    ...
    conn.execute("""
        ...
        ON CONFLICT(notice_id) DO UPDATE SET
            ...
            model_name=excluded.model_name,
            updated_at=CURRENT_TIMESTAMP
    """, (notice_id, overview, key_requirements, "", "", "", "", "", _json.dumps({"source":"local_parse","files_dir": parsed_dir, "chars": len(corpus)}), "local-parse"))
    conn.commit(); return {"overview_len": len(overview), "bullets": len(bullets)}
# End attachments/parsing/summaries
...
def ela_sam_list_saved_searches() -> list:
    ...
    return [{"id": r[0], "name": r[1], "query": _json.loads(r[2] or "{}"), "created_at": r[3]} for r in cur.fetchall()]

def ela_sam_delete_saved_search(ss_id: int) -> None:
    conn = ela_sam__get_conn_safe(); conn.execute("DELETE FROM saved_searches WHERE id=?", (ss_id,)); conn.commit()

def ela_sam_subscribe_alert(user_email: str, frequency: str, query: dict) -> int:
    conn = ela_sam__get_conn_safe()
    cur = conn.execute("INSERT INTO alert_subscriptions (user_email, frequency, query_json, enabled) VALUES (?, ?, ?, 1)", (user_email, frequency, _json.dumps(query)))
    conn.commit(); return cur.lastrowid
...
def ela_sam_toggle_alert(alert_id: int, enabled: bool) -> None:
    conn = ela_sam__get_conn_safe(); conn.execute("UPDATE alert_subscriptions SET enabled=? WHERE id=?", (1 if enabled else 0, alert_id)); conn.commit()
...
def ela_sam_send_email(to_addrs: list, subject: str, html_body: str, text_body: str = None):
    ...
    user = os.environ.get("SMTP_USER"); pwd = os.environ.get("SMTP_PASS"); sender = os.environ.get("SMTP_FROM", user or "alerts@ela.local")
    ...
    try:
        with smtplib.SMTP(host, port) as server:
            server.starttls(); server.login(user, pwd); server.sendmail(sender, to_addrs, msg.as_string())
        return True
    except Exception:
        return False

def ela_sam_refresh_tracked_status() -> dict:
    conn = ela_sam__get_conn_safe(); client = SamClient(); cur = conn.execute("SELECT notice_id FROM pipeline_items"); ids = [r[0] for r in cur.fetchall()]
    changed = 0
    for nid in ids:
        try:
            cur2 = conn.execute("SELECT status, due_date, title FROM opportunities WHERE notice_id=?", (nid,)); old = cur2.fetchone()
            ...
            if det:
                rec = ela_sam_transform_record(det); ela_sam_upsert_opportunity(conn, rec)
                diffs = ela_sam_compute_diff(old_map, {"status": rec.get("status"), "due_date": rec.get("due_date"), "title": rec.get("title")})
                if diffs: ela_sam_log_change(conn, nid, "updated", {"diffs": diffs}); changed += 1
            files = client.list_attachments(nid) or []
            cur3 = conn.execute("SELECT COUNT(1) FROM attachments WHERE notice_id=?", (nid,)); prev_count = cur3.fetchone()[0]
            if len(files) > prev_count: ela_sam_log_change(conn, nid, "attachment_added", {"count": len(files), "prev": prev_count})
        except Exception: pass
    return {"tracked": len(ids), "changed": changed}

def ela_sam_run_alerts_job() -> dict:
    alerts = ela_sam_list_alerts(); total_sent = 0
    for a in alerts:
        if not a["enabled"]: continue
        res = ela_sam_pull_new(a["query"], limit=200)
        if res.get("added") or res.get("updated"):
            rows = ela_sam_fetch_opportunities(a["query"], limit=25); lines = []
            for r in rows:
                line = f"""<p><b>{r.get('title','')}</b><br/>Agency: {r.get('agency','')} • NAICS: {r.get('naics','')} • Due: {r.get('due_date','N/A')}<br/><a href="{r.get('sam_url','')}">SAM link</a></p>"""
                lines.append(line)
            html = f"""<h3>SAM Watch Alert</h3><p><b>Fetched:</b> {res.get('fetched',0)} • <b>Added:</b> {res.get('added',0)} • <b>Updated:</b> {res.get('updated',0)}</p>{''.join(lines)}"""
...
def ela_sam_render_alerts_manager():
    try:
        ...
        with col1:
            ...
            name = st.text_input("Name", key="ss_name"); filters = ela_sam_ui_render_filters()
            if st.button("Save this search"): ss_id = ela_sam_save_search(name or "My search", filters); st.success(f"Saved search #{ss_id}")
            for s in ela_sam_list_saved_searches(): st.caption(f"• {s['name']} – {s['created_at']}")
        with col2:
            ...
            if st.button("Subscribe alert"): aid = ela_sam_subscribe_alert(email, freq, filters2); st.success(f"Alert #{aid} created")
            for a in ela_sam_list_alerts(): st.caption(f"• {a['user_email']} – {a['frequency']} – enabled={a['enabled']}")
        st.divider(); c1, c2, c3 = st.columns(3)
        with c1:
            if st.button("Run alerts now"): res = ela_sam_run_alerts_job(); st.success(f"Ran alerts • {res['sent']} email(s) sent")
        with c2:
            if st.button("Refresh tracked status now"): res = ela_sam_refresh_tracked_status(); st.success(f"Tracked {res['tracked']} • Changed {res['changed']}")
        with c3:
            if st.button("Start background scheduler"): ok = ela_sam_start_scheduler(); st.success("Scheduler started" if ok else "Scheduler not available")
    except Exception as _e:
        try: import streamlit as st; st.warning(f"[Alerts manager error] {str(_e)[:300]}")
        except Exception: pass
# End alerts/watcher
...
def ela_sam_get_company_profile() -> dict:
    conn = ela_sam__get_conn_safe()
    try:
        cur = conn.execute("SELECT json FROM company_profile WHERE key='default'"); row = cur.fetchone()
        if row and row[0]: return _json.loads(row[0])
    except Exception: pass
    import os
    return {"legal_name": os.environ.get("ELA_COMPANY_NAME", "ELA Management LLC"),
...
def ela_sam_compute_bid_readiness(notice_id: str, profile: dict = None) -> dict:
    conn = ela_sam__get_conn_safe(); profile = profile or ela_sam_get_company_profile()
    cur = conn.execute("SELECT title, agency, naics, set_aside, due_date, posted_date FROM opportunities WHERE notice_id=?", (notice_id,))
    row = cur.fetchone()
    if not row: return {"score": 0, "breakdown": [], "notes": "Opportunity not found."}
    title, agency, naics_csv, set_aside, due_date, posted_date = row
    naics_list = [x.strip() for x in (naics_csv or "").split(",") if x.strip()]
    weights = {"NAICS fit":25,"Set-aside eligibility":25,"Time runway":10,"Docs parsed":10,"Past performance":10,"Certifications":10,"Capacity vs value":10}
    cap_naics = [x.strip() for x in (profile.get("capabilities_naics") or [])]
    s1 = (1 if any(n in cap_naics for n in naics_list if n) else 0) * weights["NAICS fit"]
    elig_set = [s.strip().lower() for s in (profile.get("eligible_set_asides") or [])]; op_set = (set_aside or "").strip().lower()
    s2 = (1 if (not op_set or any(e in op_set for e in elig_set) or "small business" in op_set) else 0) * weights["Set-aside eligibility"]
    import datetime as _dt
    runway = 0
    try:
        if due_date:
            d = _dt.datetime.fromisoformat(str(due_date).split(" ")[0]); runway = (d - _dt.datetime.utcnow()).days
    except Exception: runway = 0
    s3 = weights["Time runway"] if runway >= 7 else int(weights["Time runway"]*0.5) if runway >=3 else 0
    base = ela_sam__notice_dir(notice_id); import glob, os
    parsed = len(glob.glob(os.path.join(base, "parsed", "*.txt"))); s4 = weights["Docs parsed"] if parsed >= 1 else 0
    pp = int(profile.get("past_perf_count") or 0); s5 = weights["Past performance"] if pp >= 3 else int(weights["Past performance"]*0.5) if pp in (1,2) else 0
    certs = [c.strip().upper() for c in (profile.get("certifications") or [])]; cert_score = 0
    for tag in ("SDB","WOSB","MBE","8(A)","HUBZONE","VOSB","SDVOSB"):
        if tag in certs: cert_score += 2
    s6 = min(cert_score, 10)
    max_val = float(profile.get("max_contract_value") or 0); s7 = weights["Capacity vs value"] if max_val >= 100000 else int(weights["Capacity vs value"]*0.5) if max_val >= 50000 else 0
    total = s1+s2+s3+s4+s5+s6+s7
    breakdown = [{"factor":"NAICS fit","points":s1,"reason":f"Opportunity NAICS {naics_list} vs capabilities {cap_naics}"},
                 {"factor":"Set-aside eligibility","points":s2,"reason":f"Set-aside '{set_aside}' vs eligibility {elig_set}"},
                 ...
                 {"factor":"Certifications","points":s6,"reason":", ".join(certs) or "none"},
...
def ela_sam_generate_co_outreach_email(notice_id: str, contact_name: str = None) -> dict:
    conn = ela_sam__get_conn_safe(); prof = ela_sam_get_company_profile()
    cur = conn.execute("SELECT title, agency, sol_number, set_aside, naics, due_date, sam_url FROM opportunities WHERE notice_id=?", (notice_id,))
    row = cur.fetchone()
    if not row: return {"subject":"", "body": "Context unavailable."}
    title, agency, sol, set_aside, naics, due, link = row
    ccur = conn.execute("SELECT name, email FROM opportunity_contacts WHERE notice_id=? LIMIT 1", (notice_id,)); c = ccur.fetchone()
...
def ela_sam_render_readiness_widget(notice_id: str):
    try:
        import streamlit as st
        res = ela_sam_compute_bid_readiness(notice_id); st.markdown("### Bid Readiness")
        ...
        with st.expander("Breakdown", expanded=False):
            for b in res.get("breakdown", []): st.write(f"**{b['factor']}**: {b['points']} • {b['reason']}")
    except Exception: pass
# End readiness/outreach
...
def ela_sam_extract_pricing_signals(notice_id: str) -> dict:
    ...
    row = cur.fetchone(); meta = {}
    if row: meta = dict(zip(["title","agency","naics","psc","set_aside","place_of_performance","due_date"], row))
    ...
    try:
        for path in sorted(glob.glob(os.path.join(base, "parsed", "*.txt")))[:8]:
            try:
                with open(path, "r", encoding="utf-8", errors="ignore") as fp: tokens.append(fp.read())
            except Exception: pass
    except Exception: pass
    corpus = "\n".join(tokens)
    def find_flag(p: str) -> bool:
        try: import re; return bool(re.search(p, corpus, re.I))
        except Exception: return False
    labor_flags = {"supervisor": find_flag(r"site\s*supervisor|project\s*manager|supervisory"),
    ...
    try:
        m = __import__("re").search(r"(base|initial)\s*period.*?(\d+)\s*(day|days|month|months|year|years)", corpus, __import__("re").I|__import__("re").S)
        if m: estimated_period = f"{m.group(2)} {m.group(3)}"
    except Exception: pass
    deliverables = []
    ...
    return {"meta": meta, "labor_flags": labor_flags, "estimated_period": estimated_period, "deliverables": deliverables, "suggested_labor": suggested_labor}

def ela_sam_start_pricing_checklist(notice_id: str):
    try:
        import streamlit as st
        ctx = ela_sam_extract_pricing_signals(notice_id); st.session_state["pricing_context"] = ctx
        st.session_state["active_tab"] = "Pricing Calculator"; st.rerun()
    except Exception: pass
# End pricing checklist



# ==============================
# SAM WATCH 10/10: NAICS labor presets
# ==============================
def ela_sam_get_naics_labor_presets() -> dict:
    return {"561720":[{"role":"Project Manager","rate":65},{"role":"Site Supervisor","rate":45},{"role":"Janitor/Custodian","rate":28},{"role":"Quality Inspector","rate":40}],
            "238220":[{"role":"Project Manager","rate":85},{"role":"HVAC Technician (Journeyman)","rate":75},{"role":"HVAC Apprentice/Helper","rate":45},{"role":"QA/QC Inspector","rate":60}],
            "561730":[{"role":"Project Manager","rate":65},{"role":"Crew Lead","rate":42},{"role":"Groundskeeper/Laborer","rate":30},{"role":"Irrigation Tech","rate":50}],
            "722310":[{"role":"Food Service Manager","rate":55},{"role":"Cook","rate":35},{"role":"Food Service Worker","rate":28},{"role":"Sanitation","rate":27}]}

def ela_sam_seed_pricing_by_naics(naics_csv: str, labor_flags: dict) -> list:
    presets = ela_sam_get_naics_labor_presets(); picked = []; naics_list = [x.strip() for x in (naics_csv or "").split(",") if x.strip()]
    base = None
    for n in naics_list:
        if n in presets: base = presets[n]; break
    base = base or presets.get("561720", [])
    def ensure_role(arr, role, rate):
        if not any(r["role"] == role for r in arr): arr.append({"role": role, "rate": rate})
    ...
    if labor_flags.get("qa_qc"): ensure_role(items, "QA/QC Inspector", 55.0)
    if labor_flags.get("safety"): ensure_role(items, "Safety Officer", 58.0)
    if labor_flags.get("supervisor"): ensure_role(items, "Site Supervisor", 45.0)
    return sorted(items, key=lambda x: x["role"])
# End NAICS presets
...
def ela_sam_compute_price_totals(labor_rows: list, overhead_pct=15.0, gna_pct=7.0, contingency_pct=3.0, profit_pct=10.0, tax_pct=0.0) -> dict:
    ...
    for r in labor_rows or []:
        try:
            labor_subtotal += float(r.get("rate",0))*float(r.get("hours",0))*float(r.get("qty",1))
    ...
    return {"labor_subtotal":labor_subtotal,"overhead":overhead,"gna":gna,"contingency":contingency,"profit":profit,"taxes":taxes,"total":taxable+taxes}
# End O/H+Profit
...
def ela_sam_render_overhead_profit_panel():
    try:
        import streamlit as st, pandas as pd
        ctx = st.session_state.get("pricing_context", {}); rows = ctx.get("suggested_labor", [])
        st.markdown("### Overhead • G&A • Profit")
        c1,c2,c3,c4,c5 = st.columns(5)
        with c1: overhead_pct = st.number_input("Overhead %", 0.0, 100.0, 15.0, 0.5, key="ovh_pct")
        with c2: gna_pct = st.number_input("G&A %", 0.0, 100.0, 7.0, 0.5, key="gna_pct")
        with c3: contingency_pct = st.number_input("Contingency %", 0.0, 100.0, 3.0, 0.5, key="cont_pct")
        with c4: profit_pct = st.number_input("Profit %", 0.0, 100.0, 10.0, 0.5, key="profit_pct")
        with c5: tax_pct = st.number_input("Tax %", 0.0, 100.0, 0.0, 0.5, key="tax_pct")
        ...
        totals = ela_sam_compute_price_totals(edited_rows, overhead_pct, gna_pct, contingency_pct, profit_pct, tax_pct)
        m1,m2,m3 = st.columns(3)
        with m1: st.metric("Labor Subtotal", f"${totals['labor_subtotal']:,.2f}"); st.metric("Overhead", f"${totals['overhead']:,.2f}")
        with m2: st.metric("G&A", f"${totals['gna']:,.2f}"); st.metric("Contingency", f"${totals['contingency']:,.2f}")
        with m3: st.metric("Profit", f"${totals['profit']:,.2f}"); st.metric("Taxes", f"${totals['taxes']:,.2f}")
        st.header(f"Total: ${totals['total']:,.2f}")
        st.session_state["pricing_context"] = {**ctx, "suggested_labor": edited_rows, "markups": {"overhead_pct":overhead_pct,"gna_pct":gna_pct,"contingency_pct":contingency_pct,"profit_pct":profit_pct,"tax_pct":tax_pct}, "totals": totals}
    except Exception: pass

def ela_sam_ui_render_rfp_modal(notice_id: str):
    try:
        import streamlit as st
        conn = ela_sam__get_conn_safe(); ctx = ela_sam_get_proposal_context(conn, notice_id)
        opp = (ctx or {}).get("opportunity", {}); summ = (ctx or {}).get("summary", {})
        ...
        with st.container(border=True):
            colA, colB, colC, colD = st.columns(4)
            with colA:
                if st.button("Fetch docs"): res = ela_sam_download_all_attachments(notice_id); st.success(f"Docs: listed {res['listed']} • downloaded {res['count']}")
            with colB:
                if st.button("Parse docs"): res = ela_sam_parse_local_attachments(notice_id); st.success(f"Parsed {res['parsed']} file(s).")
            with colC:
                if st.button("Summarize now"): res = ela_sam_refresh_summary_from_files(notice_id); st.success(f"Summary updated • {res['bullets']} bullets.")
            with colD:
                if st.button("Start Pricing Checklist"): 
                    try:
                        import streamlit as _st; _st.session_state["pricing_context"] = ela_sam_extract_pricing_signals(notice_id)
                        _st.session_state["active_tab"] = "Pricing Calculator"; _st.rerun()
                    except Exception: pass
        ctx = ela_sam_get_proposal_context(conn, notice_id); summ = (ctx or {}).get("summary", {})
        top = st.container()
        with top:
            left, right = st.columns([2,1])
            with left:
                st.write("**AI Summary**"); st.write(summ.get("overview") or "Run 'Fetch docs' → 'Parse docs' → 'Summarize now'.")
                st.write("**Key Requirements**"); st.write((summ.get("key_requirements") or "").replace("\n", "\n"))
            with right:
                ela_sam_render_readiness_widget(notice_id)
        qs = st.text_input("Ask RFP Analyzer about this job")
        if qs: st.info("Q&A stub: connect to your RFP chat using the parsed corpus.")
        ela_sam_render_outreach_draft(notice_id)
        col1, col2 = st.columns(2)
        with col1:
            if st.button("Open full RFP Analyzer"): st.session_state["active_tab"] = "RFP Analyzer"; st.rerun()
        with col2:
            if st.button("Start Proposal"): st.session_state["proposal_context"] = ctx; st.session_state["active_tab"] = "Proposal Builder"; st.rerun()
    except Exception as _e:
        try: import streamlit as st; st.warning(f"[RFP modal error] {str(_e)[:300]}")
        except Exception: pass
# End modal
