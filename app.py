...
# --- Compat shim: Streamlit expander doesn't support `key`; drop it if passed ---
try:
    import inspect as _insp
    _sig = _insp.signature(st.expander)
    if 'key' not in _sig.parameters:
        _orig_expander = st.expander
        def _expander_no_key(*args, **kwargs):
            kwargs.pop('key', None)
            return _orig_expander(*args, **kwargs)
        st.expander = _expander_no_key  # type: ignore
except Exception:
    pass
# --- end compat shim ---
