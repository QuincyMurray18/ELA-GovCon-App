...
with tabs[0]:
    ...
    if st.session_state.get('sam_do_search'):
            ...
            if open_browse:
                kword = "*"
            sam_df, info = sam_search(naics_for_query, min_days=int(min_days), limit=400,
            ...
            if not info.get("ok"):
                ...
                if manual_naics.strip() and "naics" in sam_df.columns:
                    keep = set(a.strip() for a in manual_naics.split(",") if a.strip())
                    sam_df = sam_df[sam_df["naics"].astype(str).isin(keep)]
                if picked_notice and "type" in sam_df.columns:
                    sam_df = sam_df[sam_df["type"].astype(str).isin(set(picked_notice))]
                if "response_due" in sam_df.columns:
                    try:
                        due = pd.to_datetime(sam_df["response_due"], errors="coerce")
                        now = pd.Timestamp.utcnow().normalize()
                        sam_df = sam_df[(due.isna()) | (due >= now + pd.Timedelta(days=int(min_days)))]
                    except Exception:
                        pass
                if "posted" in sam_df.columns and int(posted_from_days) > 0:
                    try:
                        posted = pd.to_datetime(sam_df["posted"], errors="coerce")
                        cutoff = pd.Timestamp.utcnow().normalize() - pd.Timedelta(days=int(posted_from_days))
                        sam_df = sam_df[(posted.isna()) | (posted >= cutoff)]
                    except Exception:
                        pass
                if "attachments_json" in sam_df.columns:
                    import json as _json
                    try:
                        sam_df["attachments_count"] = sam_df["attachments_json"].apply(lambda s: len(_json.loads(s)) if isinstance(s, str) and s.strip() else 0)
                    except Exception:
                        sam_df["attachments_count"] = 0
                st.session_state["sam_results_df"] = sam_df.reset_index(drop=True)
    # --- Render SAM results (persisted) ---
    sam_df = st.session_state.get("sam_results_df")
    if isinstance(sam_df, pd.DataFrame) and not sam_df.empty:
        show_cols = ["sam_notice_id","title","agency","naics","psc","place_of_performance","posted","response_due","type","attachments_count","url"]
        show_cols = [c for c in show_cols if c in sam_df.columns]
        c1, c2 = st.columns([3,1])
        with c1:
            if IN_SAM_TAB:
                grid = st.data_editor(
                    sam_df[["select"] + show_cols],
                    use_container_width=True,
                    num_rows="fixed",
                    key="sam_grid"
                )
                sel_mask = grid["select"] if "select" in grid.columns else pd.Series([False]*len(sam_df))
                sel_df = sam_df[sel_mask].copy()
            else:
                sel_df = None
        with c2:
            if st.button("Save selected to pipeline", key="sam_save_selected"):
            ...
            if st.button("Clear results", key="sam_clear_results"):
                st.session_state["sam_results_df"] = None
        st.markdown("**View attachments for a notice**")
        pick_id = st.text_input("Enter SAM Notice ID", value="", key="sam_pick_id")
        if pick_id and "sam_notice_id" in sam_df.columns:
            row = sam_df[sam_df["sam_notice_id"].astype(str) == str(pick_id)].head(1)
            if row.empty:
                st.caption("No matching notice in current results.")
            else:
                import json as _json
                atts = []
                try:
...
with tabs[4]:
    ...
    # (removed duplicate tabs block for idx)
