# ===== app.py =====
# GovCon Copilot Pro — with safe key loading, GPT-5 model fallbacks, saved NAICS, editable goals, settings, templates
...
# ---------- Safe key loader: env first, then st.secrets if available ----------
...
# ---------- Model selection (uses your available GPT-5 models) ----------
...
_OPENAI_FALLBACK_MODELS = [
    ...
    # Broad fallbacks in case account changes:
    "gpt-4o-mini",
    "gpt-4o",
]
...
def llm(system, prompt, temp=0.2, max_tokens=1400):
    """Model-aware LLM call with fallbacks to your enabled GPT-5 models."""
    if not client:
    ...
    return (f"LLM error ({err_name}). "
            "Tip: set OPENAI_MODEL to a model you have, e.g. 'gpt-5-chat-latest'.")
