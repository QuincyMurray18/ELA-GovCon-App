...
def crawl_site_for_emails(base_url: str, max_pages: int = 5):
    """
    Best-effort crawl: checks high-signal paths first, then follows internal links up to max_pages.
            ...
            try:
                ...
                for a in soup.select('a[href^="mailto:"]'):
                    ...
                    if ":" in href:
                        _, addr = href.split(":",1)
                        addr = addr.split("?")[0]
                        emails |= _extract_emails_from_text(addr)
                # queue more internal links (limited breadth)
                for a in soup.find_all("a", href=True):
                    ...
                    if urlparse(nxt).netloc == parsed.netloc and nxt not in seen and len(seen) + len(queue) < 100:
                        if any(seg in nxt.lower() for seg in ("/contact","/about","/team","/lead","/staff","/people","/privacy","/press","/sitemap")):
                            queue.insert(0, nxt)  # prioritize
                        else:
                            queue.append(nxt)
            except Exception:
                continue
...
with tabs[2]:
    ...
    st.caption("Tip: click 'Verify emails' to mark addresses as valid/invalid/unknown.")grid = st.data_editor(
        df_v,
        use_container_width=True,
        num_rows="dynamic",
        key="vendor_grid",
...
if
# (SAM search button removed here)
:
...
with tabs[7]:
    ...
    core_comp = st.text_area("Core Competencies (bullet list)", height=160, value=get_setting("cap_core","• Facility maintenance\n• Janitorial services\n• Grounds & snow"))
    diffs = st.text_area("Differentiators (bullet list)", height=160, value=get_setting("cap_diff","• Rapid mobilization\n• Cleared staff\n• Strong past performance"))
    past_perf = st.text_area("Past Performance (short blurbs)", height=160, value=get_setting("cap_past","• Agency X — $1.2M, CPARS Very Good"))
    naics_list = st.text_input("NAICS (comma-separated)", value=get_setting("cap_naics","561720, 561730, 561210"))
    soc_codes = st.text_input("PSC (comma-separated)", value=get_setting("cap_psc","S201, S208"))

    if st.button("Export 1-page PDF"):
        # Minimal PDF via reportlab fallback (if available), else create a DOCX
        try:
            ...
            w, h = LETTER
            y = h - 1*inch
            c.setFont("Helvetica-Bold", 14); c.drawString(1*inch, y, company_name); y -= 20
            c.setFont("Helvetica", 10); c.drawString(1*inch, y, f"CAGE {cage}  •  UEI {uei}  •  DUNS {duns}  •  {email}"); y -= 30
            for title, txt in [("Core Competencies", core_comp), ("Differentiators", diffs), ("Past Performance", past_perf), ("Codes", f"NAICS: {naics_list} | PSC: {soc_codes}") ]:
                c.setFont("Helvetica-Bold", 12); c.drawString(1*inch, y, title); y -= 16
                c.setFont("Helvetica", 10)
                for line in txt.splitlines():
                    c.drawString(1.1*inch, y, line); y -= 14
                    if y < 1*inch: c.showPage(); y = h - 1*inch
                y -= 10
            c.save()
            st.success("Exported PDF")
            st.download_button("Download Capability Statement (PDF)", data=open(out,"rb").read(), file_name="Capability_Statement.pdf")
        except Exception:
            import docx as docxlib
            out = "/mnt/data/capability_statement.docx"
            d = docxlib.Document()
            d.add_heading(company_name, 0)
            d.add_paragraph(f"CAGE {cage}  •  UEI {uei}  •  DUNS {duns}  •  {email}")
            for title, txt in [("Core Competencies", core_comp), ("Differentiators", diffs), ("Past Performance", past_perf), ("Codes", f"NAICS: {naics_list} | PSC: {soc_codes}") ]:
                d.add_heading(title, level=2)
                for line in txt.splitlines():
                    d.add_paragraph(line, style="List Bullet")
            d.save(out)
            st.success("Exported DOCX")
            st.download_button("Download Capability Statement (DOCX)", data=open(out,"rb").read(), file_name="Capability_Statement.docx")
...
with tabs[8]:
    ...
    aud = st.text_input("Audience", value=get_setting("wp_aud","Federal facility managers"))
    abstract = st.text_area("Abstract", height=160, value=get_setting("wp_abs","This paper outlines..."))
    sections = st.text_area("Sections (Markdown)", height=240, value=get_setting("wp_sections","# Introduction\n...\n# Conclusion\n..."))
    ...
    if st.button("Export DOCX"):
        ...
        d.add_paragraph(f"Audience: {aud}")
...
with tabs[10]:
    ...
    pick = st.multiselect("Pick tables to export", options=list(tables.keys()), default=["opportunities","vendors"])
    if st.button("Export selected to Excel"):
        ...
        if not data:
            ...
            st.download_button("Download Excel", data=x, file_name="ela_export.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    st.caption("This tab is for exporting data only.")

with tabs[9]:
    st.subheader('Auto extract key details')
    st.caption("Upload RFP files (PDF/DOCX/TXT) and extract key fields")
    ...
    if files:
        ...
        for f in files:
            ...
            rows.append({"file": f.name, "chars": len(text), "preview": text[:1000]})
        df = pd.DataFrame(rows)
        st.dataframe(df, use_container_width=True)
        st.download_button("Download CSV", data=df.to_csv(index=False).encode("utf-8"), file_name="auto_extract.csv")
    else:
