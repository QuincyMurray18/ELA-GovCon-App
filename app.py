...
try:
    with legacy_tabs[12]:
        st.subheader("Proposal Builder")
        st.caption("Write in Markdown. Save versions, switch drafts, assemble, and export to DOCX.")
        ...
        # Load content for selected draft into editor once when selection changes
        ...
        # Assemble multiple drafts into one DOCX
        st.divider()
        ...
        if st.button("Assemble full proposal (DOCX)"):
            ...
            if include_current and st.session_state.get("proposal_md"):
                parts_md.append("# " + (title or "Proposal") + "\n\n" + st.session_state.get("proposal_md",""))
            ...
            if _docx:
                st.download_button(
                    ...
                    data=_docx,
                    file_name=(title or "Proposal").strip().replace(" ","_") + "_assembled.docx",
                    mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document"
                )

        # Single-document export of current editor
