...
def save_opportunities(df):
    """Insert new opportunities into the pipeline. Assumes SQLite schema with 'opportunities' table.
    Columns inserted: sam_notice_id,title,agency,naics,psc,place_of_performance,response_due,posted,type,url,attachments_json,status
    """
    if df is None or getattr(df, "empty", True):
        return 0, 0
    # Make SQLite-safe (no NaN/np types)
    try:
    ...
    for _, r in df.iterrows():
        nid = r.get("sam_notice_id") if isinstance(r, dict) else getattr(r, "sam_notice_id", None)
        if not nid:
            continue
        ...
        if row:
            cur.execute(
                ...
                (r.get("title") if isinstance(r, dict) else getattr(r, "title", None),
                 r.get("agency") if isinstance(r, dict) else getattr(r, "agency", None),
                 r.get("naics") if isinstance(r, dict) else getattr(r, "naics", None),
                 r.get("psc") if isinstance(r, dict) else getattr(r, "psc", None),
                 r.get("place_of_performance") if isinstance(r, dict) else getattr(r, "place_of_performance", None),
                 r.get("response_due") if isinstance(r, dict) else getattr(r, "response_due", None),
                 r.get("posted") if isinstance(r, dict) else getattr(r, "posted", None),
                 r.get("type") if isinstance(r, dict) else getattr(r, "type", None),
                 r.get("url") if isinstance(r, dict) else getattr(r, "url", None),
                 r.get("attachments_json") if isinstance(r, dict) else getattr(r, "attachments_json", None),
                 nid)
            )
            updated += 1
        else:
            cur.execute("""insert into opportunities
                ...
                values(?,?,?,?,?,?,?,?,?,?,?,?)""",
                (
                    r.get("sam_notice_id") if isinstance(r, dict) else getattr(r, "sam_notice_id", None),
                    r.get("title") if isinstance(r, dict) else getattr(r, "title", None),
                    r.get("agency") if isinstance(r, dict) else getattr(r, "agency", None),
                    r.get("naics") if isinstance(r, dict) else getattr(r, "naics", None),
                    r.get("psc") if isinstance(r, dict) else getattr(r, "psc", None),
    ...
    st.divider()
    st.markdown("### RFP Analyzer Chat")

    sid = st.text_input("Session name", value=st.session_state.get("rfp_active_session","default"), key="rfp_session_name")
    st.session_state["rfp_active_session"] = sid or "default"
    sessions = st.session_state.setdefault("rfp_sessions", {})
    sess = sessions.setdefault(st.session_state["rfp_active_session"], {"messages": [], "context": ""})

    try:
        if up:
            ctx_text = "\n\n".join([read_doc(f) for f in up])[:200000]
            if ctx_text and (not sess.get("context") or len(ctx_text) > len(sess.get("context",""))):
                sess["context"] = ctx_text
    except Exception as e:
        st.caption(f"Could not cache RFP content: {e}")

    qa1, qa2, qa3, qa4 = st.columns(4)
    with qa1:
        if st.button("Compliance matrix", key="rfp_btn_compliance"):
            sess["messages"].append({"role":"user","content":"Create a compliance matrix table with columns: Requirement, Section ID, Text, Deliverable, Due Date, Owner."})
    with qa2:
        if st.button("Evaluation factors", key="rfp_btn_evals"):
            sess["messages"].append({"role":"user","content":"Summarize the evaluation factors and their relative weights if stated."})
    with qa3:
        if st.button("Submission checklist", key="rfp_btn_checklist"):
            sess["messages"].append({"role":"user","content":"Produce a submission checklist covering forms, volumes, file naming, page limits, and submission method."})
    with qa4:
        if st.button("Grade my draft", key="rfp_btn_grade"):
            sess["messages"].append({"role":"user","content":"Grade my draft and suggest targeted improvements by section with concrete fixes."})

    for m_ in sess["messages"]:
        with st.chat_message(m_["role"]):
            st.markdown(m_["content"])

    user_msg = st.chat_input("Ask about this RFP…")
    if user_msg:
        sess["messages"].append({"role":"user","content":user_msg})

    if sess["messages"] and sess["messages"][-1]["role"] == "user":
        sys_intro = "You are a federal proposal analyst. Be concise and actionable."
        ctx = sess.get("context","")
        if ctx:
            sys_intro += "\n\nRFP CONTEXT (may be truncated):\n" + ctx[:120000]
        messages = [{"role":"system","content":sys_intro}] + sess["messages"]
        reply = llm_messages(messages, max_tokens=1200)
