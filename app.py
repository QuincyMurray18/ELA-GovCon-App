...
if isinstance(sam_df, pd.DataFrame) and not sam_df.empty:
    ...
    if "select" not in sam_df.columns:
        sam_df.insert(0, "select", False)
    grid = st.data_editor(sam_df[["select"] + show_cols], use_container_width=True,
                          key="sam_results_grid",
                          column_config={"url": st.column_config.LinkColumn("Link", display_text="open")})
    sel_df = grid[grid["select"] == True] if "select" in grid.columns else sam_df.iloc[0:0]
...
# (removed duplicate block for tabs[7] 'Capability statement builder')
# (removed duplicate block for tabs[8] 'White paper builder')
# (removed duplicate block for tabs[10] 'Data Export')
with tabs[10]:
