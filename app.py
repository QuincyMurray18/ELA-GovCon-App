...
with tabs[0]:
    ...
    st.session_state.setdefault('sam_do_search', False)
    do_search = st.session_state.get('sam_do_search', False)
    do_search = False
    if st.session_state.get('sam_do_search'):
# Safe defaults for search vars

        manual_naics = locals().get('manual_naics', st.session_state.get('sam_manual_naics',''))

        keyword = locals().get('keyword', st.session_state.get('sam_keyword',''))

        search_all = locals().get('search_all', st.session_state.get('sam_all_active', True))

        open_browse = (not manual_naics.strip()) and (not keyword.strip()) and search_all
        kword = (keyword or "").strip()
        if open_browse:
            kword = "*"
        # --- Build safe inputs for sam_search ---
        manual_naics = locals().get('manual_naics', st.session_state.get('sam_manual_naics',''))
        keyword = locals().get('keyword', st.session_state.get('sam_keyword',''))
        search_all = locals().get('search_all', st.session_state.get('sam_all_active', True))
        # load watch_codes if not present
        try:
            conn = get_db()
        ...
        if isinstance(manual_naics, str) and manual_naics.strip():
            ...
            naics_for_query = []
        # other safe inputs
        picked_notice = locals().get('picked_notice', st.session_state.get('sam_notice_types', [
            "Combined Synopsis/Solicitation","Solicitation","Presolicitation","SRCSGT"]))
        min_days = int(locals().get('min_days', st.session_state.get('sam_min_days', 3)))
        posted_from_days = int(locals().get('posted_from_days', st.session_state.get('sam_posted_from_days', 30)))
        open_browse = (not manual_naics.strip()) and (not keyword.strip()) and search_all
...
if isinstance(df, pd.DataFrame) and not df.empty:
    ...
    if st.session_state.get('sam_do_search'):
        pass  # no-op guard
    # Bottom-placed SAM search button (guarded)

    if SAM_WATCH_ACTIVE:
        if st.button('Search SAM now', key='sam_search_btn_bottom'):
                st.session_state['sam_do_search'] = True
                st.rerun()
    SAM_WATCH_ACTIVE = False
...
with tabs[7]:
    # <<< inserted_capability_statement_builder_full_ui >>>
    st.subheader("Capability statement builder")
...
with tabs[8]:
    # <<< inserted_white_paper_builder_full_ui >>>
    st.subheader("White paper builder")
...
with tabs[9]:
    # <<< inserted_auto_extract_full_ui >>>
    st.subheader("Auto extract")
...
with tabs[11]:
    # <<< inserted_chat_assistant_full_ui >>>
