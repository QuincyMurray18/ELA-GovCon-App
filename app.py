...
def save_opportunities(df, default_assignee=None):
    """
    Upsert SAM search results into the opportunities table by sam_notice_id.
    Safely handles empty DataFrames and NaN values.
    """
    ...
    for _, r in df.iterrows():
        notice_id = r.get("sam_notice_id")
        if not notice_id:
            continue
        cur.execute("select id from opportunities where sam_notice_id=?", (notice_id,))
        row = cur.fetchone()
        fields = {
            "title": r.get("title"),
            "agency": r.get("agency"),
        ...
        if row:
            ...
            cur.execute(
                ...
                (fields["title"], fields["agency"], fields["naics"], fields["psc"], fields["place_of_performance"],
                 fields["response_due"], fields["posted"], fields["type"], fields["url"], fields["attachments_json"], notice_id)
            )
            updated += 1
        else:
            # Insert new
            cur.execute(
                """insert into opportunities
                   (sam_notice_id, title, agency, naics, psc, place_of_performance,
                    response_due, posted, type, url, attachments_json, status, assignee, quick_note)
                   values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (notice_id, fields["title"], fields["agency"], fields["naics"], fields["psc"],
                 fields["place_of_performance"], fields["response_due"], fields["posted"], fields["type"],
                 fields["url"], fields["attachments_json"], "New", default_assignee or "", "")
            )
            inserted += 1
    conn.commit()
...
with st.expander("How to use"):
    st.markdown("""- Filter by assignee to see your queue
- Set status to New, Reviewing, Bidding, or Submitted
- Check due dates and add quick notes or next actions
- Use the Link column to reopen the SAM notice""")
    conn = get_db()
...
f1, f2 = st.columns(2)
with f1:
    a_filter = st.selectbox("Filter by assignee", assignees, index=0)
with f2:
    ...
    if df_opp.empty:
        ...
        edit = st.data_editor(
            ...
            column_config={
                ...
                "assignee": st.column_config.SelectboxColumn("assignee", options=["","Quincy","Charles","Collin"]),
        ...
        if st.button("Save pipeline changes"):
            ...
            for _, r in edit.iterrows():
                cur.execute("update opportunities set status=?, response_due=?, title=?, agency=?, assignee=?, quick_note=? where id=?",
                            (r.get("status","New"), r.get("response_due"), r.get("title"), r.get("agency"), r.get("assignee",""), r.get("quick_note",""), int(r["id"])))
...
with tabs[1]:
    ...
    with colA:
        ...
        if results:
            ...
            if name_filter:
                df_new = df_new[df_new.get("name","").str.contains(name_filter, case=False, na=False)]
...
with tabs[4]:
    ...
    with col2:
        ...
        notice_types = st.multiselect("Notice types", options=["Combined Synopsis/Solicitation","Solicitation","Presolicitation","SRCSGT"], default=["Combined Synopsis/Solicitation","Solicitation","Presolicitation","SRCSGT"])
    with col3:
        diag = st.checkbox("Show diagnostics", value=False)
        raw = st.checkbox("Show raw API text (debug)", value=False)
        assignee_default = st.selectbox("Default assignee", ["","Quincy","Charles","Collin"], index=0)
    ...
    with cA:
        if st.button("Run search now"):
            df, info = sam_search(
                ...
                notice_types=",".join(notice_types), active=("true" if active_only else "")
            )
    ...
    if info and not info.get("ok", True):
        ...
        st.dataframe(df.assign(Link=df["url"]), use_container_width=True)
        if st.button("Save to pipeline"):
            ins, upd = save_opportunities(df, default_assignee=assignee_default)
    ...
    with cB:
        if st.button("Broad test (keyword only)"):
            ...
            df, info = sam_search([], min_days=0, limit=100, keyword=kw, posted_from_days=60, notice_types="", active=("true" if active_only else ""))
            st.session_state["sam_results_df"] = df
            st.session_state["sam_results_info"] = info
