...
with tabs[2]:
    ...
    sort_by = st.selectbox("Sort by", options=["distance_km","company","website","phone"], index=0)
    ...
    with colA:
        if st.button("Google Places import"):
            ...
            if not info.get("ok"):
                st.warning(f"Places: {info}")
            df_new = pd.DataFrame(results) if results else pd.DataFrame(columns=["company","website","phone","distance_km","city","state","notes"])
            if name_filter:
            ...
            if not df_new.empty:
                df_new["distance_miles"] = (df_new.get("distance_km", 0) * 0.621371).round(2)
st.dataframe(df_new, use_container_width=True, column_config={"website": st.column_config.LinkColumn("Website", display_text="open"), "distance_miles": st.column_config.NumberColumn("Distance (miles)")})
                st.success(f"Loaded {len(df_new)} vendors from Places")
            else:
                st.info("No vendors found with current filters.")
                ...
                for _, r in df_new.fillna("").iterrows():
                    try:
                        if find_emails and r.get("website"):
                            ...
                            email = r.get("email","") or (emails[0] if emails else "")
                            others = [e for e in emails if e != email]
                            notes = (r.get("notes","") + (f" | other emails: {', '.join(others[:5])}" if others else "")).strip()
                        else:
                            email = r.get("email",""); notes = r.get("notes","")
                        conn.execute("""insert or ignore into vendors(company,naics,trades,phone,email,website,city,state,certifications,set_asides,notes,source)
                                        values(?,?,?,?,?,?,?,?,?,?,?,?)""",
                                     (r.get("company"), "", trade, r.get("phone",""), email, r.get("website",""),
                                      r.get("city",""), r.get("state",""), "", "", notes, "GooglePlaces"))
                        saved += 1
                    except Exception:
                        skipped += 1
                conn.commit()
    ...
    grid = st.data_editor(df_v, use_container_width=True, num_rows="dynamic", key="vendor_grid", column_config={"website": st.column_config.LinkColumn("Website", display_text="open")})
...
with tabs[0]:
    ...
    if st.button("Search SAM now"):
        # Allow open browsing when no NAICS/keyword is provided
        open_browse = (not manual_naics.strip()) and (not keyword.strip()) and search_all
        kword = (keyword or "").strip()
        if open_browse:
            kword = "*"
        df, info = sam_search(naics_for_query, min_days=int(min_days)
        # If SAM API requires a filter and returned empty_filters, retry with a wildcard
...
, limit=400,
        ...
        if not info.get("ok"):
            ...
            if auto_assignee and "sam_notice_id" in df.columns:
                ...
                try:
                    conn.execute("alter table opportunities add column assignee text")
                except Exception:
                    pass
                ids = list(dict.fromkeys(df["sam_notice_id"].dropna().astype(str).tolist()))  # de-dupe
                if ids:
                    try:
                        ...
                        for i in range(0, len(ids), B):
                            ...
                            conn.execute(f"update opportunities set assignee=? where sam_notice_id in ({qmarks})", tuple([auto_assignee] + chunk))
                        conn.commit()
                    except Exception:
                        # Fallback: per-row updates
                        for sid in ids:
                            try:
                                conn.execute("update opportunities set assignee=? where sam_notice_id=?", (auto_assignee, sid))
                            except Exception:
                                pass
                        conn.commit()
