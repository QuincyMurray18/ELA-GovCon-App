...
with tabs[7]:
    ...
    # --- guards for required helpers/vars ---
    if 'get_setting' not in globals():
        def get_setting(k, default=''):
            return default
    COMPANY_CAGE = globals().get('COMPANY_CAGE','14ZP6')
    COMPANY_UEI  = globals().get('COMPANY_UEI','U32LBVK3DDF7')
    COMPANY_DUNS = globals().get('COMPANY_DUNS','14-483-4790')
    COMPANY_EMAIL= globals().get('COMPANY_EMAIL','elamgmtllc@gmail.com')
        ...
        if st.button("Export PDF", key="cap_export_pdf"):
            try:
                ...
                def _draw_block(c, title, txt, y):
                    ...
                    y -= 6
                    return y
                y = _draw_block(c, "Core Competencies", core_comp, y)
                y = _draw_block(c, "Differentiators", diffs, y)
                y = _draw_block(c, "Past Performance", past_perf, y)
                y = _draw_block(c, "Codes", f"NAICS: {naics_list} | PSC: {psc_list}", y)
                c.save()
...
with tabs[8]:
    ...
    if st.button("Export DOCX", key="wp_export_docx"):
        try:
            import docx as docxlib
            ...
            for line in (sections or "").splitlines():
                if line.startswith("## "):
...
with tabs[10]:
    ...
    def to_xlsx_bytes(dfs: dict) -> bytes:
        ...
        return buf.getvalue()
    if st.button("Export selected to Excel", key="data_export_excel"):
        if conn is None:
            st.warning("Database is not available in this context.")
        else:
...
# --- SMOKE TEST MARKERS (do not remove until verified) ---
# These ensure the tabs show some content regardless of later logic.
with tabs[3]:
    st.markdown("### ✅ Capability Statement — page loaded")
    st.subheader("Capability statement builder")

with tabs[4]:
    st.markdown("### ✅ White Paper Builder — page loaded")
    st.subheader("White paper builder")

with tabs[5]:
    # <<< inserted_margin_guidance >>>
    st.markdown("### Margin guidance & targets")
    colA, colB, colC, colD = st.columns(4)
    with colA:
        mg_job = st.selectbox("Job type", ["Facilities O&M","Janitorial","Grounds","Construction","IT Services","Security","Other"], key="mg_jobtype")
    with colB:
        mg_comp = st.selectbox("Competition", ["Sole Source","Low","Medium","High"], key="mg_competition")
    with colC:
        mg_contract = st.selectbox("Contract type", ["FFP","T&M","CPFF"], key="mg_ctype")
    with colD:
        mg_size = st.selectbox("Size", ["<$100k","$100k–$1M",">$1M"], key="mg_size")
    def _guidance_base(job):
        job = job.lower()
        g = {
            "Labor Margin %": (10, 25),
            "Materials Margin %": (5, 15),
            "Subcontractor Margin %": (5, 12),
            "Equipment Margin %": (8, 15),
            "Travel/ODC Margin %": (0, 8),
            "Contingency %": (0, 5),
            "Overhead (OH) %": (20, 40),
            "G&A %": (8, 18),
            "Profit/Fee %": (6, 12)
        }
        if "janitorial" in job:
            g["Labor Margin %"] = (8, 18); g["Materials Margin %"] = (3, 10); g["Overhead (OH) %"] = (15, 30); g["Profit/Fee %"] = (6, 10)
        elif "grounds" in job:
            g["Labor Margin %"] = (10, 20); g["Equipment Margin %"] = (10, 18); g["Profit/Fee %"] = (6, 12)
        elif "construction" in job:
            g["Labor Margin %"] = (12, 25); g["Materials Margin %"] = (6, 15); g["Subcontractor Margin %"] = (5, 10); g["Contingency %"] = (3, 10); g["Profit/Fee %"] = (5, 10)
        elif "it services" in job:
            g["Labor Margin %"] = (12, 28); g["Overhead (OH) %"] = (25, 45); g["G&A %"] = (10, 20); g["Profit/Fee %"] = (8, 14)
        elif "security" in job:
            g["Labor Margin %"] = (10, 18); g["Overhead (OH) %"] = (18, 35); g["Profit/Fee %"] = (6, 10)
        return g
    def _adjust_for_competition(g, comp):
        comp = comp.lower(); mult = 0.0
        if "high" in comp: mult = -0.2
        elif "low" in comp: mult = 0.1
        elif "sole" in comp: mult = 0.2
        new = {}
        for k,(lo,hi) in g.items():
            span = hi - lo; shift = span * mult
            new[k] = (max(0, round(lo+shift)), max(0, round(hi+shift)))
        return new
    def _adjust_for_contract(g, ctype):
        c = ctype.lower(); new = dict(g)
        if "cpff" in c:
            lo,hi = new["Profit/Fee %"]; new["Profit/Fee %"] = (max(3, lo-2), max(6, hi-3))
            lo,hi = new["Overhead (OH) %"]; new["Overhead (OH) %"] = (lo+2, hi+4)
            lo,hi = new["G&A %"]; new["G&A %"] = (lo+1, hi+3)
        elif "t&m" in c:
            lo,hi = new["Labor Margin %"]; new["Labor Margin %"] = (lo+2, hi+2)
        return new
    def _adjust_for_size(g, size):
        s = size.replace("–","-"); new = dict(g)
        if s.startswith("<$100k"):
            for k,(lo,hi) in new.items(): new[k] = (lo+2, hi+2)
        elif s.startswith(">$1M"):
            for k,(lo,hi) in new.items(): new[k] = (max(0, lo-2), max(0, hi-2))
        return new
    base = _guidance_base(mg_job)
    adj1 = _adjust_for_competition(base, mg_comp)
    adj2 = _adjust_for_contract(adj1, mg_contract)
    guide = _adjust_for_size(adj2, mg_size)
    import pandas as _pd
    rows = []
    for k,(lo,hi) in guide.items():
        mid = round((lo+hi)/2)
        rows.append({"Field": k, "Low %": lo, "Mid %": mid, "High %": hi, "Target %": mid})
    mg_df = _pd.DataFrame(rows)
    st.caption("Suggested ranges auto-tune to job type, competition, contract type, and size. Edit **Target %** for your targets.")
    mg_df = st.data_editor(mg_df, key="margin_guidance_table", use_container_width=True, num_rows="fixed",
                           column_config={"Field": st.column_config.TextColumn(disabled=True)})
    with st.expander("Quick margin calculator (optional)"):
        col1, col2, col3 = st.columns(3)
        with col1:
            base_labor = st.number_input("Base Labor ($)", min_value=0.0, value=0.0, step=100.0, key="mg_base_labor")
            base_materials = st.number_input("Base Materials ($)", min_value=0.0, value=0.0, step=100.0, key="mg_base_mat")
            base_subs = st.number_input("Base Subcontractors ($)", min_value=0.0, value=0.0, step=100.0, key="mg_base_subs")
        with col2:
            base_equip = st.number_input("Base Equipment ($)", min_value=0.0, value=0.0, step=100.0, key="mg_base_equip")
            base_travel = st.number_input("Base Travel/ODC ($)", min_value=0.0, value=0.0, step=100.0, key="mg_base_travel")
            contingency_base = st.number_input("Contingency Basis ($)", min_value=0.0, value=0.0, step=100.0, key="mg_base_cont")
        with col3:
            oh_base = st.number_input("OH Basis ($)", min_value=0.0, value=0.0, step=100.0, key="mg_base_oh")
            ga_base = st.number_input("G&A Basis ($)", min_value=0.0, value=0.0, step=100.0, key="mg_base_ga")
            fee_base = st.number_input("Profit/Fee Basis ($)", min_value=0.0, value=0.0, step=100.0, key="mg_base_fee")
        def pct(name):
            row = mg_df.loc[mg_df["Field"] == name]
            return float(row["Target %"].iloc[0])/100.0 if not row.empty else 0.0
        totals = {
            "Labor": base_labor * (1 + pct("Labor Margin %")),
            "Materials": base_materials * (1 + pct("Materials Margin %")),
            "Subcontractors": base_subs * (1 + pct("Subcontractor Margin %")),
            "Equipment": base_equip * (1 + pct("Equipment Margin %")),
            "Travel/ODC": base_travel * (1 + pct("Travel/ODC Margin %")),
        }
        subtotal = sum(totals.values())
        contingency = contingency_base * pct("Contingency %")
        oh = oh_base * pct("Overhead (OH) %")
        ga = ga_base * pct("G&A %")
        fee = fee_base * pct("Profit/Fee %")
        grand_total = subtotal + contingency + oh + ga + fee
        st.write({**totals, "Contingency": contingency, "OH": oh, "G&A": ga, "Fee/Profit": fee, "Grand Total": grand_total})
    # <<< inserted_margin_guidance >>>

    st.markdown("### ✅ Auto extract — page loaded")
    st.subheader("Auto extract key details")

with tabs[6]:
    st.markdown("### ✅ Data Export — page loaded")
    st.subheader("Data Export")

with tabs[7]:
    st.markdown("### ✅ Chat Assistant — page loaded")
    st.subheader("Chat Assistant (remembers context; accepts file uploads)")
# --- END SMOKE TEST MARKERS ---
...
with tabs[0]:
    ...
    with col3:
        ...
        default_assignee = st.selectbox(
            "Default assignee on save",
            options=["","Quincy","Charles","Collin"],
    ...
    if isinstance(df, pd.DataFrame) and not df.empty:
        ...
        grid = st.data_editor(
            df[["select"] + show_cols], use_container_width=True,
        ...
        if pick_id and "sam_notice_id" in df.columns:
            ...
            if row.empty:
                ...
                if atts:
                    ...
                    st.dataframe(
                        df_atts, use_container_width=True,
    ...
    # Bottom button only
    if st.button('Search SAM now', key='sam_search_btn_bottom'):
        open_browse = (not manual_naics.strip()) and (not keyword.strip()) and search_all
        kword = (keyword or "*") if open_browse else (keyword or None)
        df, info = sam_search(
            naics_for_query, min_days=int(min_days), limit=400,
            keyword=kword, posted_from_days=int(posted_from_days),
            notice_types=",".join(picked_notice), active="true"
        )
        if not info.get("ok"):
            st.error(f"SAM API error: {info}")
        else:
            # keep same client side filters as safety
...
# ================== RFP Analyzer Tab ==================
rfp_tabs = st.tabs(["RFP Analyzer"])
with rfp_tabs[0]:
    st.markdown("### ✅ RFP Analyzer — grade and chat")
    ...
    def load_rfp_text(files, pasted):
        texts = []
        if files:
            for f in files:
    ...
    def grade_rfp(text):
        # Simple rule based scoring with explain
        text_l = text.lower()
        def has_any(keys):
            return any(k in text_l for k in keys)

        criteria = []
        def add(name, weight, present, detail):
            score = weight if present else 0
            criteria.append({"criterion": name, "weight": weight, "score": score, "detail": detail})

        add("Statement of Work present", 10, has_any(["scope of work", "statement of work", "sow"]), "Looked for Scope of Work or SOW")
        add("Submission instructions present", 15, has_any(["submit", "submission", "emailed to", "upload on", "hand delivered", "mailed to"]), "Looked for how to submit")
        add("Due date present", 15, has_any(["due", "offers due", "proposal due", "closing date", "response date"]), "Looked for due date language")
        add("Evaluation criteria present", 15, has_any(["evaluation criteria", "best value", "lowest price", "lpta", "tradeoff", "evaluation factors"]), "Looked for evaluation factors")
        add("Past performance section", 10, has_any(["past performance", "relevant experience", "similar work"]), "Looked for past performance")
        add("Pricing instructions", 10, has_any(["pricing", "price schedule", "clins", "quote", "bid schedule", "cost proposal"]), "Looked for pricing or CLINs")
        add("NAICS or PSC present", 5, has_any(["naics", "psc"]), "Looked for NAICS or PSC codes")
        add("Period of performance present", 5, has_any(["period of performance", "pop:", "base year", "option year"]), "Looked for PoP")
        add("Attachments referenced", 5, has_any(["attachment", "amendment", "amdt", "ammendment", "addendum"]), "Looked for attachments or amendments")
        add("Compliance clauses present", 10, has_any(["far", "dfars", "provision", "clause", "52."]),"Looked for FAR DFARS provisions")

        total_weight = sum(c["weight"] for c in criteria)
        total_score = int(round(sum(c["score"] for c in criteria) * 100.0 / total_weight)) if total_weight else 0
        return total_score, criteria

    st.caption("Upload the RFP and paste any text below")
    rfp_files = st.file_uploader("Upload RFP files", type=["pdf","docx","txt"], accept_multiple_files=True, key="rfp_files")
    pasted = st.text_area("Paste RFP text", height=200, key="rfp_paste")

    c1, c2 = st.columns(2)
    with c1:
        do_grade = st.button("Grade RFP", key="rfp_grade_btn")
    with c2:
        clear_chat = st.button("Clear Q&A", key="rfp_clear_chat")

    if "rfp_source_text" not in st.session_state:
        st.session_state["rfp_source_text"] = ""

    if do_grade:
        source_text = load_rfp_text(rfp_files, pasted)
        st.session_state["rfp_source_text"] = source_text
        if not source_text.strip():
            st.warning("No text found. Upload a file or paste content.")
        else:
            score, crit = grade_rfp(source_text)
            st.metric("Overall score", f"{score}/100")
            dfcrit = pd.DataFrame(crit)
            st.dataframe(dfcrit, use_container_width=True)

    st.markdown("#### Chat Q&A about this RFP")
    if "rfp_chat" not in st.session_state:
        st.session_state["rfp_chat"] = []

    if clear_chat:
        st.session_state["rfp_chat"] = []

    # Simple retrieval QA over the source text
    def qa_answer(question, source):
        if not source.strip():
            return "No RFP text loaded yet."
        # Split into sentences and rank by keyword overlap
        import re as _re
        sents = [s.strip() for s in _re.split(r'(?<=[\.\!\?])\s+', source) if s.strip()]
        q = question.lower()
        # Score sentences by shared words
        import math
        words = [w for w in _re.findall(r"[a-z0-9]{3,}", q)]
        def score_sent(s):
            sl = s.lower()
            return sum(1 for w in words if w in sl)
        scored = sorted(((score_sent(s), s) for s in sents), reverse=True)
        best = [s for sc, s in scored[:5] if sc > 0]
        if not best:
            return "I did not find a clear match in the text. Try rephrasing."
        return "\n".join(best)

    # Render chat history
    for m in st.session_state["rfp_chat"]:
        with st.chat_message(m["role"]):
            st.write(m["content"])

    question = st.chat_input("Ask a question about the loaded RFP")
    if question:
        st.session_state["rfp_chat"].append({"role":"user","content":question})
        answer = qa_answer(question, st.session_state.get("rfp_source_text",""))
        st.session_state["rfp_chat"].append({"role":"assistant","content":answer})
        st.rerun()
