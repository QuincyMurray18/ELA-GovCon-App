...
if not default_state and default_pop:
    m = re.search(r"\b([A-Z]{2})\b", default_pop.upper())
    if m:
        default_state = m.group(1)
    ...
    if df_v.empty:
        ...
        select_all = st.checkbox("Select all filtered vendors", key="vend_select_all")
        selected_ids = []
        if select_all:
            selected_ids = [int(x) for x in df_v["id"].tolist()]
        else:
